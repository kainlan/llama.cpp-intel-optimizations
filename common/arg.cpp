...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    auto is_meta_exit_arg = [](std::string arg) {
        if (arg.compare(0, 2, "--") == 0) {
            std::replace(arg.begin(), arg.end(), '_', '-');
        }
        return arg == "-h" || arg == "--help" || arg == "--usage" || arg == "--version" || arg == "-cl" ||
               arg == "--cache-list" || arg == "--completion-bash";
    };

    bool meta_exit_requested = false;
    for (int i = 1; i < argc; ++i) {
        if (is_meta_exit_arg(argv[i])) {
            meta_exit_requested = true;
            break;
        }
    }

    // handle environment variables
    // Metadata/help commands must not let unrelated env defaults initialize GPU backends before exiting.
    if (!meta_exit_requested) {
        for (auto & opt : ctx_arg.options) {
            std::string value;
...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(common_arg({ "-fit", "--fit" }, "[on|off]",
#ifdef GGML_USE_SYCL
                       string_format("whether to adjust unset arguments to fit in device memory ('on' or 'off', "
                                     "disabled for SYCL unified cache, default: '%s')",
        ...
        string_format("whether to adjust unset arguments to fit in device memory ('on' or 'off', default: '%s')", params.fit_params ? "on" : "off"),
                       ...
                       [](common_params & params, const std::string & value) {
                           ...
                           if (is_truthy(value)) {
                               fprintf(
                                   stderr,
                                   "warning: --fit is disabled for SYCL builds; unified cache owns memory placement\n");
