...
const std::vector<ggml_type> kv_cache_types = {
    ...
    GGML_TYPE_Q8_0,    GGML_TYPE_Q4_0, GGML_TYPE_Q4_1, GGML_TYPE_IQ4_NL, GGML_TYPE_Q5_0, GGML_TYPE_Q5_1,
};
...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(
        common_arg({ "--paged-attn", "-pa" },
                   string_format("[EXPERIMENTAL] enable PagedAttention with block-based KV addressing (default: %s)\n"
                                 ...
                                 params.paged_attn ? "true" : "false"),
                   [](common_params & params) { params.paged_attn = true; })
            .set_env("LLAMA_ARG_PAGED_ATTN"));
    add_opt(common_arg({ "--prefix-cache", "-pc" },
                       string_format("[EXPERIMENTAL] enable prefix caching for paged attention (default: %s)\n"
                                     ...
                                     "requires --paged-attn to be enabled",
                                     params.prefix_cache ? "true" : "false"),
                       [](common_params & params) { params.prefix_cache = true; })
    ...
    add_opt(
        common_arg({ "--paged-layout" },
                   "use paged KV cache layout for long sequences (requires --flash-attn, enables Paged Attention V2)",
                   [](common_params & params) { params.paged_layout = true; })
            .set_env("LLAMA_ARG_PAGED_LAYOUT"));
    add_opt(common_arg({ "--gpu-sampling" }, "run sampling on GPU (SYCL only, avoids CPU sync during token generation)",
                       [](common_params & params) { params.gpu_sampling = true; })
    ...
    add_opt(common_arg({ "-sm", "--split-mode" }, "{none,layer,row,tp,pipeline,hybrid}",
    ...
    add_opt(common_arg({ "--sycl-debug" }, "LEVEL",
    ...
    add_opt(common_arg({ "--pp-size" }, "N",
                       string_format("number of pipeline stages for pipeline parallelism (0 = auto, default: %d)",
                                     params.pp_size),
                       [](common_params & params, int value) { params.pp_size = value; })
                .set_env("LLAMA_ARG_PP_SIZE"));
    add_opt(
        common_arg({ "--tp-size" }, "N",
                   string_format("tensor parallelism degree within each pipeline stage for hybrid mode (default: %d)",
                                 params.tp_size),
                   [](common_params & params, int value) { params.tp_size = value; })
            .set_env("LLAMA_ARG_TP_SIZE"));
    add_opt(common_arg({ "--pp-chunk-size" }, "N",
                       string_format("max tokens per prefill chunk for chunked prefill (0 = disabled, default: %d)",
                                     params.pp_chunk_size),
                       [](common_params & params, int value) { params.pp_chunk_size = value; })
                .set_env("LLAMA_ARG_PP_CHUNK_SIZE"));
    add_opt(common_arg({ "--pp-chunked-prefill" },
                       string_format("enable chunked prefill for pipeline parallelism (default: %s)",
                                     params.pp_chunked_prefill ? "true" : "false"),
                       [](common_params & params) { params.pp_chunked_prefill = true; })
    ...
    add_opt(common_arg({ "--gpu-memory-pct" }, "N",
    ...
    add_opt(common_arg({ "--moe-profile" }, "PATH",
                       "path to save/load MoE expert usage profile for guided expert placement",
                       [](common_params & params, const std::string & value) { params.moe_profile_path = value; })
                .set_env("LLAMA_ARG_MOE_PROFILE"));
    add_opt(common_arg({ "--moe-warmup" }, "N",
    ...
    add_opt(common_arg({ "--expert-cache-size" }, "N",
    ...
    add_opt(common_arg({ "--unified-cache-pct" }, "N",
    ...
    add_opt(common_arg({ "--unified-cache-host-pct" }, "N",
    ...
    add_opt(common_arg({ "--weight-granularity" }, "N",
                       string_format("weight cache granularity: 0=layer, 1=tensor, 2=layer_group (default: %d)",
                                     params.weight_granularity),
                       [](common_params & params, int value) {
                           if (value < 0 || value > 2) {
    ...
    add_opt(common_arg({ "--kv-offload-auto" }, "enable KV cache offload to CPU for long contexts",
                       [](common_params & params) { params.kv_offload = true; })
                .set_env("LLAMA_ARG_KV_OFFLOAD_AUTO"));
    add_opt(common_arg({ "--kv-offload-threshold" }, "N",
                       string_format("context length (tokens) to trigger KV offload (default: %d)",
    ...
    add_opt(common_arg({ "--kv-gpu-pct" }, "N",
                       string_format("percentage of KV cache to keep on GPU when offloading (default: %.1f)",
                                     params.kv_gpu_pct),
                       [](common_params & params, int value) {
                           if (value < 1 || value > 100) {
