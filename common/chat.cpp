...
static bool common_chat_extra_context_true(const json & extra_context, const std::string & key) {
    // adapted from the nlohmann find()/iterator form to the common_json facade
    // (contains()/at()), which upstream's json migration made the `json` alias
    if (!extra_context.is_object() || !extra_context.contains(key)) {
        return false;
    }

    const auto & value = extra_context.at(key);
    if (value.is_boolean()) {
...
static common_chat_params common_chat_params_init_gpt_oss(const common_chat_template &    tmpl,
    ...
    data.format            = COMMON_CHAT_FORMAT_PEG_NATIVE;
    data.supports_thinking = true;
    // Fork's Harmony boundary tags adapted to the plural field type; the
    // canonical GPT-OSS B50 chat gate is the arbiter of this pair (Phase C
    // merge ruling llama.cpp-90ns c-478m — upstream's pair is the fallback).
    data.thinking_start_tag = "<|start|>assistant";
    data.thinking_end_tags  = {"<|channel|>final<|message|>"};
