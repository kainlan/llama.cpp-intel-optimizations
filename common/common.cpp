...
char * common_get_model_or_exit(int argc, char * argv[]) {
    ...
    if (!path || strlen(path) == 0) {
        // One named skip policy, actually called: exit 77 so ctest reports
        // "Skipped", never a vacuous green (llama.cpp-nwip; the source gate
        // tests/test-skip-policy.py asserts this call by name).
        test_skip_no_model();
    }
...
struct llama_context_params common_context_params_to_llama(const common_params & params) {
    ...
    cparams.n_ubatch_auto     = params.n_ubatch_auto;
