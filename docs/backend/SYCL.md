...
2. The device link -- libggml-sycl, every test executable that embeds the backend objects, and with `GGML_SYCL_BUILD_XMX_TESTS=ON` the XMX test executables -- is almost all ocloc AOT-compiling ~112 split images per Battlemage target. These three settings change only how those compiles are scheduled and reused, never what they produce. Every device link runs through `ggml/src/ggml-sycl/sycl-device-link.sh`, which also gives it a private `TMPDIR` that it removes on exit. Measured on the libggml-sycl link: 1519 s serial, 643 s with 8 jobs, 27 s with a warm cache. Size the pool by memory: one link peaks at roughly 8 jobs x 1.4 GB. Without the pool, only `ninja -j` (nproc by default) limits how many links start together. `./scripts/sycl-build.sh --dev [target...]` builds only llama-bench, llama-cli, llama-completion and the named targets, which skips the ~48 test-executable device links a bare build pays for.
3. The cache lives in `${GGML_SYCL_OCLOC_CACHE_ROOT:-~/.cache/ggml-sycl-ocloc}`, one directory per toolchain (icpx, ocloc, the IGC libraries and their package versions), so a compiler or driver upgrade never reads an older entry. A hit returns the stored binary bit for bit. **The first variant cached sticks:** IGC's codegen for some images (the ESIMD flash-attention kernels and the MXFP4 MoE DPAS kernels, on ocloc 26.31) differs from one uncached compile to the next, so without the cache every rebuild re-rolls that machine code; with it, the variant stored first is what every later link embeds until the toolchain or that image's SPIR-V changes. To draw a fresh variant, delete the toolchain's cache directory or configure with `-DGGML_SYCL_OCLOC_CACHE=OFF`. `scripts/sycl-device-image-inventory.sh` compares the device images of two builds. On disk, a full libggml-sycl link stores ~190 MB. A link keeps its own toolchain's directory and the most recently used other one (the toolchain a rollback returns to) and removes older ones. Each is capped by NEO at `GGML_SYCL_OCLOC_CACHE_MAX_SIZE` (default 4 GiB), so the cache stays under 8 GiB. When the cache's filesystem has less than `GGML_SYCL_OCLOC_CACHE_MIN_FREE` free (default 4 GiB), a link warns and uses a throwaway cache in its private `TMPDIR` instead, so it compiles every image but adds nothing to the full disk. It does the same when any `IGC_*` regkey or `NEOReadDebugKeys` is set: those change or dump the ISA and no cache key covers the environment, so a cached link would dump nothing, or store a non-default image for every later build. The launcher's variables are listed in `docs/backend/sycl-env-vars.md`.

**Pending verification (llama.cpp-vuy0): off by default.** With `GGML_SYCL_CCACHE_BASE_DIR=1`, `scripts/sycl-build.sh` runs ccache with `base_dir` set to the source tree (ccache 4.8 or newer), so a checkout at another path -- a git worktree -- is meant to reuse the compile cache instead of starting cold. This has not yet run on a real SYCL build. The default flips only after these pass: a byte-identical device-image inventory between tree A's `base_dir` build and tree B's cache-hit build; `strings | grep -F <tree A's absolute path>` on tree B's binaries finding nothing; a cache-hit build identical to a `CCACHE_RECACHE=1` build in tree B; the seven `LLAMA_CPP_SOURCE_ROOT` tests passing from a foreign working directory with tree A removed; and the Mistral completion gate. ccache applies `base_dir` by passing paths relative to the build directory, which makes `__FILE__` and `__builtin_FILE()` relative too (`../ggml/src/ggml-sycl/ggml-sycl.cpp`), so `GGML_ABORT` messages are expected to keep reading `ggml-sycl.cpp:<line>:`. The kernel profiler and the timeline record their callsites under that relative string, so **captures taken with and without `base_dir` key the same callsite under different strings** -- compare across that boundary by basename and line. Tests that read the source tree at run time locate it through the absolute `LLAMA_CPP_SOURCE_ROOT` their build defines. A `GGML_SYCL_PROFILING_DEBUG` build (`-g`) keeps plain ccache even when asked: `base_dir` would make its DWARF include directories relative, which `scripts/parse-sycl-zebin-line-table.py` cannot match against absolute paths.
...
| GGML_SYCL_MOE_PHASE_MATERIALIZE | unset (automatic), 0, or 1 | Automatically switch planner-owned MXFP4 gate/up tensors from PP-safe SOA to decode XMX_TILED only when the placement plan recorded PP SOA promotion and the device reports XMX INT8 support. Per-tensor role/shape/residency checks remain fail-closed. Set `0` to disable the automatic phase switch or `1` to force the diagnostic path. |
| GGML_SYCL_MOE_PHASE_BULK_XMX | unset (automatic), 0, or 1 | Use whole-tensor XMX materialization for the automatic phase route. Complete gate/up pairs must be available together; a failed conversion demotes the pair and attempts to restore SOA. Set `0` to force the slower per-expert diagnostic path or `1` to force bulk materialization. The grouped-decode diagnostic may still require bulk materialization independently. |
| GGML_SYCL_MOE_DOWN_SUM_DIRECT | 0 (default) or 1 | Experimental direct down-projection weighted-sum path. It remains opt-in because the current B50 ordered ablation added less than 5% TG and its down-bias artifact ownership still requires promotion-grade validation. |
