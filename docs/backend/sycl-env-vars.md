...
| `GGML_SYCL_NONFA_ATTN_SCRATCH_MB=<MB>` | New in llama.cpp-oyfl. Overrides `unified_cache_nonfa_attn_scratch_demand_bytes()`'s shape-derived floor for the non-flash-attention batched mul_mat path's scratch demand: `demand = max(16 MiB, 1.5 x n_head x n_ubatch x n_ctx x sizeof(f16))`. This is the SCRATCH-zone sibling of `GGML_SYCL_ONEDNN_GRAPH_ZONE_MB` above, sized by analogy (same `scoped_unified_queue_temp` async-release architecture, same 1.5x concurrency factor carried over rather than independently measured — see `docs/backend/sycl-memory-design.md`'s "A non-tensor consumer" subsection) but tracked separately: this consumer is unconditional (not gated behind `GGML_SYCL_DNNL`), and it feeds `ggml_backend_sycl_set_runtime_context()`'s runtime refusal, not only the plan-time zone-raise the ONEDNN sibling above uses. An explicit non-negative value always wins over the formula. Setting this too low can make a context that would otherwise be refused (with the size arithmetic, `-c N` remediation) instead abort mid-prefill in `ggml_sycl_mul_mat_batched_sycl()`'s fallback ladder — this override is for tuning a workload the formula mis-sizes, not for silencing the refusal. |
