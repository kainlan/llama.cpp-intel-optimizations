...
| `GGML_SYCL_MMVQ_Q8_SOA_FASTPATH=0` | ON | **Q8_0 SOA MMVQ fast path (llama.cpp-6cgq; renamed from `GGML_SYCL_MMVQ_Q8_SOA_SMALLN` in spec review round 2 once the fix stopped being small-N-only).** Two changes shipped together, gated by the same variable: (1) the ROOT FIX -- `mul_mat_vec_q8_0_soa_scale_slm` (`mmvq.cpp`) stages each row's D-plane (per-block fp16 scale) through SLM once per row instead of reading it from global memory inside the main loop. Byte-level analysis (posted on the ticket before implementing) found the SOA kernel's QS-plane reads already perfectly coalesced (256 contiguous bytes per sub-group per iteration), but the D-plane reads are 8 separate, redundant, 2-byte-per-lane scattered small loads spanning only 16 contiguous bytes of genuinely distinct data per iteration -- this is the actual source of the measured SOA-vs-COALESCED gap, not occupancy. (2) a complementary occupancy lever -- `ggml_sycl_mmvq_q8_0_soa_geometry` (`mmvq-launch-geometry.hpp`) spreads small-nrows rows across more, smaller work-groups to raise the work-group count toward `2x` the device's core count (relaxed for wide-K shapes, which already hide their own latency within a sub-group); GPU verification found this alone worth only ~1 percentage point (occupancy was never the dominant limiter, and raising it can add L1 contention among the same scattered D-plane reads, roughly cancelling the benefit) -- kept because it is free at large N (never launches fewer work-groups than today) and additive on top of the D-plane fix. Both apply UNCONDITIONALLY whenever this variable is on, at every N (not gated on the geometry differing from 16): the D-plane fix helps even where the geometry stays at today's fixed 16. `=0` restores the original kernel body and its fixed-16 geometry byte-for-byte at every N, for an A/B against the change. Only affects the SOA arm; the COALESCED kernel and every other reorder MMVQ launch (Q4_0, MXFP4) in this file are unmodified. |
...
| `GGML_SYCL_Q8_DENSE_LAYOUT=soa` | coalesced | **Opt-in (llama.cpp-nz1k, prefill L2b phase 1).** Materialize Q8_0 dense projections (ATTENTION_WEIGHT/FFN_WEIGHT/OUTPUT_WEIGHT — `layout_policy::get_optimal` in `common.hpp`) SOA instead of COALESCED. Only the exact value `soa` selects SOA; anything else is the default coalesced. One layout serves every consumer (ruling 5): TG runs the existing `DMMV_SOA`/`MMVQ_SOA` `mulmat.mmvq.q8_0_soa` kernels, PP lands on `MMQ_SOA` unless `GGML_SYCL_ONEDNN_WOQ_Q8=1` (next row) routes it to the oneDNN WoQ-int8 arm. EMBEDDING is untouched. Precedence: `GGML_SYCL_Q8_DENSE_AOS=1` is checked first and wins over this variable. The default is NOT flipped in this campaign (lead ruling nz1k c-c4jp): the flip condition — B50 tg128 Q8_0 on SOA within noise of COALESCED **and** pp512 up on both cards — is decided on llama.cpp-2zsc with the interleaved A/B numbers. |
| `GGML_SYCL_ONEDNN_WOQ_Q8=1` | OFF | **Opt-in (llama.cpp-nz1k, prefill L2b phase 1).** For a Q8_0 weight materialized SOA (see the previous row), PP batches (> `MMVQ_MAX_BATCH_SIZE`) select the new `ONEDNN_SOA` kernel choice (`pick_kernel_for_layout` / `k_mul_mat_priority`, `ggml-sycl.cpp`) and run probe llama.cpp-ovkn's V4 form through `DnnlGemmWrapper::woq_gemm_q8_0` (`gemm.hpp`): **f16 activations × the stored int8 qs plane read as-is** (logical (K,N) strides {1,K}), K/32 grouped f16 scales (mask (1<<0)\|(1<<1), groups {32,1}), `fpmath f16 apply_to_int` — measured equal to f16×f16 speed on the B70 and +25% on the B50, with **no per-call weight dequant and half the weight bytes**. The only per-call staging is the scale-plane transpose `[N][K/32] → [K/32][N]` (`q8_0_soa_scale_plane_to_kbn_sycl`, `convert.cpp`; 2 bytes per 32 weights, 1/32 of the 64 bytes per 32 weights the f16 dequant wrote; it uses at most 1/32 of the dequant-sized PP scratch reservation the caller already holds) into the existing oneDNN PP weights scratch (`acquire_onednn_pp_scratch`, ruling 1 — no new allocator). The SOA d-plane order bound directly is accepted-but-garbage (probe V5) and is never passed. The arm applies to any Q8_0 weight the planner materialized SOA (an already-SOA weight such as a tile-misaligned Q8_0 head takes it under this variable alone) and applies the codebase's standard eligibility predicate (`ggml_sycl_can_use_layout_for_kernel`, the same gate every kernel-selection site uses) before its single lookup. Declines — never errors — into the SOA f16 dequant + `row_gemm` when rows are partial, K is not a multiple of 32, the SOA plane is not device-resident, the PP scratch is absent, or oneDNN refuses the primitive/throws (the primitive is checked before the scale staging is submitted, so a declining shape pays no transpose); `GGML_SYCL_MUL_MAT_ROUTE_TRACE=1` prints `[MUL-MAT-ROUTE] onednn_woq_q8 ... arm=woq_q8\|dequant_f16 decline=<reason>` per dispatch. Kernel-profiler rows: `mulmat.onednn_woq_q8.execute` (the GEMM) and `mulmat.onednn_woq_q8.stage_scales`. Phase 1 is an opt-in interim only (per-call staging is not design-compliant as a default); phase 2 (llama.cpp-2zsc) stores the d plane in `[K/32][N]` order, retires the transpose, and owns the default decision. Default and no-env behaviour is byte-identical to before. Source-gated by `tests/test-sycl-onednn-woq-q8-source.py`. **MEASURED 2026-09-03 (510de3676, one binary, 3 interleaved pairs per card, Mistral 7B Q8_0 `-p 512 -n 128 -r 2`):** B50 pp512 1145.8 → **2039.5 (+78%)**, tg128 27.23 → 26.78 (−1.7%, real: run spread 0.1%); B70 pp512 2403.8 → **5241.4 (+118%)**, tg128 67.95 → 66.30 (−2.4%). Profiler: `mulmat.onednn_woq_q8.execute` at 119–135 TOPS on the three large Mistral shapes (probe predicted 124–127), scale staging = 10% of GEMM time. All correctness gates green on both cards with the arm engaged on 221/221 PP dispatches. Those figures predate the nz1k review rounds (bf2988282..e411bc505 changed the arm's entry condition and split the GEMM wrapper); **re-verified at e411bc505 (fresh binaries, all nine gates green, arm 221/221, one interleaved anchor pair per card):** B50 pp512 1124.7 → 2005.7 (+78%), tg128 26.68 → 26.26 (−1.6%); B70 pp512 2304.3 → 4865.9 (+111%), tg128 65.31 → 63.26 (−3.1%) — same ratios, absolutes a few percent lower in BOTH arms because the codescout embedder was re-indexing on the CPU during the run. The B50 decode cost is why the default is still coalesced: the flip is decided on llama.cpp-2zsc. |
...
| `GGML_SYCL_ONEDNN_GRAPH_ZONE_MB=<MB>` | Default **64** (was 512 until llama.cpp-gwno's event_complete() fix — see below). Extra headroom `unified_cache_get_planned_onednn_scratchpad_bytes()` adds, additive, on top of the primitive-API weights+activations estimate, to hold the Graph API SDPA allocator's (llama.cpp-gwno, `GGML_SYCL_ONEDNN_CACHE_ALLOCATOR` above) outstanding scratch demand. ⚠️ **512 was sized for the wrong model**: it assumed many compiled-partition buffers could be concurrently outstanding across a ubatch (their frees only bulk-drained once per ubatch) — true only while the free path deferred reclaim until each buffer's completion event fired. Once that blocking `event_complete()` check was removed from the zone-backed free path (`unified_cache::onednn_graph_scratch_free`'s in-order-queue comment — reclaim is immediate now, safe because every Graph-scratch consumer submits on the same in-order compute queue), at most ~1 buffer is outstanding at a time in practice. Measured high-water (`onednn_graph_scratch_high_water_bytes()`, logged once at cache teardown): 12.0 MB on a B70 gemma4 pp512 run, 3.0 MB on a B50 two-model (gemma4-Q8_0 + mistral-Q4_0) `GGML_SYCL_STRICT_LEASES=1` run. 512 was not just wasteful, it was an active landing blocker: a second model's load-time zone-growth request (its own weights+activations pair plus the 512 MB floor) could exceed what `ensure_planned_arena_zones()` can grow into while the first model's leases are still live, aborting with `[VRAM-ARENA] planned zones exceed active arena but live allocations prevent rebuild` on a run that passes on master (which never needs this zone to grow). 64 MiB (~5x headroom over the largest measured high-water) clears the same run with margin. A flat, env-tunable floor rather than a per-model formula (there is no cheap way to derive "how many buffers can be concurrently outstanding" from the tensor-inventory planning call site). Raise this if `unified_cache::onednn_graph_scratch_alloc`'s "did not fit the ONEDNN zone" `GGML_LOG_WARN` fires, if the lead's `zeMemAllocDevice` uprobe count is nonzero during the benchmark phase, or if a multi-model/strict-leases run aborts sizing the VRAM arena. Only takes effect at planning time, before weights load — has no effect mid-inference. |
