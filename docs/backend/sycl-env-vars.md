...
| `GGML_SYCL_MOE_PP_ONEDNN_F16_BATCHED=1` | OFF | The batched oneDNN PP executor for MXFP4 SOA MoE experts (`try_pp_mxfp4_soa_onednn_f16_batched`, `ggml-sycl.cpp`) — batches every active expert's dequant+GEMM into one oneDNN call instead of one call per expert. Requests its device scratch ring (`moe-scratch-admission.hpp`) sized for ALL simultaneously active experts, not one; the planner (`llama_model_sycl_populate_inventory`, `src/llama-model.cpp`) now reserves that worst case — weight slot at `n_expert` x per-expert fp16 bytes (~506 MB for GPT-OSS 20B), activation/output slots at `n_ubatch * n_expert` rows rather than `n_ubatch * n_expert_used` (imbalanced routing can put up to `n_expert` rows-worth on one expert while `n_expert - 1` others still count toward `active_count`). Admission is fail-closed since `f2bdfbffe` — an under-sized plan silently downgrades every dispatch to the serialized MoE route rather than growing the zone (llama.cpp-dboi). Still OFF by default; brought up under explicit `=1` pending on-hardware VRAM/perf validation (baseline on `79ae63559`: 913 tok/s B50 / 1285 tok/s B70 PP with a since-removed unbounded-growth version of this ring). |
