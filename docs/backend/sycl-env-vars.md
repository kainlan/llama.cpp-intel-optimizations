...
| `GGML_SYCL_AUTO_UBATCH=0` | ON | Disable the `-ub auto` / no-`-ub` micro-batch selection trial added by llama.cpp-nphx Task 4b (this task, 4a, only wires the switch -- there is no trial yet to disable) -- falls back to the library default 512. An explicit `-ub N` always wins over the trial regardless of this variable. SYCL-only. |
