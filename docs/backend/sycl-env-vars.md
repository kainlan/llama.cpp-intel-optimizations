...
| `GGML_SYCL_STORED_GEMM_DEBUG=1` | Default OFF (`mxfp4-stored-gemm.cpp`, llama.cpp-6f73 plan Task G4, the non-dispatched stored-SOA small-M MXFP4 GEMM kernel). Prints one `[mxfp4-stored-gemm]` launch-geometry line per `ggml_sycl_mxfp4_soa_gemm_dpas` call: `global_range`/`local_range`, `work_groups(n_tiles)` (one work-group per 16-row N-tile), `rows_per_work_item(exec_n)` (16, the DPAS N execution size), `k_tiles_total`, and `k_tiles_per_work_item(max)` alongside `k_tiles_per_work_item(last_partition)` — the ceiling every partition but the last processes, and the clamped count the last partition actually does (they differ whenever `k_tiles` is not a multiple of `K_PARTITIONS`, e.g. 12 vs 6 at K=2880). Built for comparing this kernel's launch shape against the production reference kernel's (`mxfp4_pair_glu_xmx_tiled_dpas_m2`, `mmvq.cpp`) without a GPU-side profiler, and used by the follow-up task llama.cpp-kcya, which carries this kernel's unmet `>= 50%-of-peak` bandwidth criterion. Diagnostic only; no perf effect when unset. |
