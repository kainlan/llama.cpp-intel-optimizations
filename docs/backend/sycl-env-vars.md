...
| `GGML_SYCL_HANDLE_STRICT=1` | ⚠️ **Not yet wired** — reads its env var, but the `g_ggml_sycl_debug`-gated mismatch warnings in `ggml-sycl/common.hpp` don't check it yet. Once wired: default OFF; reports `ggml_tensor_extra_gpu` `data_handle`/`data_device` divergence (first 16 only) without needing `GGML_SYCL_DEBUG=1`. Diagnostic only, no perf effect. Plan: `docs/plans/2026-07-30-extra-device-indexed-handle-storage.md`. |
