...
| `GGML_SYCL_KERNEL_PROFILE=1` | Default OFF. Aggregate per-kernel-label device-time census (`sycl-kernel-profiler.cpp/.hpp`) — `GGML_SYCL_KERNEL_PROFILE_OUTPUT=<path>` writes CSV/JSON (`GGML_SYCL_KERNEL_PROFILE_FORMAT=csv\|json\|both`), `GGML_SYCL_KERNEL_PROFILE_RAW=1` emits per-event rows tagged with `timestamp_status` (`ok` for a real device-event read, `host_span_only` for a call site that has no `sycl::event` to attach to and records a host wall-clock span instead via `ggml_sycl_kernel_profile_record_host_span`; a site whose returned event covers only part of the real work — e.g. the oneDNN Graph SDPA execute at `fattn-onednn.cpp`, whose event's profiling window covers only the fused pattern's last kernel — still records through `record_event` with status `ok`, carrying the host submit span on the same sample, so read its device time as a lower bound). Reads real `sycl::event` device timestamps (`command_start`/`command_end`) via a deferred/pending-event flush, not at the submit site — the mechanism `GGML_SYCL_E2E_TG_PROFILE` below cannot reuse without the same redesign. Coverage as of llama.cpp-qmen (S6/I1): every decode MUL_MAT kernel family, the dense and MXFP4-batched oneDNN GEMM executors, the un-batched oneDNN GEMM path (`DnnlGemmWrapper::gemm`, label `mulmat.onednn_woq.execute` — the Q8_0/Q4_0 weight-dequant-then-GEMM route most prefill Q8_0 M≥512 GEMMs take), and the full RMS_NORM family (`norm.rms_norm`, `norm.rms_norm_mul`, `norm.rms_norm_mul_add`, `norm.add_rms_norm`) plus plain `norm.norm`/`norm.group_norm`. Zero overhead when unset for these S6 wraps specifically — the per-call metadata string is built only under `ggml_sycl_kernel_profile_enabled()`, matching the label-field assignments it belongs to (older per-batch-element sites in this file, e.g. `mxfp4.pp.gemm.execute`'s 2-D fallback loop, predate this convention and still build their metadata string unconditionally — a small per-call heap allocation even with the profiler off; not fixed here). |
| `GGML_SYCL_E2E_TG_PROFILE=1` | Default OFF (`e2e-profile.cpp/.hpp`). Per-stage (DISPATCH/MOE/ATTENTION/KV/ELEMENTWISE/GRAPH/CACHE/TRANSFER/…) host-time breakdown, one `[SYCL-E2E-TG-PROFILE]` summary plus `[SYCL-E2E-TG-STAGE]` detail lines per `graph_compute()` call (one token during TG, one ubatch during PP). ⚠️ **`device_us` in this instrument is HOST-ONLY today for every op-dispatch call site.** No caller supplies a genuine SYCL device timestamp; the accumulator and printed field are otherwise real (`test-sycl-e2e-profile.cpp` exercises them with a synthetic non-zero value end to end), and there is exactly one production non-zero producer today — `ggml-sycl.cpp:22472`'s `e2e_tg_profile_record_transfer("peer_host_bounce_measure", bytes, /*host_us=*/0.0, link.host_bounce_us)`, which passes a **host chrono span** around a host-mediated peer-link bounce copy through the `device_us` parameter (not a device event either — a host measurement given the device-time slot because that transfer has no on-device kernel of its own to time). Supplying a real device timestamp at the call site that matters most for coverage — the dominant per-op bracket (`e2e_tg_scope` around `ggml_sycl_compute_forward`'s dispatch switch) — would mean reading `sycl::event::get_profiling_info<command_start/command_end>` on an event that has usually not yet completed — an implicit host wait the no-host-waits architecture rule forbids on that hot path, and which `tests/test-sycl-e2e-profile-compute-forward-source.py`/`-fattn-source.py`'s `assert_no_waits` guards catch. `GGML_SYCL_KERNEL_PROFILE` above solves the equivalent problem correctly via a deferred flush; this instrument has no such mechanism (adding one is a real redesign, tracked as future work, not done here). Treat `host_us`/`total_host` as the real signal and `device_us`/`total_device`/`host_gap` (currently computed as `wall - total_device`, i.e. ≈ `wall` for any run without a peer-link bounce probe) as vestigial until a caller is documented to supply a real value. |
