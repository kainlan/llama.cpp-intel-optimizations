...
| `GGML_SYCL_Q8_DENSE_AOS=0` | ON | Restore COALESCED layout for Q8_0 dense projections (ATTENTION_WEIGHT/FFN_WEIGHT/OUTPUT_WEIGHT — `layout_policy::get_optimal` in `common.hpp`). Default (`=1`) routes these AOS instead: `pick_kernel_for_layout`'s COALESCED case (`ggml-sycl.cpp`) has no oneDNN arm, so a COALESCED Q8_0 dense batch always lands on `MMQ_COALESCED` (~3 TFLOPS); AOS at PP batch sizes reaches `ONEDNN_AOS`, the oneDNN jit:gemm arm measured ~3x faster for PP. TG moves from DMMV/MMVQ_COALESCED to MMVQ_AOS — verify TG has not regressed before trusting a PP win on this flag (llama.cpp-e3xj, 2026-08-19; the single-layout rule means one layout must serve both PP and TG). Only affects Q8_0; MoE expert weights (`layout_policy`'s `MOE_EXPERT_WEIGHT` branch) already route AOS/SOA separately and are untouched by this flag. |
