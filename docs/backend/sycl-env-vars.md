...
| `GGML_SYCL_MMVQ_Q8_SOA_SMALLN=0` | ON | **Small-N occupancy fix (llama.cpp-6cgq).** At a fixed row count the Q8_0 SOA and COALESCED MMVQ launches (`reorder_mul_mat_vec_q8_0_q8_1_sycl` / `coalesced_mul_mat_vec_q8_0_q8_1_sycl`, `mmvq.cpp`) build byte-identical nd_ranges — occupancy is not the source of SOA's measured deficit at small N; COALESCED's word-major tiling reaching ~100% cache-line utilization per fetch against SOA's ~50% is, and that per-fetch waste only costs wall time when too few concurrent work-groups are in flight to hide the resulting latency. Default ON spreads small-nrows rows across more, smaller work-groups (`ggml_sycl_mmvq_q8_0_soa_geometry`, `mmvq-launch-geometry.hpp`) to raise that count toward `2x` the device's core count (relaxed for wide-K shapes, which already hide their own latency within a sub-group) — a launch-geometry-only change, no scratch, no second kernel, safe under SYCL graph record/replay. Large-N shapes (already at or near the bandwidth ceiling) are untouched: the helper returns today's fixed 16 sub-groups-per-work-group there. `=0` restores today's fixed-16 geometry byte-for-byte at every N, for an A/B against the change. Only affects the SOA arm; the COALESCED kernel and every other reorder MMVQ launch (Q4_0, MXFP4) in this file are unmodified. |
