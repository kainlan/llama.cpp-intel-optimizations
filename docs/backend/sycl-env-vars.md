...
| `GGML_SYCL_ONEDNN_PP_MIN_BATCH=N` | Min batch (`src1->ne[1]`) for the dense dispatcher's oneDNN PP route, default 16. It does **not** apply to the MXFP4 direct route, whose floor is fixed at 2 (`onednn_pp_min_batch_for` in `onednn-pp-placement.hpp`, llama.cpp-je3b): that route's fallbacks are per-row MMVQ and the unified kernel, not MMQ, and its arms have admitted every non-decode batch since they were written. |
