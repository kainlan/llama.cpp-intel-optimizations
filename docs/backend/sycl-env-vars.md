...
| `GGML_SYCL_FLASH_ATTN_GRAPH_ALLOW` | unset = FORCE_OFF | Three-way override for whether decode `FLASH_ATTN_EXT` nodes are allowed into SYCL command-graph recording/replay (llama.cpp-dyi3). **Owner ruling (round 5, decode graph replay proven wrong on hardware — see the task's comment log): unset is FORCE_OFF, today's master behavior (per-op dispatch, no graph for FA-bearing decode graphs), until replay is proven correct.** `=auto` opts into the observation-gated AUTO mode: the graph engages only for decode-shape FA dispatches this context has actually observed reaching the graph-replay-verified ESIMD partitioned decode kernel (`fa_decode_kernel_observation`, `common.hpp`) — any other observed kernel family (oneDNN SDPA, tile_d512, VEC safe-decode, XMX, …) keeps the exclusion. `=1` (or any other nonzero integer) force-engages the graph regardless of the observation — the original diagnostic-only behavior, for FA shapes/paths never verified replay-safe; `=0` is an explicit force-off, equivalent to unset. Do not set `=auto` or `=1` outside a controlled diagnostic run: the known-wrong replay produces fluent-looking but incorrect output (not a crash), so a silent correctness regression is the failure mode, not a visible error. |
...
| `GGML_SYCL_GRAPH_RECORD_PLAIN=<comma list>` | Default unset (llama.cpp-dyi3, `common.hpp`). Per-family bisect switch for the same investigation: forces the listed families' op dispatch onto their normal NON-recording code path even while a SYCL command-graph recording is genuinely active, to isolate which family's recording-mode branch produces wrong output. Currently wired families, each only at call sites individually verified safe (no illegal wait()/throw/malloc during a real recording): `fattn`, `getrows`, `memops`. `mmvq` and `binbcast` are recognized names but deliberately **not** wired to any call site — a hand review found several of their recording-mode checks exist specifically to avoid an operation illegal during real recording (a blocking `.wait()`/`throw`), so a naive bypass there could crash or hang rather than just warn; requesting them prints a loud "NOT YET SAFELY IMPLEMENTED" warning instead of silently no-op'ing. Where a bypassed family's plain path submits something non-graph-capturable, it fires the `[SYCL-GRAPH-CENSUS]` warning below — expected during a bisect, not itself a correctness signal. |
| *(census, no env var)* | `mem_copy_direct_submit` (`mem-ops.cpp`) warns once per call site, `[SYCL-GRAPH-CENSUS] mem_copy_direct_submit (non-capturable) called during recording at <file>:<line> (<function>)…`, whenever a copy reaches this non-graph-capturable path (profile label `sycl.memcpy.mem_ops`) while `ggml_sycl_graph_recording_active()` is true — such a copy executes once at record time and is never re-issued on replay. Fires automatically; no env gate. |
