...
| `GGML_SYCL_ONEDNN_GRAPH_ZONE_MB=<MB>` | Default **512**. Extra headroom `unified_cache_get_planned_onednn_scratchpad_bytes()` adds, additive, on top of the primitive-API weights+activations estimate, to hold the Graph API SDPA allocator's (llama.cpp-gwno, `GGML_SYCL_ONEDNN_CACHE_ALLOCATOR` above) peak *concurrent* within-ubatch scratch demand — many compiled-partition buffers can be outstanding at once (their deferred frees are only bulk-drained roughly once per ubatch), unlike the one reused primitive-API reservation. A flat, env-tunable floor rather than a per-model formula (there is no cheap way to derive "how many attention layers can be concurrently in flight" from the tensor-inventory planning call site). Raise this if `unified_cache::onednn_graph_scratch_alloc`'s "did not fit the ONEDNN zone" `GGML_LOG_WARN` fires, or if the lead's `zeMemAllocDevice` uprobe count is still nonzero during the benchmark phase after the allocator change. Only takes effect at planning time, before weights load — has no effect mid-inference. |
