...
| `GGML_SYCL_FA_ONEDNN_MATERIALIZE=0` | Plan `DIRECT` instead of `MATERIALIZE_REQUIRED` for GQA/MQA flash-attention shapes whose K/V token stride differs from D (the `KV_NC_STRIDE_MISMATCH` gate in `ggml_sycl_flash_attn_ext_onednn_plan()`, `fattn-onednn.cpp` — the decision `llama.cpp-l7rt` refers to as `:182`, which the toggle itself has since shifted), skipping the dense f16 K/V repack before the oneDNN SDPA execute. Default **ON**; unset and `=1` are identical. This is an A/B axis, **not** a setting to flip — whether `DIRECT` should become the default is an open owner decision. It exists so the comparison runs as one binary and one library: the earlier measurement patched the predicate and loaded a second `libggml-sycl.so` through `LD_LIBRARY_PATH`, which moved the source and the loaded object together and produced a heap-corruption abort that was wrongly attributed to the `DIRECT` path (`llama.cpp-l7rt`). `DIRECT` is numerically correct at D=16 and D=128 within the descriptors fixture's tolerance and end-to-end on the Mistral digit gate; on B50 pp512 it measured at worst parity across five interleaved pairs (two at parity, three ahead). |
