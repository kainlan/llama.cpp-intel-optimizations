...
| `GGML_SYCL_ONEDNN_PP=0` | ON | Disable oneDNN for prompt processing. Read through `ggml_sycl_onednn_pp_enabled()` (`ggml-sycl.cpp`) by two questions. The per-op one is `ggml_sycl_onednn_pp_candidate`, which covers every arm that asks it: the unified dense dispatcher and, since llama.cpp-je3b, the MXFP4 per-expert direct dispatch's SOA/AOS oneDNN arms in `ggml_sycl_mul_mat`. It does **not** cover the MoE `MUL_MAT_ID` MXFP4 oneDNN executors, which are admitted by their own route policy: `GGML_SYCL_MOE_PP_ONEDNN_F16_BATCHED` (below) and `GGML_SYCL_MOE_PP_ONEDNN_F16`. So `=0` alone does not turn off all oneDNN PP on a MoE model. The other is `ggml_sycl_dense_woq_alternate_eligible_impl`: with `=0`, the planner and S1-PRELOAD also stop charging and staging dense oneDNN WOQ second copies. |
| `GGML_SYCL_DENSE_WOQ_ALTERNATES=0` | ON | Plan and stage no dense oneDNN WOQ **second copies** (`add_dense_woq_alternates` / S1-PRELOAD's `stage_woq_alt`; both read the one predicate `ggml_sycl_dense_woq_alternate_eligible_impl`). Those copies duplicate a resident primary, which the one-layout-per-weight ruling forbids; at default they fill the shared zone left after load-time KV and yield to a larger runtime context's KV (`[SYCL-PLAN] KV admission released ...`). With `=0` every dense weight's prompt processing takes the dequant-fp16 oneDNN path over its primary, and the startup log prints `[SYCL] GGML_SYCL_DENSE_WOQ_ALTERNATES=0: no dense oneDNN WOQ second copies are planned or staged`. Exists to measure the copies' PP value against the VRAM they cost. |
...
| `GGML_SYCL_ONEDNN_PP_MIN_BATCH=N` | Min batch (`src1->ne[1]`) for the dense dispatcher's oneDNN PP route, default 16. It does **not** apply to the MXFP4 direct route, whose floor is fixed at 2 (`onednn_pp_mxfp4_direct_min_batch` in `onednn-pp-placement.hpp`, llama.cpp-je3b): that route's fallbacks are per-row MMVQ and the unified kernel, not MMQ, and its arms have admitted every non-decode batch since they were written. Two other sites parse this variable with their own `getenv`: `mmvq.cpp` derives the adaptive MMVQ-MoE PP threshold from it (`n_experts * N / top_k`, unless `GGML_SYCL_MMVQ_MOE_PP` overrides it), and the `GGML_SYCL_PP_PIPELINE` pre-scan in `ggml-sycl.cpp` uses it to pick "oneDNN-PP-eligible" nodes. That pre-scan is a partial copy of the admission (batch, quantized, contiguous; no enabled / skip_type / placement check), tracked on llama.cpp-gkus. |
