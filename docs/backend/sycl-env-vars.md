...
| `GGML_SYCL_MOE_PP_ONEDNN_F16_BATCHED=1` | OFF | **Route policy**, not just an executor switch (llama.cpp-dboi, first-class integration 2026-08-21). One predicate — `ggml_sycl_moe_pp_onednn_batched_route_selected()` in `common.hpp`, shared by dispatch and the unified-cache planner — flips three things together: (i) the MXFP4 MoE layout chokepoint (`ggml_sycl_select_mxfp4_moe_layout`) holds gate/up/down on **SOA** instead of promoting to XMX_TILED, and the down-i8 planning upgrade is disabled, so one layout per weight satisfies the route; (ii) at PP the fused pair-GLU executor declines, the per-row MMVQ probes yield MXFP4 gate/up/down, and prompt routing skips the hybrid per-entry world so dispatch reaches the batched executor; (iii) the executor itself (`try_pp_mxfp4_soa_onednn_f16_batched`, `ggml-sycl.cpp`) runs — per-expert dequant to fp16 in the planned scratch ring, experts sorted by row count and split into grouped strided oneDNN batches (group `n` padded to 64-multiples for primitive reuse) to avoid ~5x rectangular padding on GPT-OSS's skewed routing. Decode (`ne12==1`) is untouched by dispatch, but the SOA layout means decode runs the SOA MMVQ/decode-fused routes, not tiled/i8. Scratch ring (`moe-scratch-admission.hpp` + `llama_model_sycl_populate_inventory`) is sized for all `n_expert` experts (~776 MB total for GPT-OSS 20B), admission fail-closed since `f2bdfbffe`; `reserve_pp_moe_onednn_scratch` satisfies from an existing adequate ring without reallocating. Measured clean-host 2026-08-21 (r=2): B50 pp512 **569** / tg128 25.4 vs default 122/32.2; B70 pp512 **922** / tg128 37.1 (B70 default is broken on a clean device — llama.cpp-o3h1). Default stays OFF pending the per-card pp/tg-tradeoff ruling; the per-card default seam lives in the policy helper. `_TRACE=1` and `_COMPARE=1` (+`_COMPARE_LIMIT`) harnesses built in. |
