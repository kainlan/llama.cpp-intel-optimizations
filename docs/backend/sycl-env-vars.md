...
| `GGML_SYCL_NONFA_ATTN_SCRATCH_MB=<MB>` | New in llama.cpp-oyfl. Overrides `unified_cache_nonfa_attn_scratch_demand_bytes()`'s shape-derived floor for the non-flash-attention batched mul_mat path's scratch demand: `demand = max(16 MiB, 3 x n_head x n_ubatch x n_ctx x sizeof(f16))`. This is the SCRATCH-zone sibling of `GGML_SYCL_ONEDNN_GRAPH_ZONE_MB` above, but the c=3 concurrency factor is MEASURED from the llama.cpp-oyfl repro log's own SCRATCH_ZONE occupancy at the moment of failure (460 MB of 512 MB, plus a 256 MiB request — ~2.8x, not the oneDNN sibling's c=1.5 an earlier version of this formula copied by analogy and which under-covered the repro), and tracked separately: this consumer is unconditional (not gated behind `GGML_SYCL_DNNL`), and it feeds `ggml_backend_sycl_set_runtime_context()`'s runtime refusal, not only the plan-time zone-raise the ONEDNN sibling above uses. See `docs/backend/sycl-memory-design.md`'s "A non-tensor consumer" subsection for the full derivation and why the plan-time raise this variable also feeds cannot take effect automatically for the standard model-load flow (`llama-model.cpp` hardcodes the pre-load envelope's `n_ctx` to 0; piping the real context size back that early is llama.cpp-fkpg's scope) — setting this variable manually, before the model loads, is the one way to raise the zone today and let a shape that would otherwise be refused actually run; the refusal message itself names the exact value that would work. An explicit non-negative value always wins over the formula. Setting this too low can make a context that would otherwise be refused (with the size arithmetic, `-c N` remediation) instead abort mid-prefill in `ggml_sycl_mul_mat_batched_sycl()`'s fallback ladder — this override is for tuning a workload the formula mis-sizes (or supplying it in advance), not for silencing the refusal. |
