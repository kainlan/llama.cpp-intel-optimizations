...
**Update (2026-09-02, `llama.cpp-la7d`):** the repo-root orphan `tests/test-dispatch-tuning.cpp` is deleted; the registered twin `ggml/src/ggml-sycl/tests/test-dispatch-tuning.cpp` gained the two case classes (`onednn_`/`mmq_` sub-branch mapping) the orphan covered and the registered twin didn't, so no coverage was lost. **Fix round (same day):** the orphan's third, non-mapping property -- one loaded cache whose three distinct `(type,m,n,k)` keys each resolve to their own kernel, versus every registered-twin case building a fresh single-entry cache -- was still missing after the port; added as `test_multi_entry_cache_discrimination()`. The shared single-entry helper was renamed `mmvq_winner_maps_to()` → `winner_maps_to()` since it now backs `onednn_`/`mmq_` cases too, not just `mmvq_`. See the updated mutation-oracle row below (was left stale at the prior case count).
...
| `test-sycl-dispatch-tuning` | `ggml/src/ggml-sycl/dispatch-tuning.cpp:242` `return ggml_sycl_mul_mat_kernel::MMVQ_COALESCED;` → `return ggml_sycl_mul_mat_kernel::MMVQ_SOA;` | CPU-ONLY | COUNT `[TEST SUMMARY] %d/%d tests passed` | `[TEST] mmvq_coalesced winner maps to MMVQ_COALESCED ... FAILED: winner did not map to MMVQ_COALESCED` then `[TEST] multi-entry cache discriminates by (type,m,n,k) key ... FAILED: mmvq key resolved to the wrong kernel from a shared multi-entry cache` then `[TEST SUMMARY] 4/6 tests passed`, rc 1 | ⚠️ **The oracle in this row before 2026-08-09 (`FAILED: mmvq kernel mismatch`, "1 of 3 assertions") belonged to a DIFFERENT, unregistered file** — see the basename-collision note in B2 chunk 2. The registered test is `ggml/src/ggml-sycl/tests/test-dispatch-tuning.cpp`, extended under `llama.cpp-bqtu` from 1 case to 3 cases, then under `llama.cpp-la7d` (2026-09-02) to **6 cases / 17 `TEST_ASSERT` call sites** (the `TEST_ASSERT` macro definition at `:44` does not itself count) (276 lines): `unified_matmul`, `mmvq_coalesced`, `mmvq_soa`, `onednn_woq_gemm`, `mmq_soa`, and a multi-entry case that loads one 3-entry JSON into a single cache and asserts each `(type,m,n,k)` key resolves to its own kernel (plus that an absent key resolves to nothing). **2-of-6 specificity, not 1-of-6**: the mutation is inside the `winner.find("coalesced")` sub-branch, and the multi-entry case's second entry is *also* `"mmvq_coalesced"` (chosen to mirror `test_mmvq_coalesced_mapping`), so this single mutation reddens both the standalone case and the multi-entry case's `key_mmvq` assertion — `mmvq_soa`/`onednn_woq_gemm`/`mmq_soa`/`unified_matmul` all stay green. Inside the multi-entry case itself, `key_onednn`'s lookup and both its `TEST_ASSERT`s (`:233-237`) run and pass — they precede `key_mmvq` (`:239-243`) in source order; `key_mmvq`'s kernel-equality `TEST_ASSERT` (`:242`) is the one that fails under the mutation, and its `TEST_FAIL`-triggered `return false` is what skips `key_mmq` (`:245-249`) and `key_absent` (`:253-255`) — but the case as a whole still only counts once toward the fail side of the summary. Each case writes its own `mkstemp` file under `/tmp` (tmpfs) and removes it before asserting. Target compiles `dispatch-tuning.cpp` directly and links `ggml`; no `libggml-sycl` relink. **Mutation reproduced by reasoning over the test's own control flow, not by executing the mutant** (`llama.cpp-la7d` fix round): `TEST_ASSERT`'s `TEST_FAIL` macro `return`s out of the enclosing case function before `TEST_PASS()` runs, so a failed case increments `g_tests_run` (via `TEST_BEGIN`) but not `g_tests_passed`; with 2 of 6 cases failing that way, `main()`'s trailing `fprintf(stderr, "\n[TEST SUMMARY] %d/%d tests passed\n", g_tests_passed, g_tests_run)` prints `4/6`. |
...
| `test-sycl-moe-fused-down-sum-policy` | `ggml/src/ggml-sycl/mmvq.cpp:250-251`, in `ggml_sycl_moe_down_q8_dpas_tile_from_env`: `if (std::strcmp(env, "tile4") == 0) { return 4; }` → `return 2;` | CPU-ONLY | PASSFAIL, `PASS: MoE XMX down-sum direct-final policy` | grep the message tail **`tile4 down Q8 DPAS tile env must parse`** (unique — 1 occurrence in the file), rc 1 | ⚠️ **The previous warning on this row — "THE MUTATION ABOVE IS TEST-LOCAL" — was itself wrong** (re-adjudicated 2026-08-09, `llama.cpp-u2mz`). The bare `:250-251` named no file and reads like the test's own line numbers; it is **`mmvq.cpp`**, and it is production. Decisive check: the compound `strcmp(env, "tile4")` occurs **exactly once in the tree**, at `mmvq.cpp:250`, and **zero times in the test file** — positive control, the same grep hits `mmvq.cpp`. ⚠️ Search for that compound, not for `tile4` alone: the bare string appears on **6 lines** of the test file (`:335-336`, `:346-347`, `:353-354`) as arguments passed *into* the production functions and inside the `"down-q8-dpas-tile4"` label comparison, so a substring search manufactures a contradiction with the sentence above. **Reachability trace:** `mmvq.cpp:250` ← `mxfp4_moe_down_q8_dpas_tile_rows()` (`:299-305`, memoized `static`) ← `mxfp4_moe_down_q8_dpas_tile_active()` (`:307-312`) ← dispatch site `:20020`, which selects `mxfp4_down_q8_dpas_tile_sycl<2>` (`:20022`) or `<4>` (`:20029`). So it satisfies the KEEP adjudication as written; **no re-point needed.** ⚠️ **Do not grep the `FAIL:` line prefix.** The full line is `FAIL: <__FILE__>:<line>: tile4 …`, and both parts are unstable: `__FILE__` is the **absolute** `/Apps/llama.cpp/tests/test-sycl-moe-fused-down-sum-policy.cpp` the build passes (the old row's `tests/…` prefix would never have matched), and the reported line of a multi-line `CHECK(...)` is compiler-dependent — the invocation is at `:335`, its closing paren at `:336`. **1 of 16 CHECKs** in the 13th of 17 subtests (`test_down_q8_dpas_tile_variant_parser_labels_and_scope`, `:320`); the sibling `tile2` CHECK at `:333` stays green, which distinguishes this from a whole-parser break. ⚠️ 3 subtests (`:211`, `:261`, `:358`) source-grep `mmvq.cpp` and `std::exit(1)` — **not** `CHECK` — if the file is not found, a confusing hard exit from an unexpected CWD; and editing env-name *strings* would trip those greps as collateral. This mutation edits a return value, not a string, so it touches none of them, and `tests/test-sycl-kernel-profiler-source-mmvq.py:153` (which greps the *signature* line) also stays green. **Maximal specificity: this is the only ctest name that reddens** — `ggml_sycl_moe_down_q8_dpas_tile_from_env` has exactly one test caller. ⚠️ **Cost: unlike the Batch B rows, this one DOES relink `libggml-sycl`** — the target compiles only the test `.cpp` and links `ggml-base ggml ggml-sycl` (`ggml/src/ggml-sycl/CMakeLists.txt:2307-2343`). ctest name is 1:1: `test-sycl-moe-fused-down-sum-policy`. |
...
- **`test-sycl-moe-fused-down-sum-policy` — KEEP, with its mutation pinned.**
  ...
  **Verified 2026-08-09, and the Batch E row already complied** — the follow-up
  believed it did not, because the row wrote its mutation as a bare `:250-251`
  with no filename and that reads as the test's own lines. It is `mmvq.cpp`.
  See the Batch E row for the reachability trace and the decisive one-occurrence
  check. Two refinements to the "5 of 17" figure, which is right only if you
  count source-text assertions as reaching production:

  | how a subtest reaches `mmvq.cpp` | subtests | count |
  |---|---|---|
  | calls production symbols (linked, via `mmvq.hpp`) | `:222` (17 calls), `:261` (23), `:320` (16) | **3** |
  | asserts on `mmvq.cpp` **source text** only | `:211`, `:358` | 2 |
  | both of the above | `:261` | — |
  | pure mock — `test_moe_down_sum_direct_final_policy` in `ggml-sycl.cpp` | the other 12 | 12 |

  Distinct production-touching subtests = **5** (`:211`, `:222`, `:261`, `:320`,
  `:358`), which is where the figure comes from; but only **3** reach production
  through linkage, and a mutation-proof needs one of those three. `:320` is the
  Batch E target.
