...
These six §6 rows have a missing-key or unowned-state Risk finding but no
filed bead, because each is either a different-shaped fix than "add a
scoping key", or is not yet confirmed to have a live reader that would
observe the gap. Per ruling 3 this is a stated deferral, not a silent gap:
