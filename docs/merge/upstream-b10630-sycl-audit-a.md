...
**Class:** port-candidate
**Why:** Fixes an off-by-one (`row > nrows` → `row >= nrows`) across several base (non-reorder) K-quant DMMV kernels, and a real correctness bug in the non-reorder Q5_K kernel that only computed one of the two `im ∈ {0,1}` halves. The fork's `dequantize_mul_mat_vec_q5_k` (`dmmv.cpp:2348-2464`) is confirmed to carry the identical bug: `const int row = item_ct1.get_group(2);` with no `nrows` parameter or bounds check, and the `#if QK_K==256` branch computes only a single `im` derived from `tid`, never looping over both halves. Since the fork's `ggml_sycl_supports_reorder_dmmv()` only reorders Q4_0, this base Q5_K kernel is the one actually exercised for Q5_K DMMV today.
**Landing zone:** `ggml/src/ggml-sycl/dmmv.cpp`, function `dequantize_mul_mat_vec_q5_k` (add the `nrows` parameter + bounds check, and loop both `im` halves as upstream now does).
**Ownership surface:** none — in-kernel bounds/loop correctness fix over existing device buffers; no allocation.
...
### 13. `1553725965470` — sycl: fuse RMS_NORM + MUL (#26015)
...
**Class:** superseded
**Why:** A large (690-line), independent third flash-attention path using oneMKL GEMM to XMX-accelerate long-context prefill, with its own env vars, gating envelope (gqa≥2, head_dim 64-512 multiple of 64, no sinks/alibi/softcap) and multi-turn correctness fixes. `fattn-mkl.cpp` does not exist in the fork and is not included from `backend.hpp`. The fork already has two integrated prefill-acceleration paths serving the identical goal — XMX-accelerated prompt processing — via `fattn-xmx-f16-v2.hpp` and the oneDNN SDPA path (`fattn-onednn.cpp`, entry 4/entry 18's lineage), the latter already measured at ~32% PP uplift with FA on. Importing a third, large, independently-gated GEMM-FA implementation duplicates functionality the fork already ships and integrates with its own cache/materializer machinery; if the MKL path is ever wanted it needs a fresh perf comparison against the fork's existing oneDNN path, not a mechanical port.

### 18. `272700b360944` — sycl: fix classification of iGPUs (#26105)
**Files:** ggml/src/ggml-sycl/common.hpp, ggml/src/ggml-sycl/ggml-sycl.cpp
**Class:** port-candidate
...
⚠️ **Ownership screen:** upstream's own diff stages the converted K/V into `ggml_sycl_pool_alloc<sycl::half>` scratch (`ctx.pool()`), the same allocator the fork's own pre-existing `Qf`/`Kf`/`Vf` materialization buffers in this file already use. `ggml_sycl_pool` is exactly the legacy direct-allocation interface the fork instruments as bypassing the unified cache (`common.hpp`: `ggml_sycl_note_direct_allocation()`, `GGML_SYCL_DIRECT_ALLOC_GUARD`) — it is not a confirmed unified-cache-routed path. **Verbatim port of the pool_alloc scratch is forbidden by the canonical contract; the port must re-express the dequant-to-F16 scratch through `unified_allocate`/`unified_allocate_owner()` → `mem_handle` (or an `alloc_owner` wrapped via `mem_handle::from_owned_alloc()`)**, not through `ctx.pool()`, even though the surrounding pre-existing GQA-materialization code in this same file has not yet been migrated either — that is a pre-existing gap this port must not extend, not a precedent to copy.
...
**Class:** n-a
**Why:** Implements SYCL kernels for four new DeepSeek-v4-specific ggml ops. Verified via `search_text` scoped to `ggml/include`: neither `LIGHTNING_INDEXER` nor `DSV4_HC` appears anywhere in the fork's `ggml/include/ggml.h` — the `GGML_OP_*` enum values these kernels dispatch on do not exist in this codebase's ggml core at all (unlike entry 6's `GGML_UNARY_OP_XIELU`, which already exists upstream-side in the fork). This SYCL-only slice cannot compile or be reached without a separate, out-of-scope core-ggml commit (op enum + CPU reference + graph-building support), which is not part of the `ggml/src/ggml-sycl` commit set this task audits. Not "N/A because we don't have the feature" (the Task 7 gotcha) — N/A because a hard compile-time prerequisite this task cannot supply is missing. Revisit if/when a core-ggml DSv4 op commit is separately identified (Task 10's core-ggml brief) and merged.
...
**Class:** port-candidate
**Why:** Fixes NVFP4's unsigned UE4M3 scale-byte decode, which upstream's *old* code mishandled by reinterpreting it as signed E4M3. The fork's `ggml_sycl_ue4m3_to_fp32()` (`common.hpp:6797`) already uses the same corrected unsigned-decode formula (`exp = (x>>3)&0xf`, `man = x&0x7`, `ldexp`-based reconstruction, `raw*0.5f`) as this commit's fix — but the fork's version retains an extra `x == 0xff → 0.0f` special case that upstream's fixed formula explicitly removed (its comment: "exp == 0xF is a valid exponent (256-448 range), not NaN"). The fork independently converged on the same fix but kept one residual bug from the pre-fix code (`x * (x != 0x7F && x != 0xFF)`): byte `0xFF` should decode to a valid non-zero value (≈240 after the `*0.5f` scale) and currently silently zeros instead. Narrow, low-risk reconciliation.
