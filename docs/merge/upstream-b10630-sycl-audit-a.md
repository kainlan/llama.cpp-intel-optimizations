...
**Class:** port-candidate
**Why:** Fixes an off-by-one (`row > nrows` → `row >= nrows`) across the base (non-reorder) Q2_K/Q3_K/Q4_K/Q6_K DMMV kernels, plus a second-half fix that only applies to the *reorder* Q5_K kernel. Verified against the fork's current source: `dmmv.cpp:1981` (q2_k), `:2089` (q3_k), and `:2203` (q4_k) all still read `if (row > nrows)`, while q6_k (`:2475`) is already `>=` — three of the four sites genuinely need the same one-character fix. The non-reorder Q5_K kernel is NOT affected: its `im` split (`:2366-2374`) distributes both halves across different threads in the same work-group (not a per-thread loop), and its own launcher (`:2941-2952`) grids exactly `nrows` groups with `local_range(1)==1`, so `row = get_group(2)` can never exceed `nrows-1` — an `nrows` guard there would be structurally inert. Upstream's im-loop restructure lands in the *reorder* Q5_K kernel, which is unreachable in this fork (`ggml_sycl_supports_reorder_dmmv()` returns true only for `GGML_TYPE_Q4_0`).
