...
**Validation summary (2026-04-24)**: see [`docs/plans/data/planner-canaries/validation-summary.md`](data/planner-canaries/validation-summary.md) for the consolidated 8-item before/after table mapping each uncertainty from the audit to its current `validated` / `partially validated` / `still blocked` status. **State after patched compute-runtime install (2026-04-24)**: five items fully validated (op-id stability, single-shape sizing within tested architectures, layer-streaming policy consistency, m09zb resolved upstream, D0.4 PASS), one partially validated (Task 2 architecture coverage caveat for SWA + state-space), two still in progress (Task 5 mini-context prototype not yet written; Task 7 weight priority benchmark gated on Task 14 KV-alloc ceiling).
...
findings. Post-patched-runtime install (2026-04-24): A3a sizing
direction (D0.2) validated, A7's direct-load path (D0.4) validated,
C2's op-id keying (D0.3b) validated. C2's multi-device CPY-name scope
(D0.3a) remains INCONCLUSIVE on host-policy grounds (B50 disabled).
Task 5 (mini-context prototype) and Task 7 (weight priority benchmark)
remain to be implemented; Task 7's canonical 4096-context shape is
gated on Task 14 (KV alloc ceiling under patched libze). See the
summary file for per-bead unlock decisions.
