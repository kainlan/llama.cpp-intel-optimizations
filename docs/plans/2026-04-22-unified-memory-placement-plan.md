...
| D0.3 — post-split CPY visibility | PASS (commit `607fca77` for op-id stability variant) | Original multi-device CPY-name scope remains INCONCLUSIVE (host has 1 visible SYCL device, B50 disabled per `feedback_disable_b50.md`); rerun requires ≥2 safely-usable devices AND E1. Single-device op-id stability check (the load-bearing claim for C2) **PASSed** 2026-04-24: 5 repeated `graph_reserve` calls on Mistral 7B produced byte-identical (op_id, op_type, name) sequences — C2 can safely key on op_id. |
...
|---|---|---|
| E1 | Address the L0 DirectSubmission non-flush that wedges any post-init `event.wait()`. **Trigger isolated 2026-04-24** (Task 10, `33e05fa36`) to the FIRST H2D copy on a freshly-initialized backend stream — `(*stream).memcpy(...).wait()` at `ggml-sycl.cpp:~13076` inside `ggml_backend_sycl_buffer_set_tensor`. Original direction (restructure `staging_buffer_pool` to delegate back-pressure to the SYCL in-order queue) was applied as Task 9 (`43cd00782`) and is keep-but-not-fixing — that path is structurally upstream of any slot reuse. Actual fix must target one of: (a) the first-H2D path in `ggml_backend_sycl_buffer_set_tensor` (e.g., bypass `(*stream).memcpy(...).wait()` for the first call after init via a CPU fast-path or fresh `sycl::queue`); (b) the init-time `ggml_backend_probe_max_alloc_size` probe state (gate behind env or shrink upper bound); or (c) the DPCT helper's queue construction (replace with plain `sycl::queue`). Bisection probes (Task 11) underway to confirm root cause. Bead: `llama.cpp-m09zb`. | No mutex held across any `event.wait()`; **bare-SYCL minimal-repro + broader-envelope-repro** (Task 4 + Task 10 artifacts) both PASS, the latter at `STAGE=init-set` within 30 s; **D0.1 canary** loads Mistral 7B cleanly in under 60 s; **D0.4 canary** (direct mmap → device `ggml_backend_tensor_set`) completes within 60 s with byte-identical readback; **first `ggml_backend_tensor_set` H2D after backend init waits < 1 s** (measured trace); `ninja -C build` + full ctest green; no new env var; zero perf regression on `llama-bench Mistral7B-Q4_0 PP512/TG128` baseline (±3%). The empty-waitlist `ext_oneapi_submit_barrier` GT-reset bug discovered in Task 4 is **out of scope** for E1 (filed as a separate Intel bug, see `docs/plans/data/e1-rca/findings.md` §"Companion Bug Report"). |
...
| D0.2 PP + TG union | PASS | A3a can size zones from a single shape; Mistral 7B (13 ops) and GPT-OSS 20B (17 ops) both produce PP == TG. Generalization (2026-04-24): 4 additional Mistral 7B quantization variants (Q2_K, Q3_K_M, Q4_K_M, Q8_0) all PASS with identical 13-op set — quantization format does not perturb op enumeration. Architecture-coverage caveat: SWA / state-space models not locally available. |
| D0.3 post-split CPY visibility / op-id stability | PASS (single-device scope, 2026-04-24) | Original multi-device CPY-name scope: INCONCLUSIVE (single device visible, B50 disabled). Single-device op-id stability check: 5 repeated `graph_reserve` calls produce byte-identical (op_id, op_type, name) sequences — C2 can safely key on op_id. Multi-device CPY naming remains unverified. |
| D0.4 direct-weight-load mechanics | INCONCLUSIVE | blocked on E1; hang at `ggml-sycl.cpp:12685` is the SAME wedge isolated by Task 10's broader-envelope bisection (`33e05fa36`) — the FIRST `(*stream).memcpy(...).wait()` after `ggml_backend_sycl_init`. m09zb is an L0 DirectSubmission non-flush in the first-H2D-after-init path, not a staging-pool-specific bug. E1 acceptance refined accordingly. |

**Validation summary (2026-04-24)**: see [`docs/plans/data/planner-canaries/validation-summary.md`](data/planner-canaries/validation-summary.md) for the consolidated 8-item before/after table mapping each uncertainty from the audit to its current `validated` / `partially validated` / `still blocked` status. Three items fully validated (op-id stability, layer-streaming policy consistency, plan-doc internal consistency); two partially validated (single-shape sizing — generalizes within tested architectures; E1 root cause — trigger isolated, hypothesis bisection in progress); three still blocked on E1 (mini-context prototype, D0.4 re-run, weight priority benchmark).
