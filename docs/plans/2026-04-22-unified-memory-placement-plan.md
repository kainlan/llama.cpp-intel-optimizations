...
|---|---|---|
| E1 | **Goal**: Address the L0 DirectSubmission non-flush that wedges any post-init `event.wait()`. Bead: `llama.cpp-m09zb`.<br><br>**Trigger location** (isolated 2026-04-24 by Task 10, `33e05fa36`): the FIRST H2D copy on a freshly-initialized backend stream — `(*stream).memcpy(...).wait()` at `ggml-sycl.cpp:~13076` inside `ggml_backend_sycl_buffer_set_tensor`.<br><br>**Status of prior attempt**: Task 9 (`43cd00782`) applied the original direction (restructure `staging_buffer_pool` to delegate back-pressure to the SYCL in-order queue) as a 1-line `event.wait_and_throw()` change. That path is structurally upstream of any slot reuse, so the fix is keep-but-not-fixing-m09zb.<br><br>**Candidate fix targets** (one of):<br>• (a) First-H2D path in `ggml_backend_sycl_buffer_set_tensor` — bypass `(*stream).memcpy(...).wait()` for the first call after init via a CPU fast-path or fresh `sycl::queue`;<br>• (b) Init-time `ggml_backend_probe_max_alloc_size` probe state — gate behind env or shrink upper bound;<br>• (c) DPCT helper's queue construction — replace with plain `sycl::queue`.<br><br>Task 11 bisection probes underway to confirm root cause. | • No mutex held across any `event.wait()`;<br>• Bare-SYCL `minimal-repro` + `broader-envelope-repro` (Task 4 + Task 10 artifacts) both PASS, the latter at `STAGE=init-set` within 30 s;<br>• `D0.1 canary` loads Mistral 7B cleanly in under 60 s;<br>• `D0.4 canary` (direct mmap → device `ggml_backend_tensor_set`) completes within 60 s with byte-identical readback;<br>• First `ggml_backend_tensor_set` H2D after backend init waits < 1 s (measured trace);<br>• `ninja -C build` + full ctest green;<br>• No new env var;<br>• Zero perf regression on `llama-bench Mistral7B-Q4_0 PP512/TG128` baseline (±3%).<br><br>**Out of E1 scope**: the empty-waitlist `ext_oneapi_submit_barrier` GT-reset bug discovered in Task 4 (filed as a separate Intel bug — see `docs/plans/data/e1-rca/findings.md` §"Companion Bug Report"). |
...
| D0.4 direct-weight-load mechanics | INCONCLUSIVE | blocked on E1; the wedge originally observed at `ggml-sycl.cpp:12685` was narrowed by Task 10's broader-envelope bisection (`33e05fa36`) to `ggml-sycl.cpp:~13076` — same wedge, two vantage points (L12685 is the outer `ggml_backend_sycl_buffer_set_tensor` entry, L~13076 is the inner `(*stream).memcpy(...).wait()` after `ggml_backend_sycl_init`). m09zb is an L0 DirectSubmission non-flush in the first-H2D-after-init path, not a staging-pool-specific bug. E1 acceptance refined accordingly. |
