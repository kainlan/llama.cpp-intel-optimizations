...
**Amendment (owner ruling, 2026-08-07): no quiet-host precondition.** The host's ambient load (~60: Emby, the ws2022ci qemu VM, Frigate ffmpeg) is its permanent operating state — run this matrix under it and never defer it waiting for quiet. The floors and bands above stand unchanged; the 2026-07-25 baseline re-measure that validated them was itself taken under load. If an arm misses its floor or band, do not wait for quiet and do not lower the guardrail: discriminate load from regression with an interleaved paired A/B on the same host state — alternate runs of branch HEAD against a known-good comparator build (the commit immediately before the suspect change, built in the persistent history worktree) on the same card and model. Paired ratio within noise → load-depressed, file the owner-reviewed baseline task per the rule above with the pairing attached as evidence; branch consistently slower within pairs → real regression, block merge. Absolute numbers taken under load still must not become new baselines.
