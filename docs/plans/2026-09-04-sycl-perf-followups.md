...
**Deferred items executed (2026-09-07 17:54-21:00, lead session, llama.cpp-2egq):** the L3
six-pair table and the L4 long-prompt baselines, on master `0d8b160c5` (binaries built at
`6e54ba2eb`, SYCL backend unchanged since), models from the byte-identical NFS copies under
`/Storage/GenAI/models` (`/models` was being migrated to bcachefs and was unmounted mid-run).
- L3 (`scripts/sycl-prefill-scaling.sh --models-dir /Storage/GenAI/models`, all six pairs
  guard-VALID): ratio1024 Mistral B70 0.903 / B50 0.952, GPT-OSS B70 0.942 / B50 0.986, gemma4
  B70 0.882 / B50 0.938; five of six at or above the 0.9 floor, gemma4 B70 reproducibly
  0.875-0.896 over three runs; per-decode intercept 73-87 ms B70 dense, ~190 ms B70 MoE,
  150-380 ms B50; recorded on llama.cpp-dfo0 (stays open on the residual, as the L3
  acceptance says) and in `docs/backend/sycl-perf-baselines.md`.
- L4 (5 guarded processes per cell, 12 cells, `artifacts/perf-6ae16115c-longprompt/`): 56 VALID
  logs; 11 full cells plus B50 Mistral pp8192 partial (n=1) because that cell faults on this
  boot with a GPU `Engine memory CAT error` (llama.cpp-0oxf, P1 -- fixed and merged as
  `ee3c0c9e3`; this cell must still be re-measured on a post-fix build before it stops being
  partial; both faulted logs kept as `.FAULT`).
  Table generated by `parse-sycl-bench-matrix.py --matrix long-prompt --table --partial-arm`
  and pasted into the baselines doc's new "Long-Prompt Baselines" section; the 2026-09-04
  snapshot is demoted to history there. `--self-test` 31/31; `--dir` run exit 0 under `--partial-arm` (exit 2 without it, by design).
- Blockers found and fixed on the way: `scripts/bench-guard.sh` hardcoded PCI 03:00.0/07:00.0
