...
- L4 (5 guarded processes per cell, 12 cells, `artifacts/perf-6ae16115c-longprompt/`): 56 VALID
  ...
  boot with a GPU `Engine memory CAT error` (llama.cpp-0oxf, P1, faulted log kept as `.FAULT`).
  ...
  snapshot is demoted to history there. `--self-test` 24/24; `--dir` run exit 0.
