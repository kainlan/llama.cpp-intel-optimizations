...
template <int mmq_x, int mmq_y, int nwarps, bool check>
class mmq_q4_0_soa_persistent_kernel;                                               // Persistent work-stealing variant
template <int mmq_x, int mmq_y, int nwarps, bool check> class mmq_q4_0_coalesced_kernel;  // Coalesced layout variant
template <int mmq_x, int mmq_y, int nwarps, bool check>
class mmq_q4_0_coalesced_persistent_kernel;  // Persistent work-stealing variant
...
static int get_persistent_groups(int device_id) {
    ...
    if (!logged[device_id]) {
        fprintf(stderr, "[MMQ] Device %d: %d EUs / %d = %d XeCores × %d WGs = %d persistent groups\n", device_id,
                eu_count, EUS_PER_XECORE, xecore_count, wgs_per_xe, groups);
...
template <typename KernelFunc, typename PropertiesT, int Dims> struct ggml_sycl_kernel_with_properties {
    ...
    void operator()(sycl::nd_item<Dims> item) const { func(item); }

    auto get(sycl::ext::oneapi::experimental::properties_tag) const { return props; }
};

template <typename KernelName, int Dims, typename KernelFunc>
static void ggml_sycl_parallel_for_grf(sycl::handler & cgh, const sycl::nd_range<Dims> & range, KernelFunc && func) {
    ...
    if (grf_size == ggml_sycl_grf_size::grf_128) {
        auto props     = sycl::ext::oneapi::experimental::properties{ sycl::ext::intel::experimental::grf_size<128> };
        using kernel_t = ggml_sycl_kernel_with_properties<std::decay_t<KernelFunc>, decltype(props), Dims>;
        cgh.parallel_for<ggml_sycl_grf128_kernel<KernelName>>(range, kernel_t{ std::forward<KernelFunc>(func), props });
        return;
    }

    auto props     = sycl::ext::oneapi::experimental::properties{ sycl::ext::intel::experimental::grf_size<256> };
    using kernel_t = ggml_sycl_kernel_with_properties<std::decay_t<KernelFunc>, decltype(props), Dims>;
    cgh.parallel_for<ggml_sycl_grf256_kernel<KernelName>>(range, kernel_t{ std::forward<KernelFunc>(func), props });
}
...
template <typename T> struct mmq_use_qr_stride : std::false_type {};

template <> struct mmq_use_qr_stride<block_q4_K> : std::true_type {};

template <> struct mmq_use_qr_stride<block_q5_K> : std::true_type {};
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q4_0_soa(const uint8_t * __restrict__ qs_base,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q4_0_coalesced(const uint8_t * __restrict__ qs_base,
                                                      const size_t d_offset,
                                                      int * __restrict__ x_ql,
                                                      sycl::half2 * __restrict__ x_dm,
                                                      int * __restrict__ x_qh,
...
static __dpct_inline__ float vec_dot_q4_0_q8_1_mul_mat_soa(const int * __restrict__ x_ql,
                                                           const sycl::half2 * __restrict__ x_dm,
                                                           const int * __restrict__ x_qh,
                                                           const int * __restrict__ x_sc,
                                                           const int * __restrict__ y_qs,
...
static __dpct_inline__ float vec_dot_q8_0_q8_1_mul_mat(const int * __restrict__ x_ql,
    ...
    // Use MMQ_TILE_NE_K (32) for tile indexing to match CUDA tile sizing
    return vec_dot_q8_0_q8_1_impl<VDR_Q8_0_Q8_1_MMQ>(&x_ql[i * (MMQ_TILE_NE_K + 1) + k], &y_qs[j * MMQ_TILE_NE_K + k],
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q8_0_soa(const int8_t * __restrict__ qs_base,
                                                const size_t d_offset,
                                                int * __restrict__ x_ql,
                                                float * __restrict__ x_df,
                                                const int &          i_offset,
...
// Coalesced layout: word-major within tiles, scales remain block-sequential
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q8_0_coalesced(const int8_t * __restrict__ qs_base,
                                                      const size_t d_offset,
                                                      int * __restrict__ x_ql,
                                                      float * __restrict__ x_df,
                                                      const int &          i_offset,
...
#pragma unroll 1
    for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 8) {
        ...
        x_sc[i * (MMQ_TILE_NE_K / 8) + i / 8 + k % (MMQ_TILE_NE_K / 8)] =
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q6_K_soa(const uint8_t * __restrict__ qs_base,
...
template <int mmq_y, int nwarps, bool need_check>
static __dpct_inline__ void load_tiles_q6_K_coalesced(const uint8_t * __restrict__ qs_base,
                                                      const size_t /*qh_offset*/,
                                                      const size_t /*scales_offset*/,
                                                      const size_t d_offset,
                                                      int * __restrict__ x_ql,
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check, bool need_sum = false>
static void mul_mat_q6_K_soa(const uint8_t * __restrict__ qs_base,
                             const size_t qh_offset,
                             const size_t scales_offset,
                             const size_t d_offset,
                             ...
                             const int                ncols_y,
                             const int                nrows_y,
                             const int                nrows_y_unpadded,
                             const int                nrows_dst,
                             const int                row_low,
                             const sycl::nd_item<3> & item_ct1,
                             int *                    tile_x_qs_q6_K,
                             sycl::half2 *            tile_x_dm_q6_K,
                             int *                    tile_x_sc_q6_K,
                             int *                    tile_y_qs,
    ...
    allocate_tiles_q6_K<mmq_y>(&tile_x_ql, &tile_x_dm, &tile_x_qh, &tile_x_sc, tile_x_qs_q6_K, tile_x_dm_q6_K,
                               tile_x_sc_q6_K);
    ...
    if (item_ct1.get_group(0) == 0 && item_ct1.get_group(1) == 0 && item_ct1.get_group(2) == 0 &&
        ...
        sycl::ext::oneapi::experimental::printf(
            "[MMQ_Q6K_SOA] nrows_x=%d ncols_x=%d ncols_y=%d nrows_dst=%d row_low=%d\n", nrows_x, ncols_x, ncols_y,
            nrows_dst, row_low);
    ...
    for (int ib0 = 0; ib0 < blocks_per_row_x; ib0 += blocks_per_iter) {
        ...
        // row_low converts local row indices to absolute indices for SoA addressing
        load_tiles_q6_K_soa<mmq_y, nwarps, need_check>(qs_base, qh_offset, scales_offset, d_offset, tile_x_ql,
                                                       tile_x_dm, tile_x_qh, tile_x_sc, item_ct1.get_local_id(1),
        ...
        if (ib0 == 0 && item_ct1.get_group(0) == 0 && item_ct1.get_group(1) == 0 && item_ct1.get_group(2) == 0 &&
            item_ct1.get_local_id(0) == 0 && item_ct1.get_local_id(1) == 0 && item_ct1.get_local_id(2) == 0) {
            sycl::ext::oneapi::experimental::printf(
                "[Y_SRC_SOA] blocks_per_col_y=%d col_y_0=%d ncols_y=%d nrows_y=%d\n", blocks_per_col_y, col_y_0,
                ncols_y, nrows_y);
            ...
            for (int col = 0; col < sycl::min(2, ncols_y); col++) {
                ...
                const sycl::half2 * y_col_ds =
                    (const sycl::half2 *) ((const char *) vy + col * y_col_stride + nrows_y_unpadded);
                sycl::ext::oneapi::experimental::printf(
                    "[Y_SRC_SOA] col=%d qs[0..3]=%d,%d,%d,%d ds[0]=[%.6f,%.6f] ds[1]=[%.6f,%.6f]\n", col, y_col_qs[0],
                    y_col_qs[1], y_col_qs[2], y_col_qs[3], (float) y_col_ds[0][0], (float) y_col_ds[0][1],
...
#pragma unroll 1
            for (int i = 0; i < mmq_x; i += nwarps) {
                const int col_y_eff = dpct::min((unsigned int) (col_y_0 + item_ct1.get_local_id(1) + i), ncols_y - 1);
                ...
                tile_y_qs[index_y] = get_int_from_int8_aligned(y_block_qs, item_ct1.get_local_id(2) % QI8_1);
            }
...
#pragma unroll 1
            for (int ids0 = 0; ids0 < mmq_x; ids0 += nwarps * QI8_1) {
                ...
                if (ib0 == 0 && ir == 0 && ids0 == 0 && item_ct1.get_group(0) == 0 && item_ct1.get_group(1) == 0 &&
                    item_ct1.get_group(2) == 0 && item_ct1.get_local_id(0) == 0 && item_ct1.get_local_id(1) < 2 &&
                    item_ct1.get_local_id(2) < 4) {
                    const int tile_idx = ids * (WARP_SIZE / QI8_1) + kby;
                    sycl::ext::oneapi::experimental::printf(
                        "[DS_LOAD_SOA] warp=%d lane=%d ids=%d kby=%d col_y_eff=%d block_idx=%d tile_idx=%d "
                        "src_ds=[%.6f,%.6f]\n",
                        (int) item_ct1.get_local_id(1), (int) item_ct1.get_local_id(2), ids, kby, col_y_eff, block_idx,
                        tile_idx, (float) dsi_val[0], (float) dsi_val[1]);
                }
#endif
...
#    if GGML_SYCL_MMQ_Q6K_DEBUG
            if (ib0 == 0 && ir == 0 && item_ct1.get_group(0) == 0 && item_ct1.get_group(1) == 0 &&
                item_ct1.get_group(2) == 0 && item_ct1.get_local_id(0) == 0 && item_ct1.get_local_id(1) == 0 &&
...
#    if GGML_SYCL_MMQ_Q6K_DEBUG
            ...
            if (ib0 == 0 && ir == 0 && item_ct1.get_group(0) == 0 && item_ct1.get_group(1) == 0 &&
                ...
                sycl::ext::oneapi::experimental::printf("[TILE_X] x_dm[0]=%.6f (isnan=%d isinf=%d)\n", d_val,
                                                        sycl::isnan(d_val) ? 1 : 0, sycl::isinf(d_val) ? 1 : 0);
                ...
                sycl::ext::oneapi::experimental::printf("[TILE_Y] y_ds[0]=%.6f (isnan=%d isinf=%d)\n", y_d,
                                                        sycl::isnan(y_d) ? 1 : 0, sycl::isinf(y_d) ? 1 : 0);
            }
#    endif
...
#pragma unroll 1
                    for (int ix = 0; ix < mmq_x / nwarps; ++ix) {
                        const float dot_result = vec_dot_q6_K_q8_1_mul_mat(
                            ...
                            item_ct1.get_local_id(2) + iy * WARP_SIZE, item_ct1.get_local_id(1) + ix * nwarps, k);
#if GGML_SYCL_MMQ_Q6K_DEBUG
                        // Debug: print first vec_dot result and trace inner values
                        if (ib0 == 0 && ir == 0 && k == 0 && iy == 0 && ix == 0 && item_ct1.get_group(0) == 0 &&
                            item_ct1.get_group(1) == 0 && item_ct1.get_group(2) == 0 && item_ct1.get_local_id(0) == 0 &&
                            item_ct1.get_local_id(1) == 0 && item_ct1.get_local_id(2) == 0) {
                            ...
                            sycl::ext::oneapi::experimental::printf("[VEC_DOT] k=%d result=%.6f (isnan=%d isinf=%d)\n",
                                                                    k, dot_result, sycl::isnan(dot_result) ? 1 : 0,
                                                                    sycl::isinf(dot_result) ? 1 : 0);
                            ...
                            sycl::ext::oneapi::experimental::printf(
                                "[VEC_DOT] index_x=%d x_ql[0..3]=0x%08x,0x%08x,0x%08x,0x%08x\n", index_x,
                                tile_x_ql[index_x], tile_x_ql[index_x + 1], tile_x_ql[index_x + 2],
                                tile_x_ql[index_x + 3]);
                            sycl::ext::oneapi::experimental::printf(
                                "[VEC_DOT] index_y=%d y_qs[0..3]=0x%08x,0x%08x,0x%08x,0x%08x\n", index_y,
                                tile_y_qs[index_y], tile_y_qs[index_y + 1], tile_y_qs[index_y + 2],
                                tile_y_qs[index_y + 3]);
        ...
        for (int i = 0; i < mmq_y; i += WARP_SIZE) {
            ...
            if (row_dst < 4 && col_dst < 4) {
                ...
                sycl::ext::oneapi::experimental::printf("[SOA_OUT] row=%d col=%d val=%.6f (isnan=%d isinf=%d)\n",
                                                        row_dst, col_dst, out_val, sycl::isnan(out_val) ? 1 : 0,
                                                        sycl::isinf(out_val) ? 1 : 0);
            }
#endif
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check, bool need_sum = false>
static void mul_mat_q6_K_coalesced(const uint8_t * __restrict__ qs_base,
                                   const size_t qh_offset,
                                   const size_t scales_offset,
                                   const size_t d_offset,
                                   const void * __restrict__ vy,
    ...
    for (int ib0 = 0; ib0 < blocks_per_row_x; ib0 += blocks_per_iter) {
        load_tiles_q6_K_coalesced<mmq_y, nwarps, need_check>(qs_base, qh_offset, scales_offset, d_offset, tile_x_ql,
                                                             tile_x_dm, tile_x_qh, tile_x_sc, item_ct1.get_local_id(1),
                                                             nrows_x - row_x_0 - 1, item_ct1.get_local_id(2),
                                                             blocks_per_row_x, row_x_0, ib0, row_low);
...
#pragma unroll 1
            ...
            if (ib0 == 0 && ir == 0 && item_ct1.get_group(0) == 0 && item_ct1.get_group(1) == 0 &&
                ...
                const float *      y_df      = (const float *) tile_y_ds;
                const sycl::float2 y0f = need_sum ? tile_y_ds[0].convert<float, sycl::rounding_mode::automatic>() :
                                                    sycl::float2(y_df[0], y_df[1]);
                const sycl::float2 y1f = need_sum ? tile_y_ds[1].convert<float, sycl::rounding_mode::automatic>() :
                                                    sycl::float2(y_df[2], y_df[3]);
                sycl::ext::oneapi::experimental::printf("[MMQ_Q6K_COAL] tile_y_qs[0..3]=0x%08x,0x%08x,0x%08x,0x%08x\n",
                                                        y_qs_data[0], y_qs_data[1], y_qs_data[2], y_qs_data[3]);
                sycl::ext::oneapi::experimental::printf(
                    "[MMQ_Q6K_COAL] tile_y_ds[0]=[%.6f,%.6f] tile_y_ds[1]=[%.6f,%.6f]\n", y0f.x(), y0f.y(), y1f.x(),
                    y1f.y());
                sycl::ext::oneapi::experimental::printf("[MMQ_Q6K_COAL] tile_x_qs[0..3]=0x%08x,0x%08x,0x%08x,0x%08x\n",
                                                        x_qs_data[0], x_qs_data[1], x_qs_data[2], x_qs_data[3]);
                sycl::ext::oneapi::experimental::printf("[MMQ_Q6K_COAL] tile_x_dm[0]=%.6f\n", x_df[0]);
            }
#endif
...
#pragma unroll 1
                    for (int ix = 0; ix < mmq_x / nwarps; ++ix) {
                        ...
                        if (ib0 == 0 && ir == 0 && k == 0 && iy == 0 && ix == 0 && item_ct1.get_group(0) == 0 &&
                            ...
                            const int          index_y_ds  = j_idx * (WARP_SIZE / QI8_1) + (ky % WARP_SIZE) / QI8_1;
                            const float *      y_df        = (const float *) tile_y_ds;
                            const sycl::float2 ds0f =
                                need_sum ? tile_y_ds[index_y_ds].convert<float, sycl::rounding_mode::automatic>() :
                            ...
                            sycl::ext::oneapi::experimental::printf("[COAL_VEC_DOT] sc_base=%d sc[0..3]=%d,%d,%d,%d\n",
                                                                    sc_base_idx, sc[0], sc[1], sc[2], sc[3]);
                            sycl::ext::oneapi::experimental::printf(
                                "[COAL_VEC_DOT] index_x=%d x_ql[0..3]=0x%08x,0x%08x,0x%08x,0x%08x\n", index_x,
                                tile_x_ql[index_x], tile_x_ql[index_x + 1], tile_x_ql[index_x + 2],
                                tile_x_ql[index_x + 3]);
                            sycl::ext::oneapi::experimental::printf(
                                "[COAL_VEC_DOT] index_y=%d y_qs[0..3]=0x%08x,0x%08x,0x%08x,0x%08x\n", index_y,
                                tile_y_qs[index_y], tile_y_qs[index_y + 1], tile_y_qs[index_y + 2],
                                tile_y_qs[index_y + 3]);
...
#pragma unroll 1
            for (int i = 0; i < mmq_x; i += nwarps) {
                ...
                const int index_y  = (item_ct1.get_local_id(1) + i) * y_stride + (kqs % y_stride);
...
static void mul_mat_q4_0_soa_persistent(const uint8_t * __restrict__ qs_base,
                                        const size_t d_offset,
                                        const void * __restrict__ vy,
                                        float * __restrict__ dst,
                                        const int ncols_x,
    ...
    while (true) {
        ...
        for (int ib0 = 0; ib0 < blocks_per_row_x; ib0 += blocks_per_iter) {
            load_tiles_q4_0_soa<mmq_y, nwarps, need_check>(
                qs_base, d_offset, tile_x_ql, tile_x_dm, tile_x_qh, tile_x_sc, item_ct1.get_local_id(1),
                nrows_x - row_x_0 - 1, item_ct1.get_local_id(2), blocks_per_row_x, row_x_0, ib0, row_low);
...
#pragma unroll 1
                for (int i = 0; i < mmq_x; i += nwarps) {
                    const int col_y_eff =
                        dpct::min((unsigned int) (col_y_0 + item_ct1.get_local_id(1) + i), ncols_y - 1);
...
#pragma unroll 1
                for (int ids0 = 0; ids0 < mmq_x; ids0 += nwarps * QI8_1) {
                    const int ids =
                        (ids0 + item_ct1.get_local_id(1) * QI8_1 + item_ct1.get_local_id(2) / (MMQ_TILE_NE_K / QI8_1)) %
                        mmq_x;
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
static void mul_mat_q4_0_soa(const uint8_t * __restrict__ qs_base,
                             const size_t d_offset,
                             const void * __restrict__ vy,
                             float * __restrict__ dst,
                             const int                ncols_x,
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
static void mul_mat_q4_0_coalesced(const uint8_t * __restrict__ qs_base,
                                   const size_t d_offset,
                                   const void * __restrict__ vy,
                                   float * __restrict__ dst,
                                   const int                ncols_x,
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
static void mul_mat_q4_0_coalesced_persistent(const uint8_t * __restrict__ qs_base,
                                              const size_t d_offset,
                                              const void * __restrict__ vy,
                                              float * __restrict__ dst,
                                              const int ncols_x,
...
#pragma unroll 1
                            for (int ix = 0; ix < mmq_x / nwarps; ++ix) {
                                sum[iy][ix] +=
                                    vec_dot_q4_0_q8_1_mul_mat_soa(tile_x_ql, tile_x_dm, tile_x_qh, tile_x_sc, tile_y_qs,
                                                                  tile_y_ds, item_ct1.get_local_id(2) + iy * WARP_SIZE,
...
static void mul_mat_q8_0(const void * __restrict__ vx,
    ...
    for (int ib0 = 0; ib0 < blocks_per_row_x; ib0 += blocks_per_iter) {
        ...
        load_tiles_q8_0<mmq_y, nwarps, need_check>(x + row_x_0 * blocks_per_row_x + ib0, tile_x_ql, tile_x_df, nullptr,
                                                   nullptr, item_ct1.get_local_id(1), nrows_x - row_x_0 - 1,
                                                   item_ct1.get_local_id(2), ib0, blocks_per_row_x);
...
#pragma unroll 1
            for (int ids0 = 0; ids0 < mmq_x; ids0 += nwarps * QI8_1) {
                ...
                const block_q8_1 * by0 =
                    &y[col_y_eff * blocks_per_col_y + ib0 * (qk / QK8_1) + ir * (MMQ_TILE_NE_K / QI8_1) + kby];
...
#pragma unroll 1
                    for (int ix = 0; ix < mmq_x / nwarps; ++ix) {
                        sum[iy][ix] += vec_dot_q8_0_q8_1_mul_mat(tile_x_ql, tile_x_df, nullptr, nullptr, tile_y_qs,
                                                                 tile_y_df, item_ct1.get_local_id(2) + iy * WARP_SIZE,
                                                                 item_ct1.get_local_id(1) + ix * nwarps, k);
                    }
                }
            }
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
static void mul_mat_q8_0_aos(const void * __restrict__ vx,
                             const void * __restrict__ vy,
                             float * __restrict__ dst,
                             const int                ncols_x,
                             const int                nrows_x,
    ...
    for (int ib0 = 0; ib0 < blocks_per_row_x; ib0 += blocks_per_iter) {
        ...
        load_tiles_q8_0_debug<mmq_y, nwarps, need_check>(x + row_x_0 * blocks_per_row_x + ib0, tile_x_ql, tile_x_df,
...
// Uses float* directly for both X and Y tile scales (no half2* casting)
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
static void mul_mat_q8_0_soa(const int8_t * __restrict__ qs_base,
                             const size_t d_offset,
                             const void * __restrict__ vy,
                             float * __restrict__ dst,
                             const int                ncols_x,
    ...
    for (int ib0 = 0; ib0 < blocks_per_row_x; ib0 += blocks_per_iter) {
        ...
        load_tiles_q8_0_soa<mmq_y, nwarps, need_check>(
            qs_base, d_offset, tile_x_ql, tile_x_df, item_ct1.get_local_id(1), nrows_x - row_x_0 - 1,
            item_ct1.get_local_id(2), blocks_per_row_x, row_x_0, ib0, row_low, debug_stream, debug_counter);
...
template <int mmq_x, int mmq_y, int nwarps, bool need_check>
static void mul_mat_q8_0_coalesced(const int8_t * __restrict__ qs_base,
                                   const size_t d_offset,
                                   const void * __restrict__ vy,
                                   float * __restrict__ dst,
                                   const int                ncols_x,
...
static void ggml_mul_mat_q4_0_q8_1_sycl(const void *    vx,
                                        const void *    vy,
                                        float *         dst,
                                        const int       ncols_x,
                                        const int       nrows_x,
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<int, 1>         tile_y_qs_acc_ct1(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1),
                                                                       cgh);

                ggml_sycl_parallel_for_grf<mmq_q4_0_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q4_0<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_qs_q4_0_acc_ct1),
                                                 get_pointer(tile_x_d_q4_0_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
...
bool ggml_sycl_mmq_bench_launch(const mmq_bench_args & args, std::vector<sycl::event> * events) {
    ...
    ::ggml_sycl_mmq_dispatch(&fake, static_cast<const char *>(args.weights), layout_base, use_layout, device_id,
                             args.ncols, args.nrows, args.ncols, args.row_low, args.row_high, args.batch,
                             args.src1_padded_row_size, static_cast<const char *>(args.activations), args.output,
                             args.dst_row_stride, 0, args.stream);
...
static void ggml_mul_mat_q6_K_q8_1_sycl_coalesced(const void *    vx,
                                                  const void *    vy,
                                                  float *         dst,
                                                  const int       ncols_x,
                                                  const int       nrows_x,
...
#define LAUNCH_Q6_K_COALESCED_KERNEL(MMQ_X, MMQ_Y, NWARPS)                                                            \
    do {                                                                                                              \
        ...
        if (!need_check) {                                                                                            \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                             \
                sycl::local_accessor<int, 1> tile_x_qs(sycl::range<1>(mmq_y * (2 * MMQ_TILE_NE_K) + mmq_y), cgh);     \
                sycl::local_accessor<sycl::half2, 1> tile_x_dm(                                                       \
                    sycl::range<1>(mmq_y * (MMQ_TILE_NE_K / QI6_K) + mmq_y / QI6_K), cgh);                            \
                sycl::local_accessor<int, 1> tile_x_sc(sycl::range<1>(mmq_y * (MMQ_TILE_NE_K / 8) + mmq_y / 8), cgh); \
                ...
                ggml_sycl_parallel_for_grf<mmq_q6_K_coalesced_kernel<mmq_x, mmq_y, nwarps, false>>(                   \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {     \
                        mul_mat_q6_K_coalesced<mmq_x, mmq_y, nwarps, false>(                                          \
                            qs_base, qh_offset, scales_offset, d_offset, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, \
                            nrows_y_unpadded, nrows_dst, row_low, item_ct1, get_pointer(tile_x_qs),                   \
                            get_pointer(tile_x_dm), get_pointer(tile_x_sc), get_pointer(tile_y_qs),                   \
                            get_pointer(tile_y_ds));                                                                  \
                    });                                                                                               \
            });                                                                                                       \
        } else {                                                                                                      \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                             \
                ...
                sycl::local_accessor<int, 1> tile_y_qs(sycl::range<1>(mmq_x * WARP_SIZE), cgh);                       \
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * (WARP_SIZE / QI8_1)), cgh);     \
                ggml_sycl_parallel_for_grf<mmq_q6_K_coalesced_kernel<mmq_x, mmq_y, nwarps, true>>(                    \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {     \
...
static void ggml_mul_mat_q4_0_q8_1_sycl_soa(const void *    vx,
                                            const void *    vy,
                                            float *         dst,
                                            const int       ncols_x,
                                            const int       nrows_x,
    ...
    if (use_persistent_mmq_kernel()) {
        ...
        if (!need_check) {
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<float, 1> tile_x_d(sycl::range<1>(mmq_y * (MMQ_TILE_NE_K / QI4_0) + mmq_y / QI4_0),
                                                        cgh);
                ...
                ggml_sycl_parallel_for_grf<mmq_q4_0_soa_persistent_kernel<mmq_x, mmq_y, nwarps, false>>(
                    cgh, sycl::nd_range<3>(block_nums_persistent * block_dims, block_dims),
                    [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q4_0_soa_persistent<mmq_x, mmq_y, nwarps, false>(
                            qs_base, d_offset, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_y_unpadded, nrows_dst,
                            row_low, work_counter, total_tiles, num_col_tiles, item_ct1, get_pointer(tile_x_qs),
                            get_pointer(tile_x_d), get_pointer(tile_y_qs), get_pointer(tile_y_ds));
                    });
            });
        } else {
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<int, 1>   tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh);
                ggml_sycl_parallel_for_grf<mmq_q4_0_soa_persistent_kernel<mmq_x, mmq_y, nwarps, true>>(
                    cgh, sycl::nd_range<3>(block_nums_persistent * block_dims, block_dims),
                    [=](sycl::nd_item<3> item_ct1) {
...
#define LAUNCH_Q4_0_SOA_KERNEL(MMQ_X, MMQ_Y, NWARPS)                                                                \
    do {                                                                                                            \
        ...
        if (!need_check) {                                                                                          \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                           \
                sycl::local_accessor<int, 1>   tile_x_qs(sycl::range<1>(mmq_y * MMQ_TILE_NE_K + mmq_y), cgh);       \
                sycl::local_accessor<float, 1> tile_x_d(                                                            \
                    sycl::range<1>(mmq_y * (MMQ_TILE_NE_K / QI4_0) + mmq_y / QI4_0), cgh);                          \
                sycl::local_accessor<int, 1>         tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);         \
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh); \
                ggml_sycl_parallel_for_grf<mmq_q4_0_soa_kernel<mmq_x, mmq_y, nwarps, false>>(                       \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {   \
                        mul_mat_q4_0_soa<mmq_x, mmq_y, nwarps, false>(                                              \
                            qs_base, d_offset, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_y_unpadded,       \
                            nrows_dst, row_low, item_ct1, get_pointer(tile_x_qs), get_pointer(tile_x_d),            \
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                           \
                ...
                sycl::local_accessor<int, 1>         tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);         \
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh); \
                ggml_sycl_parallel_for_grf<mmq_q4_0_soa_kernel<mmq_x, mmq_y, nwarps, true>>(                        \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {   \
...
static void ggml_mul_mat_q4_0_q8_1_sycl_coalesced(const void *    vx,
                                                  const void *    vy,
                                                  float *         dst,
                                                  const int       ncols_x,
                                                  const int       nrows_x,
    ...
    if (use_persistent_mmq_kernel()) {
        ...
        if (!need_check) {
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<int, 1>   tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh);
                ggml_sycl_parallel_for_grf<mmq_q4_0_coalesced_persistent_kernel<mmq_x, mmq_y, nwarps, false>>(
                    cgh, sycl::nd_range<3>(block_nums_persistent * block_dims, block_dims),
                    [=](sycl::nd_item<3> item_ct1) {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<int, 1>   tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh);
                ggml_sycl_parallel_for_grf<mmq_q4_0_coalesced_persistent_kernel<mmq_x, mmq_y, nwarps, true>>(
                    cgh, sycl::nd_range<3>(block_nums_persistent * block_dims, block_dims),
                    [=](sycl::nd_item<3> item_ct1) {
    ...
    do {                                                                                                            \
        ...
        if (!need_check) {                                                                                          \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                           \
                ...
                sycl::local_accessor<int, 1>         tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);         \
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh); \
                ggml_sycl_parallel_for_grf<mmq_q4_0_coalesced_kernel<mmq_x, mmq_y, nwarps, false>>(                 \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {   \
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                           \
                ...
                sycl::local_accessor<int, 1>         tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);         \
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh); \
                ggml_sycl_parallel_for_grf<mmq_q4_0_coalesced_kernel<mmq_x, mmq_y, nwarps, true>>(                  \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {   \
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                ggml_sycl_parallel_for_grf<mmq_q4_1_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q4_1<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_qs_q4_1_acc_ct1),
                                                 get_pointer(tile_x_dm_q4_1_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
                                                 get_pointer(tile_y_ds_acc_ct1));
                    });
            });
        }
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                sycl::local_accessor<int, 1>   tile_x_ql_q5_0_acc_ct1(sycl::range<1>(mmq_y * (2 * WARP_SIZE) + mmq_y),
                                                                      cgh);
                ...
                ggml_sycl_parallel_for_grf<mmq_q5_0_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q5_0<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_ql_q5_0_acc_ct1),
                                                 get_pointer(tile_x_d_q5_0_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
                                                 get_pointer(tile_y_ds_acc_ct1));
                    });
            });
        }
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                sycl::local_accessor<int, 1> tile_x_ql_q5_1_acc_ct1(sycl::range<1>(mmq_y * (2 * WARP_SIZE) + mmq_y),
                                                                    cgh);
                ...
                ggml_sycl_parallel_for_grf<mmq_q5_1_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q5_1<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_ql_q5_1_acc_ct1),
                                                 get_pointer(tile_x_dm_q5_1_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
                                                 get_pointer(tile_y_ds_acc_ct1));
                    });
            });
        }
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<float, 1> tile_y_df_acc_ct1(sycl::range<1>(mmq_x * WARP_SIZE / QI8_1), cgh);

                ggml_sycl_parallel_for_grf<mmq_q8_0_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q8_0<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_qs_q8_0_acc_ct1),
                                                 get_pointer(tile_x_d_q8_0_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
...
// nrows_y = padded Y dimension (for stride), nrows_y_unpadded = actual Y dimension (for ds offset)
static void ggml_mul_mat_q8_0_q8_1_sycl_soa(const void *    vx,
                                            const void *    vy,
                                            float *         dst,
                                            const int       ncols_x,
                                            const int       nrows_x,
    ...
    do {                                                                                                          \
        ...
        if (!need_check) {                                                                                        \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                         \
                sycl::local_accessor<int, 1>   tile_x_qs(sycl::range<1>(mmq_y * MMQ_TILE_NE_K + mmq_y), cgh);     \
                sycl::local_accessor<float, 1> tile_x_d(                                                          \
                    sycl::range<1>(mmq_y * (MMQ_TILE_NE_K / QI8_0) + mmq_y / QI8_0), cgh);                        \
                sycl::local_accessor<int, 1>   tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);             \
                sycl::local_accessor<float, 1> tile_y_df(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh);     \
                ggml_sycl_parallel_for_grf<mmq_q8_0_soa_kernel<mmq_x, mmq_y, nwarps, false>>(                     \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) { \
                        mul_mat_q8_0_soa<mmq_x, mmq_y, nwarps, false>(                                            \
                            qs_base, d_offset, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_y_unpadded,     \
                            nrows_dst, row_low, item_ct1, get_pointer(tile_x_qs), get_pointer(tile_x_d),          \
                            get_pointer(tile_y_qs), get_pointer(tile_y_df), nullptr, nullptr);                    \
                    });                                                                                           \
            });                                                                                                   \
        } else {                                                                                                  \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                         \
                ...
                sycl::local_accessor<int, 1>   tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);             \
                sycl::local_accessor<float, 1> tile_y_df(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh);     \
                ggml_sycl_parallel_for_grf<mmq_q8_0_soa_kernel<mmq_x, mmq_y, nwarps, true>>(                      \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) { \
...
static void ggml_mul_mat_q8_0_q8_1_sycl_coalesced(const void *    vx,
                                                  const void *    vy,
                                                  float *         dst,
                                                  const int       ncols_x,
                                                  const int       nrows_x,
    ...
    do {                                                                                                          \
        ...
        if (!need_check) {                                                                                        \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                         \
                ...
                sycl::local_accessor<int, 1>   tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);             \
                sycl::local_accessor<float, 1> tile_y_df(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh);     \
                ggml_sycl_parallel_for_grf<mmq_q8_0_coalesced_kernel<mmq_x, mmq_y, nwarps, false>>(               \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) { \
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                         \
                ...
                sycl::local_accessor<int, 1>   tile_y_qs(sycl::range<1>(mmq_x * MMQ_TILE_NE_K), cgh);             \
                sycl::local_accessor<float, 1> tile_y_df(sycl::range<1>(mmq_x * MMQ_TILE_NE_K / QI8_1), cgh);     \
                ggml_sycl_parallel_for_grf<mmq_q8_0_coalesced_kernel<mmq_x, mmq_y, nwarps, true>>(                \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) { \
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<int, 1> tile_x_sc_q2_K_acc_ct1(sycl::range<1>(mmq_y * (WARP_SIZE / 4) + mmq_y / 4),
                                                                    cgh);
                sycl::local_accessor<int, 1> tile_y_qs_acc_ct1(sycl::range<1>(mmq_x * WARP_SIZE), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(sycl::range<1>(mmq_x * WARP_SIZE / QI8_1), cgh);

                ggml_sycl_parallel_for_grf<mmq_q2_K_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q2_K<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_ql_q2_K_acc_ct1),
                                                 get_pointer(tile_x_dm_q2_K_acc_ct1),
                                                 get_pointer(tile_x_sc_q2_K_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
                                                 get_pointer(tile_y_ds_acc_ct1));
                    });
            });
        }
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<int, 1> tile_x_qh_q3_K_acc_ct1(sycl::range<1>(mmq_y * (WARP_SIZE / 2) + mmq_y / 2),
                                                                    cgh);
                sycl::local_accessor<int, 1> tile_x_sc_q3_K_acc_ct1(sycl::range<1>(mmq_y * (WARP_SIZE / 4) + mmq_y / 4),
                                                                    cgh);
                sycl::local_accessor<int, 1> tile_y_qs_acc_ct1(sycl::range<1>(mmq_x * WARP_SIZE), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(sycl::range<1>(mmq_x * WARP_SIZE / QI8_1), cgh);

                ggml_sycl_parallel_for_grf<mmq_q3_K_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q3_K<need_check>(
                            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                            get_pointer(tile_x_ql_q3_K_acc_ct1), get_pointer(tile_x_dm_q3_K_acc_ct1),
                            get_pointer(tile_x_qh_q3_K_acc_ct1), get_pointer(tile_x_sc_q3_K_acc_ct1),
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                ...
                sycl::local_accessor<int, 1> tile_x_sc_q4_K_acc_ct1(sycl::range<1>(mmq_y * (WARP_SIZE / 8) + mmq_y / 8),
                                                                    cgh);
                sycl::local_accessor<int, 1> tile_y_qs_acc_ct1(sycl::range<1>(mmq_x * QR4_K * WARP_SIZE), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(
                    sycl::range<1>(mmq_x * QR4_K * WARP_SIZE / QI8_1), cgh);

                ggml_sycl_parallel_for_grf<mmq_q4_K_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q4_K<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_ql_q4_K_acc_ct1),
                                                 get_pointer(tile_x_dm_q4_K_acc_ct1),
                                                 get_pointer(tile_x_sc_q4_K_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
                                                 get_pointer(tile_y_ds_acc_ct1));
                    });
            });
        }
        ...
        */
        {
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {
                sycl::local_accessor<int, 1> tile_x_ql_q5_K_acc_ct1(sycl::range<1>(mmq_y * (2 * WARP_SIZE) + mmq_y),
                                                                    cgh);
                sycl::local_accessor<sycl::half2, 1> tile_x_dm_q5_K_acc_ct1(
                    sycl::range<1>(mmq_y * (WARP_SIZE / QI5_K) + mmq_y / QI5_K), cgh);
                sycl::local_accessor<int, 1> tile_x_sc_q5_K_acc_ct1(sycl::range<1>(mmq_y * (WARP_SIZE / 8) + mmq_y / 8),
                                                                    cgh);
                sycl::local_accessor<int, 1> tile_y_qs_acc_ct1(sycl::range<1>(mmq_x * QR5_K * WARP_SIZE), cgh);
                sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(
                    sycl::range<1>(mmq_x * QR5_K * WARP_SIZE / QI8_1), cgh);

                ggml_sycl_parallel_for_grf<mmq_q5_K_kernel<need_check>>(
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                        mul_mat_q5_K<need_check>(vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,
                                                 get_pointer(tile_x_ql_q5_K_acc_ct1),
                                                 get_pointer(tile_x_dm_q5_K_acc_ct1),
                                                 get_pointer(tile_x_sc_q5_K_acc_ct1), get_pointer(tile_y_qs_acc_ct1),
                                                 get_pointer(tile_y_ds_acc_ct1));
                    });
            });
        }
...
#define LAUNCH_Q6_K_KERNEL(MMQ_X, MMQ_Y, NWARPS, VARIANT_ID)                                                            \
    do {                                                                                                                \
        ...
        if (!need_check) {                                                                                              \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                               \
                sycl::local_accessor<int, 1> tile_x_ql_acc_ct1(sycl::range<1>(mmq_y * (2 * MMQ_TILE_NE_K) + mmq_y),     \
                                                               cgh);                                                    \
                sycl::local_accessor<sycl::half2, 1> tile_x_dm_acc_ct1(                                                 \
                    sycl::range<1>(mmq_y * (MMQ_TILE_NE_K / QI6_K) + mmq_y / QI6_K), cgh);                              \
                sycl::local_accessor<int, 1> tile_x_sc_acc_ct1(                                                         \
                    sycl::range<1>(mmq_y * (MMQ_TILE_NE_K / 8) + mmq_y / 8), cgh);                                      \
                sycl::local_accessor<int, 1>         tile_y_qs_acc_ct1(sycl::range<1>(mmq_x * QR6_K * WARP_SIZE), cgh); \
                sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(                                                 \
                    sycl::range<1>(mmq_x * QR6_K * WARP_SIZE / QI8_1), cgh);                                            \
                ggml_sycl_parallel_for_grf<mmq_q6_K_kernel<mmq_x, mmq_y, nwarps, false, VARIANT_ID>>(                   \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {       \
                        mul_mat_q6_K<mmq_x, mmq_y, nwarps, false>(                                                      \
                            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst, item_ct1,                       \
                            get_pointer(tile_x_ql_acc_ct1), get_pointer(tile_x_dm_acc_ct1),                             \
                            get_pointer(tile_x_sc_acc_ct1), get_pointer(tile_y_qs_acc_ct1),                             \
                            get_pointer(tile_y_ds_acc_ct1));                                                            \
                    });                                                                                                 \
            });                                                                                                         \
        } else {                                                                                                        \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                               \
                ...
                sycl::local_accessor<sycl::half2, 1> tile_y_ds_acc_ct1(                                                 \
                    sycl::range<1>(mmq_x * QR6_K * WARP_SIZE / QI8_1), cgh);                                            \
                ggml_sycl_parallel_for_grf<mmq_q6_K_kernel<mmq_x, mmq_y, nwarps, true, VARIANT_ID>>(                    \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {       \
    ...
    if (compute_capability >= VER_GEN13) {
        ...
        if (tune == ggml_sycl_q6k_tune::perf) {
            ...
            LAUNCH_Q6_K_KERNEL(MMQ_X_Q6_K_AMPERE_SPILLFREE, MMQ_Y_Q6_K_AMPERE_SPILLFREE, NWARPS_Q6_K_AMPERE_SPILLFREE,
                               4);
        }
#else
...
static void ggml_mul_mat_q6_K_q8_1_sycl_soa(const void *    vx,
                                            const void *    vy,
                                            float *         dst,
                                            const int       ncols_x,
                                            const int       nrows_x,
    ...
    do {                                                                                                              \
        ...
        if (!need_check) {                                                                                            \
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                             \
                ...
                sycl::local_accessor<int, 1> tile_y_qs(sycl::range<1>(mmq_x * QR6_K * WARP_SIZE), cgh);               \
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * QR6_K * WARP_SIZE / QI8_1),     \
                                                               cgh);                                                  \
                ggml_sycl_parallel_for_grf<mmq_q6_K_soa_kernel<mmq_x, mmq_y, nwarps, false>>(                         \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {     \
            ...
            mmq_submit(stream, [&](sycl::handler & cgh) {                                                             \
                ...
                sycl::local_accessor<sycl::half2, 1> tile_y_ds(sycl::range<1>(mmq_x * QR6_K * WARP_SIZE / QI8_1),     \
                                                               cgh);                                                  \
                ggml_sycl_parallel_for_grf<mmq_q6_K_soa_kernel<mmq_x, mmq_y, nwarps, true>>(                          \
                    cgh, sycl::nd_range<3>(block_nums * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {     \
    ...
    switch (src0->type) {
        case GGML_TYPE_Q4_0:
            if (use_layout == GGML_LAYOUT_COALESCED) {
                ...
                GGML_SYCL_KTRACE("mmq_q4_0_coalesced",
                                 " ne00=%lld row_low=%lld row_diff=%lld ncols=%lld d_offset=%zu global_row=%lld",
                                 (long long) ne00, (long long) row_low, (long long) row_diff, (long long) src1_ncols,
                                 d_offset, (long long) layout_row_low);
            ...
            break;
        case GGML_TYPE_Q4_1:
            GGML_SYCL_KTRACE("mmq_q4_1", " ne00=%lld row_diff=%lld ncols=%lld", (long long) ne00, (long long) row_diff,
                             (long long) src1_ncols);
            ggml_mul_mat_q4_1_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00, row_diff, src1_ncols,
                                        src1_padded_row_size, nrows_dst, stream);
            break;
        case GGML_TYPE_Q5_0:
            GGML_SYCL_KTRACE("mmq_q5_0", " ne00=%lld row_diff=%lld ncols=%lld", (long long) ne00, (long long) row_diff,
                             (long long) src1_ncols);
            ggml_mul_mat_q5_0_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00, row_diff, src1_ncols,
                                        src1_padded_row_size, nrows_dst, stream);
            break;
        case GGML_TYPE_Q5_1:
            GGML_SYCL_KTRACE("mmq_q5_1", " ne00=%lld row_diff=%lld ncols=%lld", (long long) ne00, (long long) row_diff,
                             (long long) src1_ncols);
        ...
        case GGML_TYPE_Q8_0:
            ...
            if (mmq_xmx::can_use_xmx_q8_0(xmx_caps, static_cast<int>(ne00))) {
                GGML_SYCL_KTRACE("mmq_q8_0_xmx", " ne00=%lld row_diff=%lld ncols=%lld nrows_dst=%lld", (long long) ne00,
                                 (long long) row_diff, (long long) src1_ncols, (long long) nrows_dst);
                ...
                mmq_xmx::launch_mmq_xmx_q8_0<64, 32, 8>(src0_dd_i,                               // Q8_0 weights
                ...
                const size_t d_offset       = static_cast<size_t>(layout_rows) *
                                        ggml_sycl_q8_0_coalesced_row_quants_bytes(static_cast<int>(blocks_per_row));

                GGML_SYCL_KTRACE("mmq_q8_0_coalesced",
                                 " ne00=%lld row_low=%lld row_diff=%lld ncols=%lld d_offset=%zu global_row=%lld",
                                 (long long) ne00, (long long) row_low, (long long) row_diff, (long long) src1_ncols,
                                 d_offset, (long long) layout_row_low);
            ...
            break;
        case GGML_TYPE_Q2_K:
            GGML_SYCL_KTRACE("mmq_q2_k", " ne00=%lld row_diff=%lld ncols=%lld", (long long) ne00, (long long) row_diff,
                             (long long) src1_ncols);
            ggml_mul_mat_q2_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00, row_diff, src1_ncols,
                                        src1_padded_row_size, nrows_dst, stream);
            break;
        case GGML_TYPE_Q3_K:
            GGML_SYCL_KTRACE("mmq_q3_k", " ne00=%lld row_diff=%lld ncols=%lld", (long long) ne00, (long long) row_diff,
                             (long long) src1_ncols);
            ggml_mul_mat_q3_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00, row_diff, src1_ncols,
                                        src1_padded_row_size, nrows_dst, stream);
            break;
        case GGML_TYPE_Q4_K:
            GGML_SYCL_KTRACE("mmq_q4_k", " ne00=%lld row_diff=%lld ncols=%lld", (long long) ne00, (long long) row_diff,
                             (long long) src1_ncols);
            ggml_mul_mat_q4_K_q8_1_sycl(src0_dd_i, src1_ddq_i, dst_dd_i, ne00, row_diff, src1_ncols,
                                        src1_padded_row_size, nrows_dst, stream);
            break;
        case GGML_TYPE_Q5_K:
            GGML_SYCL_KTRACE("mmq_q5_k", " ne00=%lld row_diff=%lld ncols=%lld", (long long) ne00, (long long) row_diff,
                             (long long) src1_ncols);
        ...
        case GGML_TYPE_Q6_K:
            if (use_layout == GGML_LAYOUT_COALESCED) {
                ...
                const size_t  row_quants_bytes = mmq_q6_k_coalesced_row_quants_bytes(static_cast<int>(blocks_per_row));
                const size_t  d_offset         = static_cast<size_t>(layout_rows) * row_quants_bytes;

                GGML_SYCL_KTRACE("mmq_q6_k_coalesced",
                                 " ne00=%lld row_low=%lld row_diff=%lld ncols=%lld d_offset=%zu global_row=%lld",
                                 (long long) ne00, (long long) row_low, (long long) row_diff, (long long) src1_ncols,
                                 d_offset, (long long) layout_row_low);
                ggml_mul_mat_q6_K_q8_1_sycl_coalesced(layout_base, src1_ddq_i, dst_dd_i, ne00, row_diff, src1_ncols,
                                                      src1_padded_row_size, ne10, nrows_dst, 0, 0, d_offset,
                                                      static_cast<int>(layout_row_low), stream);
            } else if (use_layout == GGML_LAYOUT_SOA) {
                ...
                GGML_SYCL_KTRACE(
                    "mmq_q6_k_soa",
                    " ne00=%lld row_low=%lld row_diff=%lld ncols=%lld qh=%zu scales=%zu d=%zu global_row=%lld",
...
static sycl::event mmq_stream_slice(sycl::queue &                    queue,
    ...
    ggml_sycl_mmq_dispatch(ctx->src0, static_cast<const char *>(device_slice), layout_ptr, ctx->layout, ctx->device_id,
                           ctx->ne00, static_cast<int64_t>(row_count), ctx->ne10, 0, static_cast<int64_t>(row_count),
                           ctx->src1_ncols, ctx->src1_padded_row_size, ctx->src1_ddq_i, dst_ptr, ctx->dst_row_stride, 0,
                           &queue);
...
void ggml_sycl_op_mul_mat_q(ggml_backend_sycl_context & ctx,
    ...
    const bool prefer_coalesced = !explicit_layout && coalesced_supported && src1_ncols >= 64;
    ...
    if (view.ptr && view.location != ggml_sycl::cache_location::DEVICE) {
        ...
        auto result = cache->stream_dma(view, total_bytes, slice_bytes, buffer_count, mmq_stream_slice, &stream_ctx,
                                        deps, custom_copy ? mmq_stream_copy : nullptr);
        if (result.ok) {
            std::vector<sycl::event> done_deps{ result.event };
            result.event = ggml_sycl_submit_marker<ggml_sycl_mmq_marker_kernel>(*stream, done_deps);
        }
        if (cache_key.valid) {
            if (result.ok) {
