...
static void g5a() {
    ...
    GraphEpoch g{}; InvocationId i{}; const int devices[] = {0,1}; const int participants[] = {0}; snapshot snap{};
    require(reg.begin_graph(ctx,s,e,root,&g)==error::OK && reg.begin_invocation(ctx,s,e,g,root,devices,2,participants,1,0,&i)==error::OK, "G5a invoke failed");
    ...
    require(reg.complete_invocation(ctx,s,e,g,i,root,0)==error::OK, "G5a participant complete failed");
...
static void g6() {
    ...
    GraphEpoch g{}; InvocationId i{}; InvocationId j{}; const int devices[] = {0,1}; const int participants[] = {0,1};
    require(reg.begin_graph(ctx,s,e,root,&g)==error::OK && reg.begin_invocation(ctx,s,e,g,root,devices,2,participants,2,0,&i)==error::OK, "G6 invoke failed");
    require(reg.begin_invocation(ctx,s,e,g,root,devices,2,participants,2,1,&j)==error::OK && j.value == i.value, "G6 join failed");
    require(reg.seal_invocation(ctx,s,e,g,i,root)==error::OK, "G6 seal failed");
    require(reg.complete_invocation(ctx,s,e,g,i,root,1)==error::OK, "G6 first sync failed");
    require(reg.begin_graph(ctx,s,e,root,&g)==error::BUSY, "G6 terminal overwritten before final sync");
    require(reg.complete_invocation(ctx,s,e,g,i,root,0)==error::OK, "G6 second sync failed");
...
static void g6b() {
    ...
    GraphEpoch g{}; InvocationId i{}; InvocationId j{}; const int devices[] = {0,1}; const int participants[] = {0,1}; snapshot snap{};
    require(reg.begin_graph(ctx,s,e,root,&g)==error::OK && reg.begin_invocation(ctx,s,e,g,root,devices,2,participants,2,0,&i)==error::OK, "G6b invoke A failed");
    require(reg.seal_invocation(ctx,s,e,g,i,root)==error::OK, "G6b seal failed");
    require(reg.complete_invocation(ctx,s,e,g,i,root,0)==error::OK, "G6b early complete failed");
    require(reg.extract(ctx,&snap)==error::OK && snap.invocation.value!=0, "G6b finalized before reserved join");
    require(reg.begin_invocation(ctx,s,e,g,root,devices,2,participants,2,1,&j)==error::OK && j.value==i.value, "G6b join after completion failed");
    require(reg.complete_invocation(ctx,s,e,g,i,root,1)==error::OK, "G6b final complete failed");
    require(reg.extract(ctx,&snap)==error::OK && snap.invocation.value==0, "G6b final release failed");
}
...
static void g7() {
    ...
    GraphEpoch ga{}, gb{}; InvocationId ia{}, ib{}; const int d[] = {2}; const int p[] = {2};
    require(reg.begin_graph(a,sa,ea,ra,&ga)==error::OK && reg.begin_invocation(a,sa,ea,ga,ra,d,1,p,1,2,&ia)==error::OK, "G7 invoke A failed");
    require(reg.begin_graph(b,sb,eb,rb,&gb)==error::OK && reg.begin_invocation(b,sb,eb,gb,rb,d,1,p,1,2,&ib)==error::DEVICE_BUSY, "G7 missing same-device busy");
    require(reg.complete_invocation(a,sa,ea,ga,ia,ra,2)==error::OK, "G7 complete A failed");
    require(reg.begin_invocation(b,sb,eb,gb,rb,d,1,p,1,2,&ib)==error::OK, "G7 did not recover after release");
}
