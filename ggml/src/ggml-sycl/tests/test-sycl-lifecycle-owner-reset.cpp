...
static void h6() {
    ...
    require(reg.begin_invocation(a, sa, ea, ga, ra, da, 2, 0, &ia) == error::OK, "H6 invoke A failed");
    require(reg.begin_invocation(b, sb, eb, gb, rb, db, 1, 1, &ib) == error::DEVICE_BUSY, "H6 missing DEVICE_BUSY");
    require(reg.complete_invocation(a, sa, ea, ga, ia, ra, 0) == error::OK, "H6 participant complete failed");
    require(reg.begin_invocation(b, sb, eb, gb, rb, db, 1, 1, &ib) == error::OK, "H6 exact owner release failed");
}
...
static void h7() {
    ...
    require(reg.begin_graph(ctx, s, e, root, &g) == error::OK && reg.begin_invocation(ctx, s, e, g, root, d, 1, 0, &i) == error::OK, "H7 invoke failed");
...
static void h11() {
    ...
    require(reg.begin_graph(ctx, s, e, root, &g) == error::OK && reg.begin_invocation(ctx, s, e, g, root, d, 1, 0, &i) == error::OK, "H11 invoke failed");
    ...
    require(reg.begin_graph(ctx, s, e, root, &g) == error::OK && reg.begin_invocation(ctx, s, e, g, root, d, 1, 0, &i) == error::OK, "H11 invoke2 failed");
