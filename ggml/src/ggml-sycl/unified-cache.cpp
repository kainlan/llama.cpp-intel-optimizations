...
static uint32_t pp_moe_onednn_effective_ring_depth(uint32_t requested_ring_depth) {
    ...
    // Current PP MoE oneDNN staging is serialized by pp_moe_onednn_scratch_claim,
    // so one reusable slot is the accurate budget. Reserving extra future
    // pipeline slots starves the B50 weight arena for GPT-OSS 20B.
    return 1;
}

static size_t pp_moe_onednn_planned_scratch_bytes(size_t   weight_slot_bytes,
                                                  size_t   activation_slot_bytes,
                                                  size_t   output_slot_bytes,
                                                  uint32_t requested_ring_depth) {
    const uint32_t ring_depth = pp_moe_onednn_effective_ring_depth(requested_ring_depth);
    return static_cast<size_t>(ring_depth) * (weight_slot_bytes + activation_slot_bytes + output_slot_bytes);
}

uint32_t test_pp_moe_onednn_effective_ring_depth(uint32_t requested_ring_depth) {
    return pp_moe_onednn_effective_ring_depth(requested_ring_depth);
}
...
size_t test_pp_moe_onednn_planned_scratch_bytes(size_t   weight_slot_bytes,
                                                ...
                                                size_t   output_slot_bytes,
                                                uint32_t requested_ring_depth) {
    return pp_moe_onednn_planned_scratch_bytes(weight_slot_bytes, activation_slot_bytes, output_slot_bytes,
...
void unified_cache_set_planned_pp_moe_onednn_scratch(int      device_id,
    ...
    g_planned_pp_moe_onednn_scratch_bytes[device_id].store(total, std::memory_order_release);
    g_planned_pp_moe_onednn_ring_depth[device_id].store(effective_ring_depth, std::memory_order_release);
}
...
size_t unified_cache_get_planned_pp_moe_onednn_weight_slot_bytes(int device_id) {
    ...
    return g_planned_pp_moe_onednn_weight_slot_bytes[device_id].load(std::memory_order_acquire);
}
...
size_t unified_cache_get_planned_pp_moe_onednn_activation_slot_bytes(int device_id) {
    ...
    return g_planned_pp_moe_onednn_activation_slot_bytes[device_id].load(std::memory_order_acquire);
}
...
size_t unified_cache_get_planned_pp_moe_onednn_output_slot_bytes(int device_id) {
    ...
    return g_planned_pp_moe_onednn_output_slot_bytes[device_id].load(std::memory_order_acquire);
}
...
size_t unified_cache_get_planned_pp_moe_onednn_scratch_bytes(int device_id) {
    ...
    return g_planned_pp_moe_onednn_scratch_bytes[device_id].load(std::memory_order_acquire);
}
...
// ============================================================================
//
// Phase 0 of the "retire zone reset" epic.  Every reset site reports what is
// still live in its zone, so each escape can be fixed BEFORE the reset that
// hides it is deleted.  This is instrumentation only: it does not delete or
// weaken any reset and does not weaken any `refusing ...` guard.
//
// Why the existing dumps are not enough: both the VRAM and the host reset
// already log live records, but only for the first 4 refusals per zone and only
// 8 lines within each.  A long run therefore shows at most 32 lines per zone and
// tells you nothing about which cohorts recur, at what sizes, or how often.
// The deliverable of this task is an inventory, so the audit must be uncapped in
// what it *aggregates* even while it stays bounded in what it *prints*.
...
struct zone_audit_state {
    ...
    std::atomic<bool>                                                     atexit_installed{ false };
};
...
// covered directly -- the watchdog calls zone_reset_audit_report() before
// _Exit, and a fatal-signal handler flushes on SIGSEGV/SIGABRT -- but a short
// interval is the belt to those braces.
...
void zone_audit_emit(const std::string & line) {
    // WARN plus a raw stderr copy.  GGML_LOG_INFO is dropped at default
    // verbosity in EVERY tool (common_get_verbosity maps INFO to TRACE=4 against
    // a threshold of 3), so an INFO-level audit line yields an empty capture --
...
void zone_audit_install_atexit();
...
void zone_audit_report_timing_locked(const char *                    kind,
                                     const char *                    zone_name,
                                     const zone_audit_zone_timing &  t) {
    ...
    zone_audit_emit(zone_audit_format(
        ...
        kind, zone_name, (unsigned long long) ac,
        t.alloc_bytes.load(std::memory_order_relaxed) / (1024.0 * 1024.0),
...
void zone_audit_report_locked(const char * where) {
    ...
    if (st.sites.empty()) {
        ...
        zone_audit_emit(std::string("[ZONE-RESET-AUDIT] NO RESET SITE WAS VISITED -- this run proves NOTHING about "
    ...
    for (const auto & site_kv : st.sites) {
        ...
        if (s.largest_free_valid) {
            ...
            zone_audit_emit(zone_audit_format(
                "[ZONE-RESET-AUDIT]   largest_free first=%.2f MB last=%.2f MB min=%.2f MB\n",
        ...
        for (const auto & cohort_kv : s.cross_tab) {
            for (const auto & size_kv : cohort_kv.second) {
                ...
                // time = reallocated per graph = retention candidate (c-c1n3:
                // the fastest allocation is the one not made).  Same alloc_id
...
void zone_audit_fatal_signal_handler(int sig) {
    ...
    if (st.mutex.try_lock()) {
        ...
        fputs("[ZONE-RESET-AUDIT] fatal signal with audit lock held; inventory NOT flushed -- use the "
...
void zone_audit_install_atexit() {
    zone_audit_state & st       = zone_audit();
    bool               expected = false;
    if (st.atexit_installed.compare_exchange_strong(expected, true, std::memory_order_relaxed)) {
        std::atexit([] {
...
void * unified_cache::ensure_cached(const ggml_sycl_cache_id & key_id,
    ...
    if (has_copy_event) {
        ...
        // the producer event separately.  Older direct mirrors exposed the same
        // pointer before the copy completed; keeping state READY preserves that
        // contract while allowing mem_handle users to acquire a real lease.
...
void unified_cache::begin_planned_materialization(const char * reason) {
    const int old_depth = planned_materialization_depth_.fetch_add(1, std::memory_order_acq_rel);
    if (old_depth == 0 && g_ggml_sycl_debug) {
        GGML_LOG_INFO("[UNIFIED-CACHE] planned materialization begin: %s\n", reason ? reason : "?");
    }
}

void unified_cache::end_planned_materialization() {
    int old_depth = planned_materialization_depth_.load(std::memory_order_acquire);
    while (old_depth > 0) {
        if (planned_materialization_depth_.compare_exchange_weak(old_depth, old_depth - 1, std::memory_order_acq_rel,
                                                                 std::memory_order_acquire)) {
            return;
        }
    }
    GGML_LOG_WARN("[UNIFIED-CACHE] planned materialization end called without matching begin\n");
}

bool unified_cache::planned_materialization_active() const {
...
bool unified_cache::planned_materialization_allowed(const char *               op,
    ...
    static std::atomic<int> rejected_logs{ 0 };
    if (rejected_logs.fetch_add(1, std::memory_order_relaxed) < 64) {
        GGML_LOG_ERROR(
            "[UNIFIED-CACHE] planned-mode runtime materialization rejected op=%s caller=%s model=%llu "
            "name_hash=0x%llx aux=%llu type=%d ne=[%lld,%lld,%lld,%lld] layout=%d graph_active=%d\n",
            op ? op : "?", caller ? caller : "?", (unsigned long long) key.model_id, (unsigned long long) key.name_hash,
            (unsigned long long) key.aux_id, (int) key.type, (long long) key.ne[0], (long long) key.ne[1],
            (long long) key.ne[2], (long long) key.ne[3], (int) layout,
            unified_cache_is_graph_compute_active() ? 1 : 0);
    }
    if (cache_assert_enabled()) {
        GGML_ABORT("planned-mode runtime materialization attempted through unified_cache direct staging");
    }
    return false;
}
...
static bool can_replace_cache_entry_locked(const unified_cache_key &   key,
    ...
    GGML_LOG_WARN(
        ...
        path ? path : "replace", (int) key.type, (unsigned long long) key.id.model_id,
...
void unified_cache::remap_or_erase_id_mapping_locked(const ggml_sycl_cache_id & id,
    ...
    auto mapped = id_to_key_.find(id);
    if (mapped == id_to_key_.end() || !(mapped->second == removed_key)) {
        return;
    }

    unified_cache_key replacement{};
    bool              have_replacement  = false;
    auto              replacement_score = [](const unified_cache_entry & entry) {
        int score = 0;
        if (!entry.retired) {
            score += 1000;
        }
        ...
        if (entry.location == cache_location::DEVICE) {
            score += 20;
        } else if (entry.location == cache_location::HOST_PINNED) {
            score += 10;
        }
    ...
    for (const auto & pair : entries_) {
        const unified_cache_key &   candidate_key = pair.first;
        const unified_cache_entry & candidate     = pair.second;
        if (candidate_key == removed_key || candidate.retired || !detail::cache_id_equal(candidate_key.id, id)) {
            continue;
        }
        const int score = replacement_score(candidate);
        if (!have_replacement || score > best_score) {
            replacement      = candidate_key;
            best_score       = score;
...
direct_stage_result unified_cache::direct_stage_weight(ggml_sycl_cache_id   key,
    ...
    if (direct_handle) {
        ...
        direct_weight_entries_[key]  = std::move(direct_entry);
    }
    ...
    if (!ptr) {
        if (has_placement_plan()) {
            ...
            if (planned_stage_fail_log.fetch_add(1, std::memory_order_relaxed) < 10) {
                ...
                GGML_LOG_WARN(
                    "[DIRECT-STAGE] WEIGHT zone full (%zu bytes) for planned expert layout=%d; refusing unplanned host "
    ...
    direct_entries.reserve(expert_count);
    {
        ...
        for (size_t i = 0; i < expert_count; ++i) {
            ...
            if (live != 0 || old->second.retired) {
                GGML_LOG_WARN(
                    "[DIRECT-STAGE] refusing tensor-bulk expert replacement with live/retired entry model=%llu "
...
bool unified_cache::drop_expert_entry(ggml_sycl_cache_id key, const char * reason) {
    if (!key.valid) {
        return false;
    }
    ...
    }
    {
        std::unique_lock<std::shared_mutex> lock(rw_mutex_);
        auto                                mapped = id_to_key_.find(key);
        if (mapped != id_to_key_.end() && entries_.find(mapped->second) == entries_.end()) {
            remap_or_erase_id_mapping_locked(key, mapped->second);
        }
    }
...
size_t unified_cache::drop_expert_entries_for_tensor_layout(const std::vector<ggml_sycl_cache_id> & expert_keys,
    ...
    // Phase 2: drop only entries whose refcount has reached zero. A live entry
    // is still owned by a mem_handle somewhere; hiding it here would make that
    // valid handle stop resolving and corrupt phase materialization.
    size_t dropped           = 0;
    ...
    if ((dropped > 0 || skipped_in_use > 0 || skipped_in_flight > 0 || !expert_keys.empty()) &&
        moe_direct_trace_enabled()) {
        fprintf(stderr,
                "[MOE-DROP] tensor-layout keys=%zu layout=%d dropped=%zu skipped_in_use=%zu skipped_in_flight=%zu "
                "reason=%s avail=%.1fMB largest=%.1fMB\n",
                expert_keys.size(), (int) layout, dropped, skipped_in_use, skipped_in_flight, reason ? reason : "",
                zone_available(vram_zone_id::WEIGHT) / (1024.0 * 1024.0),
                zone_largest_free(vram_zone_id::WEIGHT) / (1024.0 * 1024.0));
    }
    return dropped;
...
bool unified_cache::is_cached(const ggml_sycl_cache_id & key_id, ggml_layout_mode layout) const {
    ...
    if (entry_it->second.retired || entry_it->second.layout != layout) {
        return false;
    }
    return true;
}
...
bool unified_cache::is_cached_any(const ggml_sycl_cache_id & key_id) const {
    ...
    std::shared_lock<std::shared_mutex> lock(rw_mutex_);
    for (const auto & pair : entries_) {
        if (pair.second.retired || !detail::cache_id_equal(pair.first.id, key_id)) {
            continue;
        }
        return true;
    }
    ...
    auto entry_it = entries_.find(id_it->second);
    return entry_it != entries_.end() && !entry_it->second.retired;
}
...
void unified_cache::register_ready(const ggml_sycl_cache_id & key,
    ...
    if (it == entries_.end()) {
        auto id_it = id_to_key_.find(key);
        if (id_it != id_to_key_.end()) {
            auto mapped_it = entries_.find(id_it->second);
            if (mapped_it != entries_.end() && mapped_it->second.device_ptr == device_ptr &&
                mapped_it->second.layout == layout && mapped_it->second.type == type) {
                it = mapped_it;
            }
        }
    }
...
void * unified_cache::lookup_device_only(const ggml_sycl_cache_id & key, ggml_layout_mode layout) {
    ...
    if (entry_it == entries_.end()) {
        entry_it = entries_.find(direct_expert_key);
    }
    if (entry_it == entries_.end()) {
        auto id_it = id_to_key_.find(key);
        if (id_it == id_to_key_.end()) {
...
unified_cache::weight_ptr_result unified_cache::get_weight_ptr(const ggml_sycl_cache_id & key) {
    ...
    // Direct maps are derived mirrors for legacy lookup/diagnostics.  They are
    // not ownership records and cannot satisfy smart-handle resolution without
    // the canonical refcounted entries_ record created by staging/register.
    {
        std::shared_lock<std::shared_mutex> dlock(direct_stage_mutex_);
        auto                                it = direct_weight_entries_.find(key);
        if (it != direct_weight_entries_.end() && it->second.ptr) {
            static std::atomic<int> direct_weight_mirror_miss_log{ 0 };
            if (direct_weight_mirror_miss_log.fetch_add(1, std::memory_order_relaxed) < 10) {
                GGML_LOG_WARN(
                    "[DIRECT-STAGE] refusing unowned dense direct mirror model=%llu name_hash=0x%llx layout=%d; "
                    "canonical unified-cache entry is missing\n",
                    (unsigned long long) key.model_id, (unsigned long long) key.name_hash, (int) it->second.layout);
            }
        }
    }
    return result;
...
unified_cache::weight_ptr_lease_result unified_cache::acquire_entry_lease(const unified_cache_key & key) {
    ...
    // Direct mirrors are not lease roots.  If one exists without a canonical
    // entry, fail resolution and let the caller surface the planner/materializer
    // gap instead of returning an unrefcounted pointer.
    {
        std::shared_lock<std::shared_mutex> dlock(direct_stage_mutex_);
        const auto &                        entries =
        ...
        if (it != entries.end() && it->second.ptr) {
            static std::atomic<int> direct_mirror_unleased_log{ 0 };
            if (direct_mirror_unleased_log.fetch_add(1, std::memory_order_relaxed) < 10) {
                GGML_LOG_WARN(
                    "[DIRECT-STAGE] refusing unleased direct mirror type=%d model=%llu name_hash=0x%llx layout=%d; "
                    "canonical unified-cache entry is missing\n",
                    (int) key.type, (unsigned long long) key.id.model_id, (unsigned long long) key.id.name_hash,
...
cache_ptr_view unified_cache::get_view(const ggml_sycl_cache_id & key_id, ggml_layout_mode layout) {
    ...
    if (entry.has_ready_event) {
        if (event_complete(entry.ready_event)) {
            entry.has_ready_event = false;
        } else {
            return view;
        }
...
size_t unified_cache::evict_one(size_t /* new_size */) {
    ...
    if (it != entries_.end()) {
        ...
        // Remove from lookup only if it still names the entry being evicted.
        remap_or_erase_id_mapping_locked(evict_key.id, evict_key);

        {
            std::unique_lock<std::shared_mutex> direct_lock(direct_stage_mutex_);
            if (evict_key.type == cache_entry_type::MOE_EXPERT) {
                direct_expert_entries_.erase(evict_key.id);
            } else if (evict_key.type == cache_entry_type::DENSE_WEIGHT) {
                direct_weight_entries_.erase(evict_key.id);
            }
        }
...
size_t unified_cache::finalize_evictions_locked() {
    ...
    for (auto & pair : entries_) {
        ...
        if (live != 0) {
            GGML_LOG_WARN(
                "[UNIFIED-CACHE] async eviction finalize deferred: model=%llu name_hash=0x%llx layout=%d "
                "in_use=%u\n",
                (unsigned long long) pair.first.id.model_id, (unsigned long long) pair.first.id.name_hash,
...
size_t unified_cache::reclaim_weight_entries(weight_reclaim_mode mode, uint32_t slot) {
    ...
    // inventory needs is the per-entry size/attribution breakdown.
    std::vector<zone_audit_live_entry> audit_preserved;
    const bool                         audit_weights = zone_reset_audit_enabled();
    ...
    }
    {
        ...
        for (auto it = entries_.begin(); it != entries_.end();) {
            ...
            if (!weight_entry_reclaimable(entry, mode, live_mask)) {
                entries_preserved++;
                if (audit_weights) {
                    // Cohort splits the three reasons an entry survives, because
                    // they are three different tickets: a lease with no live
                    // owner is a leak, a live owner is correct, and an untagged
                    // entry is an attribution gap.
                    const char * why = (live != 0 && !owned_by_live && !unattributed) ? "weight:leaked_lease" :
                                       ...
                                       owned_by_live                                  ? "weight:owned_by_live_model" :
                                                                                        "weight:unattributed";
                    audit_preserved.push_back({ (uint64_t) it->first.id.name_hash, entry.size,
    ...
    if (audit_weights) {
        zone_audit_record_site("weight-reclaim", weight_reclaim_mode_name(mode),
                               ggml_sycl_get_device_id_from_queue(queue_), zone_largest_free(vram_zone_id::WEIGHT),
                               true, audit_preserved);
    }
...
bool unified_cache::validate() const {
    std::unique_lock<std::shared_mutex>                                                      lock(rw_mutex_);
    bool                                                                                     ok = true;
    std::unordered_set<ggml_sycl_cache_id, detail::cache_id_hash, detail::cache_id_equal_fn> live_ids;

    for (const auto & pair : entries_) {
        const auto & key   = pair.first;
        const auto & entry = pair.second;
        if (!entry.retired) {
            live_ids.insert(key.id);
        }
    ...
    for (const auto & pair : id_to_key_) {
        ...
        if (it->second.retired || !detail::cache_id_equal(it->first.id, pair.first)) {
            GGML_SYCL_DEBUG("[UNIFIED-CACHE] validate: invalid id_to_key target model=%llu name_hash=0x%llx\n",
                            (unsigned long long) pair.first.model_id, (unsigned long long) pair.first.name_hash);
            ok = false;
        }
    }

    for (const ggml_sycl_cache_id & id : live_ids) {
        auto it = id_to_key_.find(id);
        if (it == id_to_key_.end()) {
            GGML_SYCL_DEBUG(
                "[UNIFIED-CACHE] validate: missing id_to_key for live entries model=%llu name_hash=0x%llx\n",
                (unsigned long long) id.model_id, (unsigned long long) id.name_hash);
            ok = false;
            continue;
        }
        auto entry_it = entries_.find(it->second);
        if (entry_it == entries_.end() || entry_it->second.retired || !detail::cache_id_equal(entry_it->first.id, id)) {
            GGML_SYCL_DEBUG(
                "[UNIFIED-CACHE] validate: id_to_key does not name a live sibling model=%llu name_hash=0x%llx\n",
                (unsigned long long) id.model_id, (unsigned long long) id.name_hash);
            ok = false;
        }
    }
...
//   - Each visible GPU gets its own sycl::context containing only that device.
//     Device 0 is not special here; it can be primary or secondary depending
//     on the planner/executor route being submitted.
...
void init_shared_context_queues(int total_gpus) {
    ...
    if (g_shared_ctx_queues_initialized && total_gpus <= g_shared_ctx_queues_initialized_for) {
    ...
    g_shared_ctx_queues_initialized_for = std::max(g_shared_ctx_queues_initialized_for, total_gpus);
    if (n_created > 0) {
        GGML_LOG_INFO("[PER-DEV-QUEUE] %d secondary queues created with per-device contexts\n", n_created);
    } else if (n_available > 0) {
        GGML_LOG_INFO("[PER-DEV-QUEUE] %d secondary queues already available with per-device contexts\n", n_available);
    }
}
...
// Uses gpu_dpct_ids[] (pre-scheduler-hiding GPU map) first so secondary devices
// hidden from the scheduler still resolve to the logical GPU IDs used by handles.
static int get_device_id_from_queue(sycl::queue & queue) {
    try {
        ...
        // Fallback: check all dpct devices through the process-wide logical map.
...
static void query_device_memory_no_info(int device_id, sycl::queue & queue, size_t & free_mem, size_t & total_mem) {
    free_mem  = 0;
    total_mem = 0;

    try {
        ggml_sycl_get_device(device_id).get_memory_info(free_mem, total_mem);
    } catch (const sycl::exception & e) {
        GGML_LOG_WARN("[UNIFIED-CACHE] Direct memory query failed for device %d: %s\n", device_id, e.what());
    } catch (const std::exception & e) {
        GGML_LOG_WARN("[UNIFIED-CACHE] Direct memory query failed for device %d: %s\n", device_id, e.what());
    }
    ...
    if (total_mem == 0) {
        try {
            total_mem = queue.get_device().get_info<sycl::info::device::global_mem_size>();
        } catch (const sycl::exception & e) {
            GGML_LOG_WARN("[UNIFIED-CACHE] global_mem_size query failed for device %d: %s\n", device_id, e.what());
        }
    }
    if (free_mem == 0) {
        free_mem = total_mem;
...
static std::string query_device_name_no_info(int device_id, sycl::queue & queue) {
    try {
        return queue.get_device().get_info<sycl::info::device::name>();
    } catch (const sycl::exception & e) {
        GGML_LOG_WARN("[UNIFIED-CACHE] Device name query failed for device %d: %s\n", device_id, e.what());
    }
    return "unknown";
}
...
static unified_cache * create_cache_for_device(int                     device_id,
    ...
    if (budget == 0) {
        ...
        if (hint && hint->total_mem > 0) {
            ...
            // Do not call ggml_backend_sycl_get_device_memory() here: cache
            // creation holds g_cache_rw_mutex and may run while ggml_sycl_info()
            // is still in static initialization.  Query the queue/device
            // directly so this path stays independent of ggml_sycl_info().
            query_device_memory_no_info(device_id, queue, free_mem, total_mem);
        }
    ...
    // Total VRAM for arena_reserve's runtime-headroom calculation.  Use the
    // same direct query as budget calculation; the public backend memory helper
    // dereferences ggml_sycl_info() and can re-enter static initialization.
    size_t       total_vram_for_ctor = hint ? hint->total_mem : size_t(0);
    if (total_vram_for_ctor == 0) {
        if (detected_total_mem == 0) {
            query_device_memory_no_info(device_id, queue, detected_free_mem, detected_total_mem);
        }
        total_vram_for_ctor = detected_total_mem;
    }
    try {
...
[[maybe_unused]] static void runtime_reset_reclaimed_note_locked(const runtime_alloc_record & rec) {
    ...
    g_runtime_reset_reclaimed_allocs[rec.handle.alloc_id] = {
        rec.handle.ptr,  rec.handle.size,      rec.handle.device,    rec.handle.tier,
        rec.handle.role, rec.handle.vram_zone, rec.handle.host_zone, rec.handle.zone_managed,
    };
}
...
static bool runtime_reset_reclaimed_matches_locked(const alloc_handle & handle) {
    ...
    const bool matches = rec.ptr == handle.ptr && rec.size == handle.size && rec.device == handle.device &&
                         rec.tier == handle.tier && rec.role == handle.role && rec.vram_zone == handle.vram_zone &&
                         rec.host_zone == handle.host_zone && rec.zone_managed == handle.zone_managed;
    if (matches) {
        if (unified_alloc_strict_mode()) {
            GGML_LOG_ERROR(
                "[UNIFIED-ALLOC] strict reset-reclaimed handle free ptr=%p alloc_id=%llu size=%zu device=%d "
                "tier=%s role=%d vram_zone=%d host_zone=%d\n",
                handle.ptr, static_cast<unsigned long long>(handle.alloc_id), handle.size, handle.device,
                alloc_tier_name(handle.tier), static_cast<int>(handle.role), static_cast<int>(handle.vram_zone),
                static_cast<int>(handle.host_zone));
            return false;
        }
...
bool unified_alloc(const alloc_request & req_in, alloc_handle * out) {
    ...
    *out = {};
    ...
    if (!ptr && (tier == alloc_tier::HOST_PINNED || kv_spill_to_host)) {
        ...
        if (!ptr && ucache) {
            // Route to the correct host zone based on lifetime. KV spills go to
            // KV; permanent weight data, persistent host-compute owners, and
            // expert-cache pools go to WEIGHT so graph-boundary STAGING/SCRATCH
...

    {
        ...
        if (it == g_runtime_alloc_registry.end()) {
            if (runtime_reset_reclaimed_matches_locked(handle)) {
                if (unified_alloc_lifetime_trace_enabled()) {
                    GGML_LOG_WARN(
                        "[UNIFIED-ALLOC-LIFE] handle-free-reset-reclaimed id=%llu ptr=%p size=%zu device=%d "
                        "tier=%s role=%d vram_zone=%d host_zone=%d\n",
            ...
            if (unified_alloc_lifetime_trace_enabled()) {
                GGML_LOG_WARN(
                    "[UNIFIED-ALLOC-LIFE] handle-free-miss id=%llu ptr=%p size=%zu device=%d tier=%s role=%d "
                    "category=%d vram_zone=%d host_zone=%d\n",
                    (unsigned long long) handle.alloc_id, handle.ptr, handle.size, handle.device,
                    alloc_tier_name(handle.tier), (int) handle.role, (int) handle.category, (int) handle.vram_zone,
                    (int) handle.host_zone);
            }
            if (unified_alloc_strict_mode()) {
        ...
        if (handle.alloc_id != 0 && it->second.handle.alloc_id != handle.alloc_id) {
            if (runtime_reset_reclaimed_matches_locked(handle)) {
                if (unified_alloc_lifetime_trace_enabled()) {
                    GGML_LOG_WARN(
                        "[UNIFIED-ALLOC-LIFE] handle-free-reused-reset-reclaimed id=%llu ptr=%p size=%zu "
...
bool unified_cache::reserve_pp_moe_onednn_scratch(size_t   weight_slot_bytes,
    ...
    if (!ok) {
        ...
        GGML_LOG_WARN(
            "[UNIFIED-CACHE] PP MoE oneDNN scratch ring allocation failed "
            "(depth=%u weight=%.1f MB activation=%.1f MB output=%.1f MB)\n",
            ring_depth, weight_slot_bytes / (1024.0 * 1024.0), activation_slot_bytes / (1024.0 * 1024.0),
...
bool unified_cache::get_pp_moe_onednn_scratch_slot(uint32_t slot, pp_moe_onednn_scratch_slot & out) {
    ...
    const pp_moe_onednn_scratch_slot & reserved = pp_moe_onednn_scratch_slots_[slot];
...
pp_moe_onednn_scratch_result unified_cache_get_pp_moe_onednn_scratch_slot(int device_id, uint32_t slot) {
    pp_moe_onednn_scratch_result result;
    unified_cache *              cache = get_unified_cache_for_device(device_id);
    if (!cache) {
        return result;
    }
...
void * unified_cache::load_partial_rows(const char *               tensor_name,
    ...
    GGML_SYCL_DEBUG("[PARTIAL-ROWS] Loaded '%s' device %d: rows [%lld, %lld), %.2f MB SOA\n", tensor_name, device_idx,
                    (long long) row_start, (long long) (row_start + row_count), partial_bytes / (1024.0f * 1024.0f));
...
unified_cache * unified_cache_register_for_queue(int device_id, sycl::queue & queue) {
    ...
    size_t          deferred_reserve = 0;
    {
        ...
        // Hidden/secondary devices must use the same budget and arena policy as
        // primary devices. The explicit queue preserves the isolated per-device
        // context needed to avoid Level Zero multi-device context failures.
        result = create_cache_for_device(device_id, &deferred_reserve, nullptr, &queue);
    }
    if (result && deferred_reserve > 0) {
        result->update_reserved_bytes(deferred_reserve);
    }
...
direct_stage_result unified_cache_direct_stage_weight(int                  device_id,
    ...
    return cache->direct_stage_weight(key, src, src_size, dst_size, layout, fill_fn, fill_ctx, queue, out_handle);
}
...
void unified_cache::host_zone_reset(host_zone_id zone) {
    ...
    // Queried before taking the registry lock: zone_largest_free_block() takes
    // the pinned pool's own mutex, and there is no reason to nest the two.
    const bool   audit_on            = zone_reset_audit_enabled();
    const size_t audit_largest_free  = audit_on ? host_arena_->zone_largest_free_block(zone) : 0;

    {
        ...
        // See the matching note in zone_reset(): the audit is uncapped in what it
        // aggregates because the refusal dump above cannot produce an inventory.
        std::vector<zone_audit_live_entry> audit_live;
        for (auto it = g_runtime_alloc_registry.begin(); it != g_runtime_alloc_registry.end(); ++it) {
            if (it->second.handle.host_zone == zone) {
                ...
                if (audit_on) {
                    const alloc_handle & h = it->second.handle;
                    audit_live.push_back({ h.alloc_id, h.size, static_cast<int>(h.role), static_cast<int>(h.category),
        ...
        if (audit_on) {
            zone_audit_record_site("host-zone-reset", host_zone_name(zone), -1, audit_largest_free, true, audit_live);
        }
...
void unified_cache::reset_scratch_pool() {
    if (zone_reset_audit_enabled()) {
        ...
        // allocations escaped" is not a well-formed question here.  What the
        // audit CAN report is how much the pool is carrying at each reset --
        // which is what a per-handle model would have to hold instead.
        const size_t                       off = scratch_pool_off_.load(std::memory_order_relaxed);
        std::vector<zone_audit_live_entry> live;
        if (off > 0) {
            live.push_back({ 0, off, -1, -1, -1, std::string("scratch_pool:bump_in_use") });
        }
        zone_audit_record_site("scratch-pool-reset", "bump", ggml_sycl_get_device_id_from_queue(queue_),
                               scratch_pool_size_ > off ? scratch_pool_size_ - off : 0, scratch_pool_size_ > 0, live);
        if (zone_reset_audit_suppresses_reset()) {
            return;
        }
    }
...
void unified_cache::zone_reset(vram_zone_id zone) {
    ...
    if (zone == vram_zone_id::KV && arena_chunks_.size() == 1 &&
        arena_zones_[static_cast<int>(vram_zone_id::WEIGHT)].allocator == nullptr) {
        size_t device_weight_entries = 0;
        {
            std::shared_lock<std::shared_mutex> lock(rw_mutex_);
            for (const auto & it : entries_) {
                const unified_cache_entry & entry = it.second;
                if ((entry.type == cache_entry_type::DENSE_WEIGHT || entry.type == cache_entry_type::MOE_EXPERT) &&
                    entry.location == cache_location::DEVICE && entry.device_ptr && vram_owns(entry.device_ptr)) {
                    device_weight_entries++;
                }
            }
        }
        if (device_weight_entries > 0) {
    ...
    if (arena_base_ && z.size > 0) {
        ...
        size_t                      detail_lines     = 0;
        size_t                      live_allocations = 0;
        // Audit collects EVERY live record, uncapped -- the refusal dump above is
        // capped at 4 refusals x 8 lines per zone, which cannot produce an
        // inventory.  Collected here rather than in a second pass so it sees
        // exactly what the refusal saw, under the same lock.
        std::vector<zone_audit_live_entry> audit_live;
        const bool                         audit_on = zone_reset_audit_enabled();
        for (auto it = g_runtime_alloc_registry.begin(); it != g_runtime_alloc_registry.end(); ++it) {
        ...
        if (audit_on) {
            zone_audit_record_site("device-zone-reset", vram_zone_name(zone),
                                   ggml_sycl_get_device_id_from_queue(queue_), zone_largest_free(zone), true,
                                   audit_live);
        }
        if (live_allocations > 0) {
            if (zone_logs.fetch_add(1, std::memory_order_relaxed) < 8) {
...
uint32_t unified_cache::arena_chunk_lease_count(int chunk_idx) const {
    if (chunk_idx < 0 || chunk_idx >= static_cast<int>(arena_chunks_.size())) {
        return 0;
    }
    return arena_chunks_[chunk_idx].lease_count.load();
}
...
static bool placement_priority_is_moe_weight(placement_priority priority) {
    return priority == placement_priority::MOE_GATE || priority == placement_priority::MOE_DOWN ||
           priority == placement_priority::MOE_UP || priority == placement_priority::MOE_GATE_PROJ;
}
...
static void reorder_plan_entries_for_moe_materialization(placement_plan &                       plan,
                                                         const std::vector<moe_triplet_group> & moe_groups) {
    if (moe_groups.empty()) {
        return;
    }

    std::vector<uint8_t>         used(plan.entries.size(), 0);
    std::vector<placement_entry> reordered;
    reordered.reserve(plan.entries.size());

    // Keep dense/non-expert entries in their existing priority order.  MoE expert
    // entries are appended in the same triplet order used for placement so S1
    // model-load materialization stages gate/up/down for the same expert group
    // together instead of role-by-role across the whole model.
    for (size_t i = 0; i < plan.entries.size(); ++i) {
        if (plan.entries[i].expert_id < 0) {
            reordered.push_back(plan.entries[i]);
            used[i] = 1;
        }
    }

    for (const moe_triplet_group & group : moe_groups) {
        std::vector<size_t> indices = group.indices;
        std::sort(indices.begin(), indices.end(), [&](size_t a, size_t b) {
            const placement_entry & ea     = plan.entries[a];
            const placement_entry & eb     = plan.entries[b];
            const int               role_a = moe_materialization_role_order(ea.expert_role);
            const int               role_b = moe_materialization_role_order(eb.expert_role);
            if (role_a != role_b) {
                return role_a < role_b;
            }
            return static_cast<uint8_t>(ea.priority) < static_cast<uint8_t>(eb.priority);
        });

        for (size_t idx : indices) {
            if (idx >= plan.entries.size() || used[idx]) {
                continue;
            }
            reordered.push_back(plan.entries[idx]);
            used[idx] = 1;
        }
    }
    ...
    plan.entries = std::move(reordered);
}
...
static std::vector<placement_tensor_info> placement_tensor_inventory_from_pairs(
    const std::vector<std::pair<std::string, size_t>> & tensor_inventory) {
    std::vector<placement_tensor_info> converted;
    converted.reserve(tensor_inventory.size());
    for (const auto & item : tensor_inventory) {
        converted.emplace_back(item.first, item.second);
    }
    return converted;
}

static void copy_tensor_info_to_entry(placement_entry & entry, const placement_tensor_info & tensor) {
...
static size_t planner_layout_bytes_coalesced_for_dims(ggml_type type, int64_t ncols, int64_t nrows) {
    if (ncols <= 0 || nrows <= 0) {
        return 0;
    }
    ...
    switch (type) {
        case GGML_TYPE_Q4_0:
            return static_cast<size_t>(nrows) * static_cast<size_t>(ncols / 2) + nblocks * sizeof(ggml_half);
        case GGML_TYPE_Q8_0:
        ...
        case GGML_TYPE_MXFP4:
            return static_cast<size_t>(nrows) * static_cast<size_t>(ncols / 2) + nblocks;
        default:
            break;
    }
    return 0;
...
static bool planner_mxfp4_i8_supported(const placement_entry & entry, int device_id) {
    if (entry.type != GGML_TYPE_MXFP4 || entry.ne[0] <= 0 || entry.ne[1] <= 0 || (entry.ne[0] % QK_MXFP4) != 0) {
        return false;
    }
    if (device_id < 0 || device_id >= ggml_sycl_info().device_count) {
        return false;
...
// Mirrors ggml_sycl_moe_prompt_down_specialized_layouts_enabled(): prompt down
// MXFP4_I8/DPAS is still opt-in, so default plans must budget SOA for PP.
...
static bool planner_xmx_tiled_moe_enabled() {
    ...
    env = std::getenv("GGML_SYCL_XMX_MOE");
...
static size_t planner_layout_bytes_xmx_tiled_for_dims(ggml_type type, int64_t ncols, int64_t nrows, int device_id) {
    ...
    const auto & caps = ggml_sycl_info().devices[device_id].xmx_caps;
    if (!caps.supported || !caps.supports_int8 || caps.K == 0 || caps.N == 0 || caps.optimal_tiles_n <= 0 ||
        (ncols % static_cast<int64_t>(caps.K)) != 0 || (caps.K % 2) != 0) {
        return 0;
    }
...
static bool planner_mxfp4_xmx_tiled_supported(const placement_tensor_info & tensor,
                                              ...
                                              int                           device_id) {
    if (!planner_xmx_tiled_moe_enabled() || tensor.type != GGML_TYPE_MXFP4 ||
        (role != expert_tensor_role::GATE && role != expert_tensor_role::UP) || !tensor.has_shape() || device_id < 0 ||
        device_id >= ggml_sycl_info().device_count) {
        return false;
    }
    ...
    const auto & caps = ggml_sycl_info().devices[device_id].xmx_caps;
    const auto   policy =
        ggml_sycl_make_moe_device_policy(caps, device_id, /*total_vram=*/0, /*free_vram_at_init=*/0,
                                         /*max_alloc_size=*/0, /*safe_max_alloc_size=*/0, /*vram_budget=*/0,
                                         /*weight_budget=*/0, /*arena_total=*/0, /*arena_scratch=*/0,
                                         /*arena_runtime=*/0, /*arena_onednn=*/0, tensor.ne[0], tensor.ne[1],
                                         tensor.ne[2] > 0 ? tensor.ne[2] : 1, /*device_resident=*/true,
                                         /*tiled_kernel_validated=*/true);
    return policy.mxfp4_device_layout.layout == GGML_LAYOUT_XMX_TILED &&
           planner_layout_bytes_xmx_tiled_for_dims(tensor.type, tensor.ne[0], tensor.ne[1], device_id) != 0;
}

static bool planner_mxfp4_xmx_tiled_supported(const placement_entry & entry, int device_id) {
    if (!planner_xmx_tiled_moe_enabled() || entry.type != GGML_TYPE_MXFP4 ||
        (entry.expert_role != expert_tensor_role::GATE && entry.expert_role != expert_tensor_role::UP) ||
        entry.ne[0] <= 0 || entry.ne[1] <= 0 || device_id < 0 || device_id >= ggml_sycl_info().device_count) {
        return false;
    }
    ...
    const auto   policy =
        ggml_sycl_make_moe_device_policy(caps, device_id, /*total_vram=*/0, /*free_vram_at_init=*/0,
                                         /*max_alloc_size=*/0, /*safe_max_alloc_size=*/0, /*vram_budget=*/0,
                                         /*weight_budget=*/0, /*arena_total=*/0, /*arena_scratch=*/0,
                                         /*arena_runtime=*/0, /*arena_onednn=*/0, entry.ne[0], entry.ne[1],
                                         entry.ne[2] > 0 ? entry.ne[2] : 1, /*device_resident=*/true,
                                         /*tiled_kernel_validated=*/true);
    return policy.mxfp4_device_layout.layout == GGML_LAYOUT_XMX_TILED &&
...
static size_t planner_layout_bytes_for_dims(ggml_type        type,
    ...
    if (ncols <= 0 || nrows <= 0 || type < 0 || type >= GGML_TYPE_COUNT) {
        return fallback_bytes;
    }
    switch (layout) {
        case GGML_LAYOUT_COALESCED:
            {
                const size_t bytes = planner_layout_bytes_coalesced_for_dims(type, ncols, nrows);
                return bytes != 0 ? bytes : fallback_bytes;
            }
        case GGML_LAYOUT_MXFP4_I8:
            {
                if (type != GGML_TYPE_MXFP4 || (ncols % QK_MXFP4) != 0) {
                    return fallback_bytes;
                }
...
static size_t planner_layout_bytes_for_expert(const placement_tensor_info & tensor,
                                              int                           n_experts,
                                              ggml_layout_mode              layout,
                                              int                           device_id,
                                              size_t                        fallback_bytes) {
    if (!tensor.has_shape() || n_experts <= 0) {
        return fallback_bytes;
    }
...
static ggml_layout_mode planner_default_device_layout(const placement_entry & entry, int device_id) {
    if (entry.expert_id >= 0) {
        ...
        const ggml_layout_mode layout =
            layout_policy::get_optimal(entry.type, tensor_usage::MOE_EXPERT_WEIGHT, device_id);
        if (layout == GGML_LAYOUT_AOS || layout == GGML_LAYOUT_SOA || layout == GGML_LAYOUT_COALESCED) {
            return layout;
        }
        return GGML_LAYOUT_SOA;
...
static bool planner_moe_multi_device_executor_supports_layout(const placement_tensor_info & tensor,
                                                              ggml_layout_mode              layout) {
    if (tensor.type == GGML_TYPE_MXFP4 || tensor.type == GGML_TYPE_Q4_0 || tensor.type == GGML_TYPE_Q6_K) {
        // Secondary-device MoE dispatch currently consumes compact SOA groups.
        // Primary/local entries are handled by planner_moe_primary_executor_supports_layout()
        // below and must keep their own capability-selected layouts.
        return layout == GGML_LAYOUT_SOA;
    }
    return false;
}

static bool planner_moe_multi_device_executor_supports_layout(const placement_entry & entry, ggml_layout_mode layout) {
    if (entry.type == GGML_TYPE_MXFP4 || entry.type == GGML_TYPE_Q4_0 || entry.type == GGML_TYPE_Q6_K) {
        return layout == GGML_LAYOUT_SOA;
    }
    return false;
}

static bool planner_moe_static_secondary_route_supports_prompt(const placement_entry & entry, ggml_layout_mode layout) {
    GGML_UNUSED(entry);
    GGML_UNUSED(layout);
    // Static model-load placement is used by both prompt and decode.  The
    // runtime currently validates secondary SOA for decode route groups only;
    // prompt secondary routes must remain primary/host until the PP secondary
    // executor has an end-to-end numeric equivalence gate.
    return false;
}

static bool planner_moe_static_secondary_route_supported(const placement_entry & entry, ggml_layout_mode layout) {
    return planner_moe_multi_device_executor_supports_layout(entry, layout) &&
           planner_moe_static_secondary_route_supports_prompt(entry, layout);
}

static ggml_layout_mode planner_multi_device_moe_common_layout(const placement_tensor_info & tensor,
                                                               int                           primary_device_id) {
    const ggml_layout_mode preferred =
        planner_default_device_layout(tensor, tensor_usage::MOE_EXPERT_WEIGHT, primary_device_id);
    if (planner_moe_multi_device_executor_supports_layout(tensor, preferred)) {
        return preferred;
    }
    if (planner_moe_multi_device_executor_supports_layout(tensor, GGML_LAYOUT_SOA)) {
        return GGML_LAYOUT_SOA;
    }
    return GGML_LAYOUT_AOS;
}

static size_t planner_layout_bytes_for_entry(const placement_entry & entry, ggml_layout_mode layout, int device_id) {
    if (layout == GGML_LAYOUT_XMX_TILED) {
...
static bool planner_moe_primary_executor_supports_layout_on_device(const placement_entry & entry,
                                                                   ggml_layout_mode        layout,
                                                                   int                     device_id);
static bool planner_moe_primary_executor_supports_layout(const placement_entry & entry,
                                                         ggml_layout_mode        layout,
                                                         int                     device_id);
...
static ggml_layout_mode planner_multi_device_moe_layout_for_target(const placement_entry & entry,
    ...
    if (device_id == primary_device_id &&
        planner_moe_primary_executor_supports_layout_on_device(entry, preferred, device_id)) {
        return preferred;
    }
    if (device_id == primary_device_id &&
        planner_moe_primary_executor_supports_layout_on_device(entry, GGML_LAYOUT_SOA, device_id)) {
        return GGML_LAYOUT_SOA;
    }

    if (planner_moe_multi_device_executor_supports_layout(entry, preferred)) {
        return preferred;
    }
...
static bool planner_moe_primary_executor_supports_layout(const placement_entry & entry,
    ...
    return planner_moe_primary_executor_supports_layout_on_device(entry, layout, device_id);
}

static bool planner_entry_has_alternate_layout_on_device(const placement_entry & entry,
                                                         ggml_layout_mode        layout,
                                                         int                     target_device) {
    for (const placement_alternate_layout & alt : entry.alternate_layouts) {
        const int alt_target = alt.target_device >= 0 ? alt.target_device : entry.target_device;
        if (alt.layout == layout && alt_target == target_device) {
            return true;
        }
    }
    return false;
...
static bool planner_entry_add_alternate_layout_on_device(placement_entry & entry,
                                                         ggml_layout_mode  layout,
                                                         int               target_device,
                                                         size_t            dst_size,
                                                         size_t            vram_charge_size) {
    if (planner_entry_has_alternate_layout_on_device(entry, layout, target_device)) {
        return false;
    }

    placement_alternate_layout alt;
    alt.layout           = layout;
    alt.dst_size         = dst_size;
    alt.vram_charge_size = vram_charge_size != 0 ? vram_charge_size : placement_vram_charge_bytes(dst_size);
    alt.target_device    = target_device == entry.target_device ? -1 : target_device;
...
static size_t planner_expected_moe_pp_rows(const placement_kv_info &            kv_info,
                                           const ggml_sycl_placement_envelope * envelope) {
    const size_t pp_tokens = static_cast<size_t>(planner_effective_ubatch_for_moe_pp(kv_info, envelope));
    const size_t top_k     = static_cast<size_t>(std::max(kv_info.n_expert_used, 1));
...
static bool planner_moe_layout_needs_pp_soa_on_device(const placement_entry &              entry,
                                                      ...
                                                      const placement_kv_info &            kv_info,
                                                      const ggml_sycl_placement_envelope * envelope) {
    if (entry.expert_id < 0 || entry.type != GGML_TYPE_MXFP4 || layout == GGML_LAYOUT_SOA ||
        (entry.expert_role != expert_tensor_role::GATE && entry.expert_role != expert_tensor_role::UP &&
    ...
    if (layout == GGML_LAYOUT_MXFP4_I8) {
        return entry.expert_role == expert_tensor_role::DOWN && !planner_moe_prompt_down_specialized_layouts_enabled();
    }

    return layout == GGML_LAYOUT_MXFP4_DPAS || layout == GGML_LAYOUT_COALESCED;
}
...
static bool planner_moe_layout_needs_pp_soa(const placement_entry &              entry,
    ...
    return planner_moe_layout_needs_pp_soa_on_device(entry, layout, device_id, kv_info, envelope);
}
...
static size_t planner_moe_pp_soa_charge_for_entry(const placement_entry & entry, int device_id, size_t * out_size) {
    const size_t soa_size = planner_layout_bytes_for_entry(entry, GGML_LAYOUT_SOA, device_id);
    if (out_size != nullptr) {
        *out_size = soa_size;
    }
    return soa_size != 0 ? placement_vram_charge_bytes(soa_size) : 0;
}

static size_t prepare_single_moe_pp_gate_up_primary_layouts_for_pack(placement_plan &                     plan,
                                                                     int                                  device_id,
                                                                     const placement_kv_info &            kv_info,
                                                                     const ggml_sycl_placement_envelope * envelope) {
    if (device_id < 0 || device_id >= ggml_sycl_info().device_count) {
    ...
    for (placement_entry & entry : plan.entries) {
        ...
        size_t       soa_size   = 0;
        const size_t soa_charge = planner_moe_pp_soa_charge_for_entry(entry, device_id, &soa_size);
        if (soa_size == 0 || soa_charge == 0) {
            no_layout_bytes++;
            continue;
        }

        entry.layout           = GGML_LAYOUT_SOA;
        entry.dst_size         = soa_size;
        entry.vram_charge_size = soa_charge;
        promoted++;
    }
    if (promoted > 0) {
...
static size_t planner_moe_group_pp_soa_alternate_charge(const placement_plan &               plan,
                                                        const moe_triplet_group &            group,
                                                        int                                  device_id,
                                                        const placement_kv_info &            kv_info,
                                                        const ggml_sycl_placement_envelope * envelope,
                                                        size_t *                             no_layout_bytes) {
    size_t charge = 0;
...
static size_t add_single_moe_pp_executable_alternates(placement_plan &                     plan,
    ...
    struct pending_tensor {
        std::string              name;
        std::vector<pending_alt> alts;
        size_t                   charge = 0;
    };
    ...
    for (placement_entry & entry : plan.entries) {
        ...
        candidates++;
        if (planner_entry_has_alternate_layout_on_device(entry, GGML_LAYOUT_SOA, device_id)) {
            already_present++;
            continue;
        }

        const size_t alt_size = planner_layout_bytes_for_entry(entry, GGML_LAYOUT_SOA, device_id);
        if (alt_size == 0) {
            no_layout_bytes++;
            continue;
        }
        ...
        auto [it, inserted]     = tensor_index.emplace(entry.name, pending.size());
        if (inserted) {
            pending_tensor tensor;
            tensor.name = entry.name;
            pending.push_back(std::move(tensor));
        }
        pending_tensor & tensor = pending[it->second];
        tensor.alts.push_back({ static_cast<size_t>(&entry - plan.entries.data()), alt_size, alt_charge });
        tensor.charge += alt_charge;
    }

    for (const pending_tensor & tensor : pending) {
        if (charge_budget && remaining < tensor.charge) {
            skipped_capacity += tensor.alts.size();
            continue;
        }

        for (const pending_alt & alt_spec : tensor.alts) {
            placement_entry & entry = plan.entries[alt_spec.idx];

            placement_alternate_layout alt;
            alt.layout           = GGML_LAYOUT_SOA;
            alt.dst_size         = alt_spec.size;
            alt.vram_charge_size = alt_spec.charge;
            alt.target_device    = -1;
            entry.alternate_layouts.push_back(alt);

            charged_bytes += alt_spec.charge;
            added++;
        }

        if (charge_budget) {
            remaining -= tensor.charge;
            plan.weight_vram_bytes += tensor.charge;
            plan.vram_bytes += tensor.charge;
            if (per_device_vram != nullptr) {
                *per_device_vram += tensor.charge;
            }
        }
    }
    ...
    return charge_budget ? charged_bytes : 0;
}
...
static void add_multi_moe_pp_executable_alternates(placement_plan &                     plan,
    ...
    for (placement_entry & entry : plan.entries) {
        ...
        if (!entry.on_device || entry.target_device < 0) {
            skip_not_device++;
        } else if (entry.type != GGML_TYPE_MXFP4) {
            skip_non_mxfp4++;
        } else if (entry.layout == GGML_LAYOUT_SOA) {
            skip_already_soa++;
        } else if (entry.expert_role != expert_tensor_role::GATE && entry.expert_role != expert_tensor_role::UP &&
                   entry.expert_role != expert_tensor_role::DOWN) {
            skip_bad_role++;
        } else if (!planner_moe_primary_executor_supports_layout(entry, GGML_LAYOUT_SOA, entry.target_device)) {
            skip_no_soa++;
        } else if (entry.target_device >= ggml_sycl_info().device_count) {
            skip_no_caps++;
        } else {
            const bool xmx_needs_soa = entry.layout == GGML_LAYOUT_XMX_TILED;
            const bool non_xmx_needs_soa =
        ...
        candidates++;
        if (planner_entry_has_alternate_layout_on_device(entry, GGML_LAYOUT_SOA, entry.target_device)) {
            already_present++;
            continue;
        }
        ...
        const size_t alt_size = planner_layout_bytes_for_entry(entry, GGML_LAYOUT_SOA, entry.target_device);
        if (alt_size == 0) {
            no_layout_bytes++;
            continue;
        }
...
static void add_multi_moe_decode_secondary_alternates(placement_plan &                   plan,
                                                      std::vector<size_t> &              remaining,
                                                      const std::vector<device_budget> & device_budgets,
                                                      const std::vector<double> &        moe_device_scores) {
    if (device_budgets.size() <= 1 || remaining.size() != device_budgets.size()) {
        return;
    }

    std::vector<size_t> moe_indices;
    moe_indices.reserve(plan.entries.size());
    for (size_t i = 0; i < plan.entries.size(); ++i) {
        const placement_entry & entry = plan.entries[i];
        if (entry.expert_id >= 0 || placement_priority_is_moe_weight(entry.priority)) {
            moe_indices.push_back(i);
        }
    }

    const char * hotness_source = nullptr;
    auto         groups         = build_moe_triplet_groups(plan, moe_indices, &hotness_source);
    if (groups.empty()) {
        return;
    }

    std::vector<int> device_order;
    device_order.reserve(device_budgets.size());
    for (size_t d = 0; d < device_budgets.size(); ++d) {
        if (device_budgets[d].dense_supported && device_budgets[d].dense_score > 0.0) {
            device_order.push_back(static_cast<int>(d));
        }
    }
    std::sort(device_order.begin(), device_order.end(), [&](int a, int b) {
        const double score_a =
            static_cast<size_t>(a) < moe_device_scores.size() ? moe_device_scores[static_cast<size_t>(a)] : 0.0;
        const double score_b =
            static_cast<size_t>(b) < moe_device_scores.size() ? moe_device_scores[static_cast<size_t>(b)] : 0.0;
        if (score_a != score_b) {
            return score_a > score_b;
        }
        if (device_budgets[static_cast<size_t>(a)].dense_score != device_budgets[static_cast<size_t>(b)].dense_score) {
            return device_budgets[static_cast<size_t>(a)].dense_score >
                   device_budgets[static_cast<size_t>(b)].dense_score;
        }
        return device_budgets[static_cast<size_t>(a)].device_id < device_budgets[static_cast<size_t>(b)].device_id;
    });

    size_t              considered         = 0;
    size_t              complete           = 0;
    size_t              replicated_groups  = 0;
    size_t              replicated_entries = 0;
    size_t              skipped_incomplete = 0;
    size_t              skipped_host_base  = 0;
    size_t              skipped_support    = 0;
    size_t              skipped_capacity   = 0;
    size_t              already_replicated = 0;
    size_t              charged_bytes      = 0;
    std::vector<size_t> per_device_groups(device_budgets.size(), 0);
    std::vector<size_t> per_device_charge(device_budgets.size(), 0);

    for (const moe_triplet_group & group : groups) {
        considered++;
        ...
        for (size_t idx : group.indices) {
            ...
            if (base_target == -2) {
                base_target = entry.target_device;
            } else if (entry.target_device != base_target) {
                base_valid = false;
                break;
            }
        }
        if (!base_valid || base_target < 0) {
            skipped_host_base++;
            continue;
        }

        bool group_already_replicated = false;
        for (int dev_idx : device_order) {
            if (dev_idx < 0 || static_cast<size_t>(dev_idx) >= device_budgets.size()) {
                continue;
            }
            const int target_device = device_budgets[static_cast<size_t>(dev_idx)].device_id;
            if (target_device == base_target) {
                continue;
            }

            size_t group_charge = 0;
            bool   supported    = true;
            bool   duplicate    = true;
            for (size_t idx : group.indices) {
                ...
                const size_t alt_size = planner_layout_bytes_for_entry(entry, GGML_LAYOUT_SOA, target_device);
                if (alt_size == 0) {
                    supported = false;
                    break;
                }
                group_charge += placement_vram_charge_bytes(alt_size);
                duplicate =
                    duplicate && planner_entry_has_alternate_layout_on_device(entry, GGML_LAYOUT_SOA, target_device);
            }
            if (!supported) {
                skipped_support++;
                continue;
            ...
            if (duplicate) {
                group_already_replicated = true;
                already_replicated++;
                break;
            }
            if (remaining[static_cast<size_t>(dev_idx)] < group_charge) {
                skipped_capacity++;
                continue;
            }
            ...
            for (size_t idx : group.indices) {
                placement_entry & entry = plan.entries[idx];
                if (planner_entry_has_alternate_layout_on_device(entry, GGML_LAYOUT_SOA, target_device)) {
                    continue;
                }
                placement_alternate_layout alt;
                alt.layout           = GGML_LAYOUT_SOA;
                alt.dst_size         = planner_layout_bytes_for_entry(entry, GGML_LAYOUT_SOA, target_device);
                alt.vram_charge_size = placement_vram_charge_bytes(alt.dst_size);
                alt.target_device    = target_device;
                entry.alternate_layouts.push_back(alt);
                replicated_entries++;
            }

            remaining[static_cast<size_t>(dev_idx)] -= group_charge;
            plan.weight_vram_bytes += group_charge;
            plan.vram_bytes += group_charge;
            if (static_cast<size_t>(dev_idx) < plan.per_device_vram.size()) {
                plan.per_device_vram[static_cast<size_t>(dev_idx)] += group_charge;
            }
            per_device_groups[static_cast<size_t>(dev_idx)]++;
            per_device_charge[static_cast<size_t>(dev_idx)] += group_charge;
            charged_bytes += group_charge;
            replicated_groups++;
            group_already_replicated = true;
            break;
        }

        if (!group_already_replicated && device_order.size() <= 1) {
            skipped_support++;
        }
    }

    GGML_LOG_INFO(
        "[PLACEMENT-MOE-MULTI] decode secondary alternates: considered=%zu complete=%zu groups=%zu entries=%zu "
        "already=%zu skipped_incomplete=%zu skipped_host_base=%zu skipped_support=%zu skipped_capacity=%zu "
        "charged=%.1f MB hotness=%s\n",
        considered, complete, replicated_groups, replicated_entries, already_replicated, skipped_incomplete,
        skipped_host_base, skipped_support, skipped_capacity, charged_bytes / (1024.0 * 1024.0),
        hotness_source ? hotness_source : "static(layer,expert)");
    for (size_t d = 0; d < device_budgets.size(); ++d) {
        if (per_device_groups[d] == 0 && per_device_charge[d] == 0) {
            continue;
        }
        GGML_LOG_INFO(
            "[PLACEMENT-MOE-MULTI] decode secondary target logical=%zu physical=%d groups=%zu charged=%.1f MB "
            "remaining=%.1f MB score=%.2f\n",
            d, device_budgets[d].device_id, per_device_groups[d], per_device_charge[d] / (1024.0 * 1024.0),
            remaining[d] / (1024.0 * 1024.0), d < moe_device_scores.size() ? moe_device_scores[d] : 0.0);
    }
}

static void apply_multi_moe_per_target_layouts(placement_plan &                   plan,
                                               std::vector<size_t> &              remaining,
                                               const std::vector<device_budget> & device_budgets,
                                               bool static_primary_xmx_executor_supported = true) {
    if (device_budgets.empty()) {
        return;
    }
    ...
    for (placement_entry & entry : plan.entries) {
        ...
        const bool static_xmx_executor_supported =
            static_primary_xmx_executor_supported &&
        ...
        const size_t old_size = entry.dst_size;
        const size_t new_size = target_layout == GGML_LAYOUT_AOS ?
                                    entry.src_size :
                                    planner_layout_bytes_for_entry(entry, target_layout, entry.target_device);
        if (new_size == 0) {
            continue;
        }
        ...
        if (entry.on_device) {
            ...
            if (new_size > old_size) {
                ...
                plan.weight_host_bytes += extra;
                plan.host_bytes += extra;
                host_delta += static_cast<int64_t>(extra);
            } else if (old_size > new_size) {
                const size_t freed     = old_size - new_size;
                plan.weight_host_bytes = plan.weight_host_bytes > freed ? plan.weight_host_bytes - freed : 0;
                plan.host_bytes        = plan.host_bytes > freed ? plan.host_bytes - freed : 0;
                host_delta -= static_cast<int64_t>(freed);
            }
            entry.vram_charge_size = placement_vram_charge_bytes(new_size);
            host_changed++;
        }

        entry.layout   = target_layout;
    ...
    GGML_LOG_INFO(
        "[PLACEMENT-MOE-MULTI] per-target layouts: device_changed=%zu host_changed=%zu xmx_tiled=%zu "
        "i8_or_dpas=%zu soa=%zu aos=%zu skipped_capacity=%zu device_delta=%.1f MB host_delta=%.1f MB\n",
        device_changed, host_changed, xmx_entries, i8_entries, soa_entries, aos_entries, skipped_capacity,
        static_cast<double>(charged_delta) / (1024.0 * 1024.0), static_cast<double>(host_delta) / (1024.0 * 1024.0));
    if (moe_direct_trace_enabled()) {
        fprintf(stderr,
                "[PLACEMENT-MOE-MULTI-LAYOUT-TRACE] device_changed=%zu host_changed=%zu xmx=%zu i8_or_dpas=%zu "
                "soa=%zu aos=%zu skipped_capacity=%zu device_delta=%.1fMB host_delta=%.1fMB\n",
                device_changed, host_changed, xmx_entries, i8_entries, soa_entries, aos_entries, skipped_capacity,
...
static void maybe_upgrade_multi_moe_primary_layouts(placement_plan & plan,
    ...
    if (device_index >= plan.per_device_vram.size()) {
        return;
    }
    ...
    for (placement_entry & entry : plan.entries) {
        ...
        if (mixed_owner_tensors.find(entry.name) != mixed_owner_tensors.end()) {
            mixed_skipped++;
            continue;
        }

        const ggml_layout_mode target_layout = planner_default_device_layout(entry, device_id);
        if (target_layout == entry.layout ||
            !planner_moe_primary_executor_supports_layout(entry, target_layout, device_id)) {
            continue;
        }

        const size_t new_size = planner_layout_bytes_for_entry(entry, target_layout, device_id);
        if (new_size == 0) {
            continue;
        }
        const size_t old_charge =
        ...
        if (new_charge > old_charge) {
            const size_t extra = new_charge - old_charge;
            if (remaining <= k_layout_upgrade_guard || extra > remaining - k_layout_upgrade_guard) {
                continue;
            }
            remaining -= extra;
            plan.weight_vram_bytes += extra;
            plan.vram_bytes += extra;
            plan.per_device_vram[device_index] += extra;
            charged_bytes += extra;
        } else if (old_charge > new_charge) {
            const size_t freed = old_charge - new_charge;
            remaining += freed;
            plan.weight_vram_bytes = plan.weight_vram_bytes > freed ? plan.weight_vram_bytes - freed : 0;
            plan.vram_bytes        = plan.vram_bytes > freed ? plan.vram_bytes - freed : 0;
            plan.per_device_vram[device_index] =
                plan.per_device_vram[device_index] > freed ? plan.per_device_vram[device_index] - freed : 0;
    ...
    if (upgraded_entries > 0) {
        GGML_LOG_INFO(
            "[PLACEMENT-MOE-MULTI] primary executor layout upgrades: device=%d entries=%zu xmx_tiled=%zu soa=%zu "
            "mixed_skipped=%zu charged=%.1f MB released=%.1f MB remaining=%.1f MB guard=%.1f MB\n",
            device_id, upgraded_entries, xmx_entries, soa_entries, mixed_skipped, charged_bytes / (1024.0 * 1024.0),
            released_bytes / (1024.0 * 1024.0), remaining / (1024.0 * 1024.0),
            k_layout_upgrade_guard / (1024.0 * 1024.0));
    } else if (mixed_skipped > 0) {
        GGML_LOG_INFO(
            "[PLACEMENT-MOE-MULTI] primary executor layout upgrades skipped for %zu mixed-owner expert entries on "
            "device=%d; common planned layout remains authoritative until mixed per-entry layout dispatch is "
            "available\n",
            mixed_skipped, device_id);
    }
}
...
static size_t maybe_upgrade_moe_down_layouts_to_i8(placement_plan & plan,
    ...
    for (size_t i = 0; i < plan.entries.size(); ++i) {
        ...
        considered++;
        if (entry.expert_role != expert_tensor_role::DOWN) {
            skip_not_down++;
            continue;
        }
        ...
        auto & candidate   = by_tensor[entry.name];
        candidate.name     = entry.name;
        candidate.layer_id = entry.layer_id;
        candidate.indices.push_back(i);
    }

    std::vector<down_candidate> candidates;
    candidates.reserve(by_tensor.size());
    for (auto & item : by_tensor) {
        down_candidate & candidate = item.second;
        if (n_experts > 0 && candidate.indices.size() != static_cast<size_t>(n_experts)) {
            continue;
        }
        bool   complete     = !candidate.indices.empty();
        size_t extra_bytes  = 0;
        size_t extra_charge = 0;
        for (size_t idx : candidate.indices) {
            const placement_entry & entry = plan.entries[idx];
            const size_t            i8_bytes =
                planner_layout_bytes_for_dims(entry.type, entry.ne[0], entry.ne[1], GGML_LAYOUT_MXFP4_I8, 0);
            if (i8_bytes == 0 || i8_bytes <= entry.dst_size) {
                complete = false;
                break;
            }
            ...
            extra_bytes += i8_bytes - entry.dst_size;
            extra_charge += preserve_primary_soa ? new_charge : (new_charge > old_charge ? new_charge - old_charge : 0);
        }
        if (!complete || extra_charge == 0) {
            continue;
        }
        candidate.extra_bytes  = extra_bytes;
        candidate.extra_charge = extra_charge;
        candidates.push_back(std::move(candidate));
    }

    std::sort(candidates.begin(), candidates.end(), [](const down_candidate & a, const down_candidate & b) {
        if (a.layer_id != b.layer_id) {
            return a.layer_id < b.layer_id;
        }
    ...
    for (const down_candidate & candidate : candidates) {
        ...
        remaining -= candidate.extra_charge;
        plan.weight_vram_bytes += candidate.extra_charge;
        plan.vram_bytes += candidate.extra_charge;
        charged_bytes += candidate.extra_charge;
        upgraded_tensors++;
        upgraded_entries += candidate.indices.size();
        ...
        for (size_t idx : candidate.indices) {
            ...
            entry.dst_size = planner_layout_bytes_for_dims(entry.type, entry.ne[0], entry.ne[1], GGML_LAYOUT_MXFP4_I8,
                                                           entry.dst_size);
    ...
    if (!candidates.empty()) {
        GGML_LOG_INFO(
            "[PLACEMENT-MOE] layout upgrades: mxfp4_i8_down tensors=%zu/%zu expert_entries=%zu "
            "preserved_soa=%zu charged=%.1f MB remaining=%.1f MB guard=%.1f MB\n",
            upgraded_tensors, candidates.size(), upgraded_entries, preserved_soa, charged_bytes / (1024.0 * 1024.0),
            remaining / (1024.0 * 1024.0), k_layout_upgrade_guard / (1024.0 * 1024.0));
        if (moe_direct_trace_enabled()) {
            fprintf(stderr,
                    "[PLACEMENT-MOE-DOWN-I8-TRACE] device=%d tensors=%zu/%zu expert_entries=%zu preserved_soa=%zu "
                    "charged=%.1fMB remaining=%.1fMB guard=%.1fMB\n",
                    device_id, upgraded_tensors, candidates.size(), upgraded_entries, preserved_soa,
        ...
        for (const auto & item : by_tensor) {
            if (n_experts > 0 && item.second.indices.size() != static_cast<size_t>(n_experts)) {
                incomplete_tensors++;
            }
        }
        fprintf(stderr,
    ...
    {
        // The two triggers deliberately use different populations. declined_all is
        // computed over `considered` (entry-level, all expert roles) so it still
        // fires when candidate construction rejected everything and candidates is
        // empty; declined_some is computed over candidates.size() (tensor-level,
        // down-role survivors) because that is the only population the upgrade
        // loop iterates. Together they cover any decline.
        ...
        if (planner_moe_layout_debug_enabled() || declined_all || declined_some) {
            ...
            if (declined_all) {
                ...
                GGML_LOG_INFO(
                    "[MOE-LAYOUT] down-i8 upgraded only %zu of %zu candidates -- the other %zu stay on SOA. "
                    "These candidates passed every eligibility check, so this is the VRAM headroom guard "
                    "(%.1f MB left, %.1f MB reserve), not an eligibility failure.\n",
                    upgraded_tensors, candidates.size(), candidates.size() - upgraded_tensors, remaining_mb,
                    k_layout_upgrade_guard / (1024.0 * 1024.0));
            }
        }
    }
...
static size_t maybe_upgrade_moe_gate_up_layouts_to_i8(placement_plan & plan,
                                                      size_t &         remaining,
                                                      int              device_id,
                                                      int              n_experts) {
    constexpr size_t k_layout_upgrade_guard = 64ull * 1024ull * 1024ull;

    struct gate_up_candidate {
        int                 layer_id = -1;
        std::vector<size_t> gate_indices;
        std::vector<size_t> up_indices;
        size_t              extra_charge = 0;
    };
    ...
    for (size_t i = 0; i < plan.entries.size(); ++i) {
        ...
        auto & candidate   = by_layer[entry.layer_id];
        candidate.layer_id = entry.layer_id;
        if (entry.expert_role == expert_tensor_role::GATE) {
            candidate.gate_indices.push_back(i);
        } else {
            candidate.up_indices.push_back(i);
        }
    }

    std::vector<gate_up_candidate> candidates;
    candidates.reserve(by_layer.size());
    for (auto & item : by_layer) {
        ...
        bool   complete     = !candidate.gate_indices.empty() && !candidate.up_indices.empty();
        size_t extra_charge = 0;
        auto   add_role     = [&](const std::vector<size_t> & indices) {
            for (size_t idx : indices) {
                const placement_entry & entry = plan.entries[idx];
                const size_t            i8_bytes =
                    planner_layout_bytes_for_dims(entry.type, entry.ne[0], entry.ne[1], GGML_LAYOUT_MXFP4_I8, 0);
        ...
        add_role(candidate.gate_indices);
        if (complete) {
    ...
    std::sort(candidates.begin(), candidates.end(),
              [](const gate_up_candidate & a, const gate_up_candidate & b) { return a.layer_id < b.layer_id; });

    size_t upgraded_layers  = 0;
    size_t upgraded_entries = 0;
    size_t charged_bytes    = 0;
    for (const gate_up_candidate & candidate : candidates) {
        if (remaining <= k_layout_upgrade_guard || candidate.extra_charge > remaining - k_layout_upgrade_guard) {
            continue;
        }
        remaining -= candidate.extra_charge;
        ...
        charged_bytes += candidate.extra_charge;
        upgraded_layers++;
        upgraded_entries += candidate.gate_indices.size() + candidate.up_indices.size();
        ...
        upgrade_role(candidate.up_indices);
    }

    if (!candidates.empty()) {
        GGML_LOG_INFO(
            "[PLACEMENT-MOE] layout upgrades: mxfp4_i8_gate_up layers=%zu/%zu expert_entries=%zu "
            "charged=%.1f MB remaining=%.1f MB guard=%.1f MB\n",
            upgraded_layers, candidates.size(), upgraded_entries, charged_bytes / (1024.0 * 1024.0),
            remaining / (1024.0 * 1024.0), k_layout_upgrade_guard / (1024.0 * 1024.0));
    } else if (moe_direct_trace_enabled() && considered > 0 && skip_executor > 0) {
        fprintf(stderr,
                "[PLACEMENT-MOE-GATE-UP-I8-TRACE] device=%d considered=%zu skip_executor=%zu "
...
static void populate_host_zone_sizing(placement_plan &                           plan,
    ...
    if (n_experts > 0) {
        ...
        for (const auto & item : tensor_inventory) {
            if (expert_tensor_role_from_tensor_name(item.name.c_str()) != expert_tensor_role::UNKNOWN) {
                max_moe_tensor_bytes = std::max(max_moe_tensor_bytes, item.size);
            }
        }
    ...
    if (g_sycl_tp_config.enabled && g_sycl_tp_config.world_size > 1) {
        ...
        for (const auto & item : tensor_inventory) {
            const tensor_usage usage = infer_tensor_usage(item.name.c_str());
            if (usage == tensor_usage::FFN_WEIGHT) {
                max_ffn_weight_bytes = std::max(max_ffn_weight_bytes, item.size);
            } else if (usage == tensor_usage::ATTENTION_WEIGHT) {
                max_attn_weight_bytes = std::max(max_attn_weight_bytes, item.size);
            }
        }
    ...
    if (plan.device_id >= 0) {
        plan.pp_moe_onednn_weight_slot_bytes =
            unified_cache_get_planned_pp_moe_onednn_weight_slot_bytes(plan.device_id);
        plan.pp_moe_onednn_activation_slot_bytes =
            unified_cache_get_planned_pp_moe_onednn_activation_slot_bytes(plan.device_id);
        plan.pp_moe_onednn_output_slot_bytes =
            unified_cache_get_planned_pp_moe_onednn_output_slot_bytes(plan.device_id);
        plan.pp_moe_onednn_scratch_bytes = unified_cache_get_planned_pp_moe_onednn_scratch_bytes(plan.device_id);
        plan.pp_moe_onednn_ring_depth    = unified_cache_get_planned_pp_moe_onednn_ring_depth(plan.device_id);
    }
    if (plan.pp_moe_onednn_scratch_bytes > 0) {
        GGML_LOG_INFO(
            "[SYCL-PLAN] PP MoE oneDNN scratch ring: depth=%u total=%.1f MB "
            "weight_slot=%.1f MB activation_slot=%.1f MB output_slot=%.1f MB\n",
            plan.pp_moe_onednn_ring_depth, plan.pp_moe_onednn_scratch_bytes / (1024.0 * 1024.0),
            plan.pp_moe_onednn_weight_slot_bytes / (1024.0 * 1024.0),
            plan.pp_moe_onednn_activation_slot_bytes / (1024.0 * 1024.0),
...
placement_plan compute_placement_plan(const std::vector<std::pair<std::string, size_t>> & tensor_inventory,
                                      ...
                                      const ggml_sycl_placement_envelope *                envelope,
                                      int                                                 n_experts) {
    return compute_placement_plan(placement_tensor_inventory_from_pairs(tensor_inventory), vram_budget, device_id,
...
placement_plan compute_placement_plan(const std::vector<placement_tensor_info> & tensor_inventory,
    ...
    plan.pp_moe_onednn_output_slot_bytes     = unified_cache_get_planned_pp_moe_onednn_output_slot_bytes(device_id);
    plan.pp_moe_onednn_scratch_bytes         = unified_cache_get_planned_pp_moe_onednn_scratch_bytes(device_id);
    plan.pp_moe_onednn_ring_depth            = unified_cache_get_planned_pp_moe_onednn_ring_depth(device_id);

    std::map<int, bool>   layer_has_attention;
    std::map<int, size_t> layer_weight_bytes;
    std::map<int, size_t> layer_weight_charge_bytes;
    ...
    for (const placement_tensor_info & tensor : tensor_inventory) {
        ...
        const size_t        src_size = tensor.size;
        ...
        if (n_experts > 0 && usage == tensor_usage::MOE_EXPERT_WEIGHT) {
            ...
            for (int e = 0; e < n_experts; ++e) {
                ...
                entry.dst_size         = dst_size != 0 ? dst_size : expert_size;
                entry.layout           = layout;
                entry.vram_charge_size = placement_vram_charge_bytes(entry.dst_size);
                entry.kv_size          = 0;
                entry.layer_id         = layer_id;
        ...
        if (entry.layer_id >= 0) {
            ...
            layer_weight_charge_bytes[entry.layer_id] += entry.vram_charge_size;
    ...
    if (vram_arena_enabled()) {
        ...
        if (cache && cache->arena_active()) {
            ...
            // The arena may be capped below the public VRAM budget to leave
    ...
    for (size_t idx : standalone_dense_indices) {
        ...
        if (charge <= remaining) {
            ...
            remaining -= charge;
            plan.weight_vram_bytes += charge;
            plan.vram_bytes += charge;
        } else {
            entry.on_device     = false;
            entry.target_device = -1;
    ...
    // domain or mark the triplet as an explicit host/CPU domain.
    {
        ...
        if (!planner_xmx_moe_forced()) {
            prepare_single_moe_pp_gate_up_primary_layouts_for_pack(plan, device_id, kv_info, envelope);
        }
        auto   moe_groups             = build_moe_triplet_groups(plan, moe_indices, &hotness_source);
        size_t pp_alt_groups          = 0;
        size_t pp_alt_entries         = 0;
        size_t pp_alt_charge          = 0;
        size_t pp_alt_no_layout_bytes = 0;
        for (moe_triplet_group & group : moe_groups) {
            const size_t group_pp_alt_charge = planner_moe_group_pp_soa_alternate_charge(
                plan, group, device_id, kv_info, envelope, &pp_alt_no_layout_bytes);
            if (group_pp_alt_charge == 0) {
                continue;
            }
            group.charge_bytes += group_pp_alt_charge;
            pp_alt_groups++;
            pp_alt_charge += group_pp_alt_charge;
            for (size_t idx : group.indices) {
                if (idx < plan.entries.size() &&
                    planner_moe_layout_needs_pp_soa_on_device(plan.entries[idx], plan.entries[idx].layout, device_id,
                                                              kv_info, envelope)) {
                    pp_alt_entries++;
                }
            }
        }
        if (pp_alt_groups > 0 || pp_alt_no_layout_bytes > 0) {
            GGML_LOG_INFO(
                "[PLACEMENT-MOE] PP executable alternate reserve: device=%d groups=%zu entries=%zu charge=%.1f MB "
                "no_layout_bytes=%zu\n",
                device_id, pp_alt_groups, pp_alt_entries, pp_alt_charge / (1024.0 * 1024.0), pp_alt_no_layout_bytes);
        }
        ...
        for (const auto & group : moe_groups) {
            ...
            if (on_device) {
                ...
                plan.vram_bytes += group.charge_bytes;
                stats.device_groups++;
...
//      a layer go entirely to the preferred owner while it has budget, then
//      overflow as complete layers to another GPU before host fallback.
//   3. For MoE EXPERT tensors: place complete gate/up/down expert triplets
//      with the layer execution owner while budget allows, then spill to the
//      next device with enough remaining budget.  This keeps decode locality
//      first and uses secondary GPUs only when capacity requires it.
...
placement_plan compute_multi_device_plan(const std::vector<device_budget> &                  device_budgets,
                                         ...
                                         const ggml_sycl_placement_envelope *                envelope,
                                         int                                                 n_experts) {
    return compute_multi_device_plan(device_budgets, placement_tensor_inventory_from_pairs(tensor_inventory), n_layers,
                                     mode, kv_info, envelope, n_experts);
}
...
placement_plan compute_multi_device_plan(const std::vector<device_budget> &         device_budgets,
    ...
    for (const auto & db : device_budgets) {
        ...
        plan.pp_moe_onednn_weight_slot_bytes =
            std::max(plan.pp_moe_onednn_weight_slot_bytes,
                     unified_cache_get_planned_pp_moe_onednn_weight_slot_bytes(db.device_id));
        plan.pp_moe_onednn_activation_slot_bytes =
            std::max(plan.pp_moe_onednn_activation_slot_bytes,
                     unified_cache_get_planned_pp_moe_onednn_activation_slot_bytes(db.device_id));
        plan.pp_moe_onednn_output_slot_bytes =
            std::max(plan.pp_moe_onednn_output_slot_bytes,
                     unified_cache_get_planned_pp_moe_onednn_output_slot_bytes(db.device_id));
        plan.pp_moe_onednn_scratch_bytes = std::max(
            plan.pp_moe_onednn_scratch_bytes, unified_cache_get_planned_pp_moe_onednn_scratch_bytes(db.device_id));
        plan.pp_moe_onednn_ring_depth =
            std::max(plan.pp_moe_onednn_ring_depth, unified_cache_get_planned_pp_moe_onednn_ring_depth(db.device_id));
    }
    ...
    for (size_t d = 0; d < n_devs; d++) {
        ...
        // In GLOBAL cache mode there is only one physical cache/arena.  A
        // multi-device plan may still be built from synthetic budgets in unit
        // tests, so do not reuse the global device-0 arena capacity for every
        // requested device.
        if (vram_arena_enabled()) {
            auto * cache = get_effective_mode() == unified_cache_mode::GLOBAL ?
                               nullptr :
                               get_existing_cache_for_device(device_budgets[d].device_id);
            if (cache && cache->arena_active()) {
                arena_total     = cache->arena_total_size();
                scratch_reserve = cache->zone_capacity(vram_zone_id::SCRATCH);
                onednn_reserve  = cache->zone_capacity(vram_zone_id::ONEDNN);
                runtime_reserve = cache->zone_capacity(vram_zone_id::RUNTIME);
                // Match the actual allocation surface used by planned
    ...
    // Step 1: Compute legacy layer-to-device assignment for explicit layer/hybrid
    // modes. Dense weight placement itself is scored below by device capability
    // and capacity; VRAM size is only a fit constraint.
    ...
    //   EXPERT: Dense to primary device; MoE experts stay primary while capacity permits, then spill.
    //   HYBRID: Dense by layer owner; MoE experts prefer layer owner, spill by capacity.
    ...
    auto commit_bytes = [&](int target_dev_idx, size_t weight_bytes, size_t weight_charge, size_t kv_cost) {
        const size_t total_cost = weight_charge + kv_cost;
    ...
    std::sort(dense_order.begin(), dense_order.end(), [&](int a, int b) {
        const auto & da    = device_budgets[static_cast<size_t>(a)];
        const auto & db    = device_budgets[static_cast<size_t>(b)];
        const double net_a = dense_net_score[static_cast<size_t>(a)];
        const double net_b = dense_net_score[static_cast<size_t>(b)];
        if (net_a != net_b) {
            return net_a > net_b;
        }
        if (da.dense_score != db.dense_score) {
            return da.dense_score > db.dense_score;
        }
        if (da.vram_budget != db.vram_budget) {
            return da.vram_budget > db.vram_budget;
        }
        return da.device_id < db.device_id;
    });
    ...
    for (int rank = 0; rank < static_cast<int>(dense_order.size()); ++rank) {
        const int    idx = dense_order[static_cast<size_t>(rank)];
    ...
    auto choose_dense_target_device = [&](size_t total_cost, const char ** reason) -> int {
        for (int idx : dense_order) {
        ...
        if (reason) {
            *reason = "no-compatible-gpu-capacity";
        }
        return -1;
    };

    std::vector<size_t> dense_assigned_charge(n_devs, 0);
    std::vector<size_t> dense_assigned_bytes(n_devs, 0);
    std::vector<size_t> dense_group_count(n_devs, 0);
    std::vector<size_t> dense_rank_group_count(n_devs, 0);
    size_t              dense_net_best_groups = 0;
    size_t              dense_spill_groups    = 0;
    ...
    if (use_cohesive_no_p2p_moe) {
        ...
        apply_multi_moe_per_target_layouts(plan, remaining, device_budgets,
                                           /*static_primary_xmx_executor_supported=*/false);
        if (!device_budgets.empty() && !remaining.empty()) {
            const size_t primary_down_charged =
                maybe_upgrade_moe_down_layouts_to_i8(plan, remaining[0], device_budgets[0].device_id, n_experts,
                                                     /*static_i8_executor_supported=*/false);
...

        {
            ...
            if (n_devs == 1) {
                for (size_t d = 0; d < n_devs; ++d) {
                    size_t down_charged = 0;
                    if (!planner_moe_prompt_down_specialized_layouts_enabled()) {
                        add_single_moe_pp_executable_alternates(plan, remaining[d], device_budgets[d].device_id,
                                                                kv_info, envelope, &plan.per_device_vram[d]);
                        down_charged = maybe_upgrade_moe_down_layouts_to_i8(plan, remaining[d],
                                                                            device_budgets[d].device_id, n_experts);
                    } else {
                        down_charged = maybe_upgrade_moe_down_layouts_to_i8(plan, remaining[d],
                                                                            device_budgets[d].device_id, n_experts);
                        add_single_moe_pp_executable_alternates(plan, remaining[d], device_budgets[d].device_id,
                ...
                // secondary executor today. Keep static I8 planning disabled for
                // multi-device plans until the route-aware executor is proven.
                const bool multi_static_i8_executor_supported = false;
                add_multi_moe_decode_secondary_alternates(plan, remaining, device_budgets, moe_device_scores);
                size_t primary_down_charged = 0;
                if (!planner_moe_prompt_down_specialized_layouts_enabled()) {
                    add_multi_moe_pp_executable_alternates(plan, remaining, device_budgets, kv_info, envelope);
                    primary_down_charged = maybe_upgrade_moe_down_layouts_to_i8(
    ...
    // device_layers: per-device dense layer range from actual planned owners.
    for (size_t d = 0; d < n_devs; d++) {
        int dev_id = device_budgets[d].device_id;
        int first = n_layers, last = -1;
        for (int l = 0; l < n_layers; l++) {
            auto it = plan.layer_device.find(l);
            if (it != plan.layer_device.end() && it->second == dev_id) {
    ...
    for (const auto & entry : plan.entries) {
        if (!placement_priority_is_moe_weight(entry.priority)) {
            continue;
        }
        if (entry.layer_id < 0) {
            continue;
    ...
    for (size_t d = 0; d < n_devs; d++) {
        ...
        GGML_LOG_INFO(
            "[PLACEMENT-MULTI] Device logical=%zu physical=%d raw_dense_score=%.2f layers [%d, %d], "
            "%.1f MB planned bytes used (%.1f MB budget)\n",
            d, device_budgets[d].device_id, device_budgets[d].dense_score, first_layer, last_layer,
            plan.per_device_vram[d] / (1024.0 * 1024.0), device_budgets[d].vram_budget / (1024.0 * 1024.0));
    }
