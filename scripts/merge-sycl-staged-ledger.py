...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if status not in {
            ...
            "pti_instcount_runtime_cost",
            "asm_line_static_cost",
            "dwarf_line_table_only",
        }:
        ...
        if status == "gtpin_bbl_runtime_cost" and source_line_status != "gtpin-bbl-runtime-cost":
            ...
            return 2
        if status == "pti_instcount_runtime_cost" and source_line_status != "pti-instcount-runtime-cost":
            print("coverage.layer_status source_attribution_incomplete")
            for line in source_line_rows:
                print(line)
            for line in source_attribution_rows:
                print(line)
            print("failed to merge staged ledger: PTI instcount source attribution requires source_line.status pti-instcount-runtime-cost")
            return 2
