...
static int run_mmq_bench() {
    ...
    if (!sycl_backend) {
        ...
        return LLAMA_TEST_EXIT_SKIP;  // ctest SKIP_RETURN_CODE: this mode requires a SYCL backend
    }
...
int main() {
    ...
    if (!enable || std::atoi(enable) == 0) {
        ...
        return LLAMA_TEST_EXIT_SKIP;  // ctest SKIP_RETURN_CODE: the opt-in benchmark is disabled
    }
