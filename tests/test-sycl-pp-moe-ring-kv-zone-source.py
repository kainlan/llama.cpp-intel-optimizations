...
MUTATIONS = [
    ...
    ("admission before the release", "sycl",
     "if (!cache->release_pp_moe_onednn_scratch_ring()) {", "if (!true) {",
     "the admission reads the KV zone before the old ring is released"),
    ("RUNTIME not net of the MMID pools", "sycl",
     "const size_t runtime_pending   = arena && kv_zone ? kv_zone->runtime_pending_bytes : 0;",
     "const size_t runtime_pending   = 0;",
     "the admission is not given the RUNTIME zone less what follows the ring"),
    ...
    ("output zone not recorded", "cache",
     "slot.output_in_kv_zone     = arena_active() && output_zone == vram_zone_id::KV;",
     "slot.output_in_kv_zone     = false;",
     "the output slot does not record the zone it was allocated from"),
