...
static bool validate_inline_dot(const GeneratedWeights &     weights,
    ...
    if (max_abs > tol) {
        ...
        oss << "mxfp4_inline_dot validation failed: max_abs=" << max_abs
            << " mean_abs=" << mean_abs << " tol=" << tol;
        error = oss.str();
        return false;
    }
    return true;
...
bool run_mxfp4_inline_dot(const GeneratedWeights &     weights,
    ...
    auto submit_dot = [&]() {
        return queue.submit([&](sycl::handler & cgh) {
            ...
            cgh.parallel_for(sycl::nd_range<2>(sycl::range<2>(out_count, local_size),
    ...
    const double bytes = static_cast<double>(weight_bytes) * static_cast<double>(n) +
